...
ComfyUI 是一个功能强大的、基于节点的 Stable Diffusion 及其他扩散模型界面。与传统的仅提供简单提示框的文生图界面不同，ComfyUI 将整个图像生成流程以可视化图形的方式呈现，让您能够精细控制从文本编码、潜空间操作到最终解码的每一个步骤。
...
#### 创建虚拟环境
在 Linux 上，在您选择的目录中打开终端，并运行以下命令以创建 venv：
...
<!-- @device:halo_box -->
<!-- @os:windows -->
要在 Windows 上启动 ComfyUI，请点击桌面上的 ComfyUI Desktop 启动器。按照步骤安装 AMD 本地版本。

<p align="center">
  <img src="assets/new_installer.png" alt="ComfyUI Desktop Launcher and Installer" width="600"/>
</p>

然后，点击应用顶部中间的 ComfyUI 按钮，这将打开一个设置选项卡。打开"存储"选项卡，确保路径设置如下，以访问预安装的模型。
...
<!-- @device:halo,stx,krk,rx7900xt,rx9070xt,r9700 -->
<!-- @os:windows -->
要在 Windows 上启动 ComfyUI，只需点击桌面上的 ComfyUI 快捷方式。
<!-- @os:end -->
...
| 参数 | 控制内容 | 推荐值 |
|-----------|------------------|-------------------|
| **shift** | 调整噪声调度时序——较高的值会将更多细节细化推迟到后期步骤 | 1.0–4.0（默认值为 3.0） |
...
AuraFlow 采样方法专为 Z-Image Turbo 等流匹配模型设计，确保在整个生成过程中噪声分布的合理性。

## 使用工作流

### 保存工作流

点击菜单中的 **Save** 按钮，将您的工作流导出为 JSON 文件。此操作将保存：
...
- **构建自定义工作流**：串联多次生成、添加放大处理或创建图像变体
- **浏览社区工作流**：[ComfyUI Examples](https://github.com/comfyanonymous/ComfyUI_examples) 提供了许多即用型工作流

ComfyUI 的优势在于实验性：以不同方式连接节点、调整参数，并观察每次变化对输出的影响。这种动手探索有助于建立对扩散模型工作原理的直觉。
