...
> **Consejo**: Mantén la ventana de la terminal abierta mientras usas ComfyUI. Cerrarla detendrá el servidor.
<!-- @os:end -->
<!-- @device:end -->
...
Antes de generar imágenes, necesitas cargar la plantilla de Z-Image Turbo. Aquí te explicamos cómo encontrarla:
...
El flujo de trabajo de Z-Image Turbo utiliza cuatro componentes de modelo clave que trabajan juntos:

| Componente | Función |
|-----------|------|
| **Codificador de texto** (Qwen 3 4B) | Convierte tu prompt de texto en embeddings que el modelo de difusión comprende |
| **Modelo de difusión** (Z-Image Turbo) | La red neuronal central que elimina el ruido iterativamente de las representaciones latentes para generar imágenes |
...
1. **Ingresa tu prompt** en el nodo Z-Image principal. Sé descriptivo. Aquí hay un ejemplo:
   ```
...
La ejecución completa del flujo de trabajo debería completarse en menos de 30 segundos. Tu imagen generada aparece en el nodo **Save Image** y se guarda en la carpeta `output/`.
...
| **cfg** | Escala de guía sin clasificador—qué tan de cerca seguir el prompt | 1.0–2.0 (los modelos turbo usan una guía muy baja) |
...
Para ajustar las dimensiones de salida, encuentra el nodo **Empty Latent Image** y modifica **width** y **height**. Mantén las dimensiones en 1024 píxeles o menos en el lado más largo para una calidad óptima.
...
|-----------|------------------|-------------------|
| **shift** | Ajusta el tiempo de la programación de ruido—valores más altos empujan más el refinamiento de detalles hacia los pasos posteriores | 1.0–4.0 (el valor predeterminado es 3.0) |

Cuándo ajustar **shift**:

- **Valores bajos (1.0–2.0)**: Convergencia más rápida, buena para composiciones simples
- **Valores altos (3.0–4.0)**: Refinamiento más gradual, puede mejorar los detalles finos en escenas complejas

El método de muestreo AuraFlow está diseñado específicamente para modelos de coincidencia de flujo como Z-Image Turbo, asegurando una distribución adecuada del ruido a lo largo del proceso de generación.
...
Arrastra un archivo JSON de flujo de trabajo al lienzo, o usa **Load** desde el menú. El flujo de trabajo de Z-Image Turbo que ves por defecto se carga desde un archivo de flujo de trabajo guardado.
...
- **Explorar nodos LoRA**: Aplica adaptadores de estilo o sujeto sin reentrenar
- **Agregar prompts negativos**: Conecta un segundo nodo CLIP Text Encode a la entrada de condicionamiento **negative** del KSampler para guiar al modelo a alejarse de características no deseadas como desenfoque, artefactos o marcas de agua
- **Construir flujos de trabajo personalizados**: Encadena múltiples generaciones, agrega escalado de resolución o crea variaciones de imágenes
- **Explorar flujos de trabajo de la comunidad**: [ComfyUI Examples](https://github.com/comfyanonymous/ComfyUI_examples) tiene muchos flujos de trabajo listos para usar

La fortaleza de ComfyUI es la experimentación: conecta nodos de manera diferente, ajusta parámetros y observa cómo cada cambio afecta el resultado. Esta exploración práctica desarrolla la intuición sobre cómo funcionan los modelos de difusión.
