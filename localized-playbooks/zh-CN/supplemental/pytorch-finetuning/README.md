...
```bash
pip install transformers==4.57.1 safetensors==0.6.2 accelerate peft trl bitsandbytes "fsspec[http]>=2023.1.0,<=2025.9.0" "modelscope[datasets]"
```
<!-- @test:end -->
<!-- @os:end -->
...
<!-- @test:id=install-deps timeout=300 setup=activate-venv -->
```bash
pip install transformers==4.57.1 safetensors==0.6.2 datasets==4.2.0 accelerate peft trl "fsspec[http]>=2023.1.0,<=2025.9.0" "modelscope[datasets]"
```
<!-- @test:end -->
<!-- @os:end -->
...
在本示例中，我们使用 **google/gemma-3-4b-it**，这是一个**公开（public）** 模型。对于**受限（gated）** 模型，您必须先在 Hugging Face 上接受该模型的使用条款，然后进行身份验证，训练脚本才能下载该模型。
...
微调会教会模型如何回应要求提供某个主题名言的提示，并以 `<quote text> - <author>` 的格式返回结果。LoRA 和全量微调脚本使用 **AI-ModelScope/databricks-dolly-15k**（通用的指令/回复对），因此具体任务因脚本而异；但思路是相同的——让模型适应您选择的数据集和格式。
