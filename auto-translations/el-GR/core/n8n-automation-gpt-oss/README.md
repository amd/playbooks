...
<!-- @github-only -->
> [!IMPORTANT]
> Αυτό το playbook χρησιμοποιεί ειδικές ετικέτες που το GitHub δεν μπορεί να αποδώσει. Επισκεφτείτε το [amd.com/playbooks](https://amd.com/playbooks) για να δείτε σωστά αυτό το περιεχόμενο.
<!-- @github-only:end -->
...
Αυτό το playbook σας διδάσκει πώς να ρυθμίσετε έναν συνοψιστή οικονομικών ειδήσεων με τεχνητή νοημοσύνη που κάνει scraping στην ενότητα επιχειρήσεων του AP News, εξάγει βασικούς τίτλους, και χρησιμοποιεί ένα τοπικό LLM που εκτελείται στο σύστημά σας για να δημιουργήσει μια περίληψη προσανατολισμένη σε επενδυτές.

## Τι Θα Μάθετε

- Πώς να εγκαταστήσετε και να εκκινήσετε το n8n
- Εισαγωγή και ρύθμιση μιας προκατασκευασμένης ροής εργασίας
- Σύνδεση στο Lemonade χρησιμοποιώντας την εγγενή ενσωμάτωση n8n
- Κατανόηση των κόμβων ροής εργασίας και της ροής δεδομένων

## Τι είναι το Lemonade;

Το [Lemonade](https://lemonade-server.ai) είναι μια πλατφόρμα τοπικής εξυπηρέτησης LLM κατασκευασμένη για υλικό AMD. Παρέχει ένα API συμβατό με OpenAI που εκτελείται εξ ολοκλήρου στο μηχάνημά σας—τα δεδομένα σας δεν φεύγουν ποτέ από τη συσκευή σας.
...
Το n8n περιλαμβάνει έναν **εγγενή κόμβο Lemonade** (`Lemonade Chat Model`) που παρέχει μια ενσωμάτωση πρώτης κατηγορίας - χωρίς ανάγκη για χειροκίνητη ρύθμιση. Αυτό κάνει τη σύνδεση του τοπικού σας LLM σε ροές εργασίας αυτοματοποίησης απλή υπόθεση.
...
<!-- @os:windows -->
> **Ζήτημα PATH**: Εάν η εντολή `n8n --version` αναφέρει ότι δεν βρέθηκε η εντολή, βεβαιωθείτε ότι ο καθολικός κατάλογος bin του npm βρίσκεται στο `PATH` του χρήστη. Η συνήθης διαδρομή εγκατάστασης είναι στο `C:\Users\<username>\AppData\Roaming\npm`.
...
<!-- @os:linux -->
Θα χρησιμοποιήσουμε τώρα την υπηρεσία Podman για να κάνουμε containerize την εγκατάσταση του n8n.

Παρακαλώ κατεβάστε τα ακόλουθα σε έναν κατάλογο της επιλογής σας: [compose.yml](assets/compose.yml)
...
<!-- @os:windows -->
Το n8n εκκινεί έναν τοπικό διακομιστή web. Πατήστε `'o'` ή ανοίξτε τον browser σας στο `http://localhost:5678` για να αποκτήσετε πρόσβαση στον επεξεργαστή.
<!-- @os:end -->


> **Συμβουλή**: Κρατήστε το παράθυρο τερματικού ανοιχτό ενώ χρησιμοποιείτε το n8n. Το κλείσιμό του ενδέχεται να διακόψει τον διακομιστή.
...
<!-- @os:windows -->
Ανοίξτε το GUI του Lemonade κάνοντας κλικ στο εικονίδιο Lemonade. Κάντε δεξί κλικ στο εικονίδιο της γραμμής συστήματος για να ανοίξετε την εφαρμογή. Στη συνέχεια, μπορείτε να προσθέσετε μοντέλα, backends, και να φορτώσετε τα προεγκατεστημένα μοντέλα.
<!-- @os:end -->

>**Συμβουλή**: Μόλις εκτελείται, το GUI του Lemonade είναι επίσης προσβάσιμο στη διεύθυνση http://localhost:13305
...
1. Ανοίξτε το `http://localhost:5678` στον browser σας
...
2. Κάντε κλικ στο **Start from Scratch** για να ανοίξετε τον επεξεργαστή ροής εργασίας. Εναλλακτικά, κάντε κλικ στο κουμπί + πάνω αριστερά, και έπειτα στο **Add workflow**.
3. Κάντε κλικ στο μενού **...** (τρεις τελείες) στην πάνω δεξιά γραμμή και επιλέξτε **Import from file**
...
| **Fetch Financial News Webpage** | Αίτημα HTTP GET προς `https://apnews.com/business` |
| **Delay to Ensure Page Load** | Κόμβος αναμονής για να διασφαλιστεί η πλήρης φόρτωση του περιεχομένου της σελίδας |
| **Extract News Headlines & Text** | Κόμβος HTML που εξάγει τίτλους, επιλογές συντακτών, κύριες ειδήσεις και τοπικά νέα χρησιμοποιώντας επιλογείς CSS |
...
> **Σημείωση**: Πριν κάνετε τη δοκιμή, εκτελέστε `lemonade status` σε ένα τερματικό για να επιβεβαιώσετε ότι ο διακομιστής Lemonade εκτελείται.
<!-- @device:halo_box -->
...
5. Κάντε διπλό κλικ στον κόμβο **Convert to File** για να κατεβάσετε το αντίστοιχο αρχείο κειμένου στο κάτω πλαίσιο.
...
Ο agent λαμβάνει τα καθαρισμένα δεδομένα ειδήσεων και παράγει μια δομημένη σύνοψη με το κλίμα της αγοράς.

### Αποθήκευση της Ροής Εργασίας σας

Κάντε κλικ στο όνομα της ροής εργασίας στο επάνω μέρος και μετονομάστε το αν το επιθυμείτε. Οι ροές εργασίας αποθηκεύονται αυτόματα καθώς εργάζεστε.
