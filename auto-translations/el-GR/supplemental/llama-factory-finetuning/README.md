...
<!-- @device:stx,krk -->
> **Σημείωση:** Οι τεχνικές fine-tuning σε αυτό το playbook απαιτούν τουλάχιστον **32 GB μνήμης συστήματος (RAM)**, με τουλάχιστον **16 GB από αυτά διαθέσιμα στην GPU** (τα 16 GB αποτελούν μέρος των 32 GB, όχι επιπλέον αυτών).
<!-- @device:end -->


<!-- @device:rx7900xt,rx9070xt,r9700 -->
<!-- @os:windows -->
> **Σημείωση:** Οι τεχνικές fine-tuning σε αυτό το playbook απαιτούν τουλάχιστον **16 GB συνολικής μνήμης GPU** και **32 GB μνήμης συστήματος (RAM)**.
> - Στα Windows, η συνολική μνήμη GPU συνδυάζει την αποκλειστική μνήμη VRAM της κάρτας γραφικών με την κοινόχρηστη μνήμη GPU (η οποία δανείζεται από τη μνήμη συστήματος).
...
<!-- @os:linux -->
> **Σημείωση:** Οι τεχνικές fine-tuning σε αυτό το playbook απαιτούν κάρτα γραφικών με τουλάχιστον **16 GB αποκλειστικής μνήμης GPU** και **32 GB μνήμης συστήματος (RAM)**.
> - Στο Linux, η εκπαίδευση εκτελείται εξ ολοκλήρου στην αποκλειστική VRAM της κάρτας γραφικών.
> - Δεν γίνεται επιστροφή σε κοινόχρηστη μνήμη GPU (μνήμη συστήματος) όταν εξαντλείται η VRAM.
> - Κάρτες με λιγότερα από 16 GB αποκλειστικής VRAM θα εξαντλήσουν τη μνήμη κατά την εκπαίδευση στο Linux, ακόμη και αν το σύστημα διαθέτει άφθονη RAM.
<!-- @os:end -->
<!-- @device:end -->
...
- Πώς να ρυθμίσετε τις παραμέτρους fine-tuning LLM (χρησιμοποιώντας το Qwen/Qwen3-4B-Instruct-2507 ως παράδειγμα)
- Πώς να εκτελέσετε fine-tuning με το LLaMA Factory
- Πώς να εκτελέσετε συμπερασμό (inference) με το προσαρμοσμένο μοντέλο
- Πώς να εξάγετε το προσαρμοσμένο μοντέλο

## Εκτιμώμενος Χρόνος

- Διάρκεια: Θα χρειαστούν περίπου 60 λεπτά για να εκτελέσετε αυτό το playbook (ανάλογα με το μέγεθος του μοντέλου/συνόλου δεδομένων σας και την ταχύτητα δικτύου).
- Δείτε το [LLaMA Factory GitHub](https://github.com/hiyouga/LlamaFactory) για περισσότερες πληροφορίες.

## Ρύθμιση Παραμετροποίησης Μνήμης
...
### Εγκατάσταση Επιπρόσθετων Εξαρτήσεων
...
Έχοντας εγκαταστήσει με επιτυχία το LLaMA Factory, ας εκτελέσουμε fine-tuning σε αυτό.

## Χρήση του LLaMA Factory CLI για Fine-Tuning

Αυτή η ενότητα θα καλύψει τον τρόπο προετοιμασίας συνόλων δεδομένων fine-tuning, τη ρύθμιση παραμέτρων LoRA/QLoRA και την εκτέλεση fine-tuning με LoRA.

### Προετοιμασία Συνόλου Δεδομένων

Το LLaMA Factory υποστηρίζει σύνολα δεδομένων fine-tuning σε μορφή Alpaca και ShareGPT. Όλα τα διαθέσιμα σύνολα δεδομένων έχουν οριστεί στο [dataset_info.json](https://github.com/hiyouga/LlamaFactory/blob/main/data/dataset_info.json). Αν χρησιμοποιείτε προσαρμοσμένο σύνολο δεδομένων, βεβαιωθείτε ότι έχετε προσθέσει μια περιγραφή του συνόλου δεδομένων στο `dataset_info.json` και ότι έχετε καθορίσει το όνομα του συνόλου δεδομένων πριν από την εκπαίδευση. Περισσότερες λεπτομέρειες μπορείτε να βρείτε στην τεκμηρίωσή τους [εδώ](https://llamafactory.readthedocs.io/en/latest/getting_started/data_preparation.html).

Σε αυτό το playbook, θα χρησιμοποιήσουμε τα σύνολα δεδομένων identity και alpaca_en_demo ως παράδειγμα, και θα ρυθμίσουμε τις πληροφορίες του συνόλου δεδομένων στο επόμενο βήμα.
...
- `lora_rank` - Η διαστατικότητα του πίνακα χαμηλής τάξης (low-rank matrix) που χρησιμοποιείται στο LoRA, τυπικές τιμές: 4, 6, 8, 16 (μικρότερες τιμές = λιγότερες παράμετροι = ταχύτερο fine-tuning· μεγαλύτερες τιμές = καλύτερη προσαρμογή στην εργασία αλλά υψηλότερη χρήση πόρων).
...
- `output_dir` - Διαδρομή εξόδου του fine-tuning
- `logging_steps` - Διάστημα καταγραφής σε steps
- `save_steps` - Διάστημα αποθήκευσης checkpoint μοντέλου.
- `overwrite_output_dir` - Εάν επιτρέπεται η αντικατάσταση του output directory.
...
- `warmup_ratio` - Ποσοστό προθέρμανσης (warmup) ρυθμού μάθησης
...
- Ρύθμιση του `lora_rank` από `8` σε `6` για μείωση της χρήσης μνήμης κατά το fine-tuning.
- Χρήση `fp16` αντί για `bf16` για ευρύτερη συμβατότητα με AMD GPUs και χαμηλότερη χρήση μνήμης.
...
### Εκτέλεση LLaMA Factory Fine-Tuning 

Το **llamafactory-cli** είναι το επίσημο εργαλείο διεπαφής γραμμής εντολών (CLI) για το LLaMA Factory, σχεδιασμένο για να απλοποιεί ολοκληρωμένες ροές εργασίας LLM (προετοιμασία δεδομένων → fine-tuning → αξιολόγηση → ανάπτυξη) χωρίς να χρειάζεται να γράφετε πολύπλοκο κώδικα.

Για εκπαίδευση/fine-tuning, το **llamafactory-cli train** είναι η βασική υποεντολή του LLaMA Factory CLI. Αφαιρεί την πολυπλοκότητα των ροών εργασίας fine-tuning (προεπεξεργασία δεδομένων, ρύθμιση υπερπαραμέτρων, βελτιστοποίηση υλικού) σε μία μόνο εντολή CLI, υποστηρίζοντας πολλαπλά παραδείγματα fine-tuning (LoRA/QLoRA/Full Fine-Tuning) και είναι βελτιστοποιημένο για GPUs με χαμηλούς πόρους (π.χ. QLoRA σε 16GB VRAM).

Μπορείτε να εκτελέσετε το fine-tuning του LLaMA Factory χρησιμοποιώντας την παρακάτω εντολή, η οποία βασίζεται στο τροποποιημένο αρχείο ρυθμίσεων για το Qwen3 LoRA fine-tuning.
...
Μετά την εκτέλεση του LLM finetuning, όλα τα παραγόμενα αποτελέσματα αποθηκεύονται στο "output_dir", συμπεριλαμβανομένων των αρχείων checkpoint μοντέλου, των αρχείων ρυθμίσεων και των μετρικών εκπαίδευσης.
...
Το **llamafactory-cli chat** έχει σχεδιαστεί για διαδραστική συνομιλία/inference με LLMs (τόσο για βασικά μοντέλα όσο και για μοντέλα με LoRA fine-tuning). Το LLaMA Factory παρέχει το δείγμα ρύθμισης για την εκτέλεση inference των fine-tuned μοντέλων στο [examples/inference](https://github.com/hiyouga/LlamaFactory/tree/main/examples/inference). Μπορείτε επίσης να τροποποιήσετε αυτό το δείγμα ρύθμισης για να αλλάξετε τις ρυθμίσεις, όπως το backend inference.

Χρησιμοποιήστε την παρακάτω εντολή για να δοκιμάσετε το fine-tuned μοντέλο Qwen3:
...
Το `LLaMA-Factory` υποστηρίζει επίσης βελτιστοποίηση (fine-tuning) LLMs χωρίς κώδικα μέσω μιας διεπαφής ιστού στο πρόγραμμα περιήγησης.
...
```
Το `LlamaFactory Web UI` προσφέρει μια απλοποιημένη διεπαφή για τη διαχείριση ροών εργασίας μηχανικής μάθησης, συμπεριλαμβανομένων της εκπαίδευσης, της αξιολόγησης, της πρόβλεψης, της συνομιλίας και της εξαγωγής μοντέλων. Ακολουθεί μια σύντομη παρουσίαση κάθε καρτέλας:

* **Train**: Αυτή η καρτέλα σάς επιτρέπει να επιλέξετε ένα μοντέλο και ένα σύνολο δεδομένων, να ρυθμίσετε τις παραμέτρους εκπαίδευσης και να ξεκινήσετε τη διαδικασία εκπαίδευσης. Είναι σημαντικό να κατανοήσετε τις υποχρεωτικές και προαιρετικές παραμέτρους για να βελτιστοποιήσετε τη ρύθμιση της εκπαίδευσης.
...
* **Chat**: Μόλις ολοκληρωθεί η εκπαίδευση, φορτώστε το μοντέλο στην καρτέλα Chat για να αλληλεπιδράσετε μαζί του και να δείτε τα αποτελέσματα της δουλειάς σας. Αυτή η δυνατότητα επιτρέπει την επικοινωνία σε πραγματικό χρόνο με το εκπαιδευμένο μοντέλο.
