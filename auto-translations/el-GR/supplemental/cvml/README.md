...
Η [Ryzen AI CVML Library](https://ryzenai.docs.amd.com/en/latest/ryzen_ai_libraries.html#ryzen-ai-cvml-library) είναι ένα AMD C++ toolkit υπολογιστικής όρασης και μηχανικής μάθησης που παρέχει ισχυρές δυνατότητες αντίληψης επί της συσκευής — συμπεριλαμβανομένης της εκτίμησης βάθους, της ανίχνευσης προσώπου και της παρακολούθησης πλέγματος προσώπου. Βασισμένη στους οδηγούς Ryzen AI, η βιβλιοθήκη επιλέγει αυτόματα το καλύτερο διαθέσιμο υλικό (GPU ή NPU) για συμπέρασμα, επιτρέποντάς σας να προσθέτετε λειτουργίες AI σε εφαρμογές C++ χωρίς να ανησυχείτε για εκπαίδευση μοντέλων ή ενσωμάτωση πλαισίου. Όλη η επεξεργασία γίνεται τοπικά στο σύστημά σας, καθιστώντας την ιδανική για εφαρμογές που απαιτούν απόρρητο και χαμηλή καθυστέρηση.

Αυτό το playbook σας διδάσκει πώς να ρυθμίσετε τη Ryzen AI CVML Library, να δημιουργήσετε τις συμπεριλαμβανόμενες δείγμα εφαρμογές και να εκτελέσετε ανίχνευση προσώπου σε μια δείγμα εικόνα.

## Τι θα Μάθετε

- Πώς να εγκαταστήσετε τις προαπαιτούμενες εξαρτήσεις και να ρυθμίσετε τη Ryzen AI CVML Library στο σύστημά σας
- Πώς λειτουργεί το CVML C++ API: contexts, feature objects και image buffers
- Πώς να δημιουργήσετε και να εκτελέσετε τις συμπεριλαμβανόμενες δείγμα εφαρμογές χρησιμοποιώντας CMake και OpenCV
- Πώς να εκτελέσετε ανίχνευση προσώπου σε μια εικόνα με bounding boxes και landmarks
- Πώς να ενσωματώσετε λειτουργίες CVML στις δικές σας εφαρμογές C++
...
<!-- @os:windows -->
- [OpenCV 4.11](https://github.com/opencv/opencv/releases/tag/4.11.0) — κατεβάστε το `opencv-4.11.0-windows.exe`, εκτελέστε το και αποσυμπιέστε σε έναν τοπικό φάκελο (π.χ. `C:\opencv`)
- [CMake](https://cmake.org/download/) — κατεβάστε τον Windows x86-64 MSI installer και κατά την εγκατάσταση επιλέξτε **"Add CMake to the system PATH for all users"**
- [Ryzen AI NPU driver](https://ryzenai.docs.amd.com/en/latest/inst.html) — εγκαταστήστε την πιο πρόσφατη διαθέσιμη έκδοση
- [Visual Studio 2022 Community](https://aka.ms/vs/17/release/vs_community.exe) με το φόρτο εργασίας "Desktop development with C++" (περιλαμβάνει MSVC compiler, Windows SDK και εργαλεία δόμησης C++)
<!-- @os:end -->

<!-- @os:linux -->
- OpenCV 4.11 — πρέπει να δημιουργηθεί από πηγαίο κώδικα (τα πακέτα apt στο Ubuntu 22.04 και 24.04 δεν παρέχουν την έκδοση 4.11). Δείτε [Δημιουργία OpenCV από Πηγαίο Κώδικα](#building-opencv-from-source) παρακάτω.
- CMake — εγκαταστήστε μέσω apt:
  ```bash
  sudo apt install cmake
  ```
- Ubuntu 22.04 ή 24.04 (kernel >= 6.11.0-21-generic)
- [Ryzen AI NPU driver](https://ryzenai.docs.amd.com/en/latest/linux.html#install-npu-drivers) (Linux installer — απαιτείται για συμπέρασμα NPU)
...
Δημιουργήστε έναν λογαριασμό AMD στο [account.amd.com](https://account.amd.com) εάν δεν έχετε ήδη, και στη συνέχεια συνδεθείτε για να κατεβάσετε τη Ryzen AI CVML Library από τον παρακάτω σύνδεσμο πύλης:

```
https://account.amd.com/en/forms/downloads/xef.html?filename=72293_Ryzen_AI_Library_26.05.20.zip
```

Αφού κατεβάσετε, αποσυμπιέστε το πακέτο σε έναν τοπικό κατάλογο (π.χ., `C:\RyzenAI-Library` στα Windows ή `~/RyzenAI-Library` στο Linux) και ορίστε τη μεταβλητή περιβάλλοντος `AMD_CVML_SDK_ROOT` στην τοποθεσία αποσυμπίεσης:
...
| Φάκελος | Περιεχόμενα |
|--------|----------|
| `cmake/` | Πληροφορίες συσκευασίας για τη συνάρτηση `find_package` του CMake |
...
### Ρύθμιση Ειδικά για Linux

#### Δημιουργία OpenCV από Πηγαίο Κώδικα

Εγκαταστήστε τις εξαρτήσεις δόμησης OpenCV:

```bash
sudo apt install unzip wget ubuntu-restricted-extras libunwind-dev libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev libgtk2.0-dev libgtk-3-dev pkg-config ffmpeg
```

Κατεβάστε, διαμορφώστε και δημιουργήστε το OpenCV 4.11.0 με τα modules contrib (αναφορά: [Εκπαιδευτικό εγχειρίδιο εγκατάστασης OpenCV Linux](https://docs.opencv.org/4.11.0/d7/d9f/tutorial_linux_install.html#tutorial_linux_install_quick_build_contrib)):
...
Εάν εκτελείτε Ubuntu 22.04, ενημερώστε επίσης τους οδηγούς MESA Vulkan:
...
#### Πρόσθετες Εξαρτήσεις Ubuntu 24.04

Εάν εκτελείτε Ubuntu 24.04, εγκαταστήστε πρόσθετα απαιτούμενα πακέτα:
...
Η CVML Library παρέχει ένα απλό C++ API όπου κάθε λειτουργία αντίληψης (εκτίμηση βάθους, ανίχνευση προσώπου, πλέγμα προσώπου) έχει το δικό της αρχείο κεφαλίδας και αντικείμενο λειτουργίας. Δεν εργάζεστε με ακατέργαστα μοντέλα — η βιβλιοθήκη χειρίζεται αυτόματα τη φόρτωση μοντέλων, την προεπεξεργασία και το συμπέρασμα.

### Διαθέσιμες Λειτουργίες

| Λειτουργία | Αρχείο Κεφαλίδας | Περιγραφή |
|---------|------------|-------------|
| **Εκτίμηση Βάθους** | `cvml-depth-estimation.h` | Δημιουργεί χάρτες βάθους ανά pixel από εικόνες RGB |
| **Ανίχνευση Προσώπου** | `cvml-face-detector.h` | Ανιχνεύει πρόσωπα με bounding boxes, landmarks (μάτια, μύτη, στόμα) και βαθμολογίες εμπιστοσύνης |
...
1. **Δημιουργία Context** — Το `amd::cvml::Context` διαχειρίζεται κοινόχρηστους πόρους όπως καταγραφή και επιλογή backend συμπεράσματος.
2. **Δημιουργία Αντικειμένου Λειτουργίας** — Δημιουργήστε στιγμιότυπο της συγκεκριμένης λειτουργίας (π.χ. `amd::cvml::DepthEstimation`) έναντι του context.
3. **Αναδίπλωση Δεδομένων Εισόδου** — Χρησιμοποιήστε το `amd::cvml::Image` για να ενθυλακώσετε το buffer εικόνας RGB σας χωρίς αντιγραφή δεδομένων.
4. **Εκτέλεση** — Καλέστε τη μέθοδο επεξεργασίας της λειτουργίας και διαβάστε τα αποτελέσματα.
...
> **Σημείωση:** Οι λειτουργίες που χρησιμοποιούν το backend ONNX για λειτουργίες NPU ενδέχεται να εμφανίσουν μεγαλύτερη καθυστέρηση εκκίνησης κατά την πρώτη εκτέλεση. Οι επόμενες εκτελέσεις θα είναι ταχύτερες.

> **Σημείωση:** Εάν ο οδηγός NPU δεν είναι εγκατεστημένος στο σύστημα προορισμού, η Ryzen AI CVML library θα επιστρέψει αυτόματα στο backend GPU για λειτουργίες συμπεράσματος.

## Δημιουργία των Δείγμα Εφαρμογών

Η CVML Library περιλαμβάνει έτοιμες προς δόμηση δείγμα εφαρμογές για κάθε λειτουργία. Ας τις δημιουργήσουμε όλες ταυτόχρονα.
...
2. Δημιουργήστε τα δείγματα με CMake:
   ...
   Μετά από επιτυχή δόμηση, τα εκτελέσιμα βρίσκονται στο:
...
Το δείγμα ανίχνευσης προσώπου ανιχνεύει πρόσωπα σε μια εικόνα, βίντεο ή ζωντανή τροφοδοσία κάμερας. Σχεδιάζει bounding boxes, βαθμολογίες εμπιστοσύνης και πέντε facial landmarks (δύο μάτια, μύτη και δύο άκρα στόματος) σε κάθε ανιχνευμένο πρόσωπο.

Αρχικά, μεταβείτε στον φάκελο του εκτελέσιμου ανίχνευσης προσώπου:
...
Στη συνέχεια κατεβάστε μια δείγμα εικόνα για χρήση ως είσοδο (φωτογραφία από [Jopwell](https://www.pexels.com/photo/man-in-gray-crew-neck-shirt-smiling-on-focus-photo-895863/), ελεύθερη χρήση μέσω Pexels):

```bash
curl -L -o sample_face.jpg "https://images.pexels.com/photos/895863/pexels-photo-895863.jpeg?cs=srgb&dl=pexels-jopwell-895863.jpg&fm=jpg"
```

**Εκτελέστε ανίχνευση προσώπου στη δείγμα εικόνα:**
...
Θα εμφανιστεί ένα παράθυρο που δείχνει την εικόνα με bounding boxes γύρω από τα ανιχνευμένα πρόσωπα, βαθμολογίες εμπιστοσύνης και σημεία facial landmark (μάτια, μύτη, άκρα στόματος).

<p align="center">
  <img src="assets/human_face_output.png" alt="Face detection output showing bounding box, confidence score, and facial landmarks" width="600"/>
</p>

**Αποθηκεύστε την σχολιασμένη έξοδο σε αρχείο:**
...
**Χρησιμοποιήστε το ακριβές μοντέλο** για υψηλότερη ακρίβεια (σε βάρος της ταχύτητας):
...
| Μοντέλο | Ταχύτητα | Ακρίβεια | Κατάλληλο Για |
|-------|-------|----------|----------|
...
## Ενσωμάτωση CVML στη Δική σας Εφαρμογή

Για να χρησιμοποιήσετε τη CVML Library στο δικό σας έργο C++, προσθέστε την μέσω του `find_package` του CMake:
...
Όπου το `AMD_CVML_SDK_ROOT` δείχνει στη ρίζα του φακέλου Ryzen AI CVML Library. Στη συνέχεια συμπεριλάβετε την κατάλληλη κεφαλίδα για τη λειτουργία που θέλετε:
...
Για κάθε δείγμα παρακάτω, μεταβείτε πρώτα στον φάκελο του εκτελέσιμου, ακολουθώντας το ίδιο μοτίβο με την ενότητα [Εκτέλεση Ανίχνευσης Προσώπου](#running-face-detection) παραπάνω (π.χ. `cd build\cvml-sample-depth-estimation\Release` στα Windows ή `cd build/cvml-sample-depth-estimation` στο Linux). Στα Windows, προσθέστε `.exe` σε κάθε εντολή (π.χ. `cvml-sample-depth-estimation.exe`).

- **Δοκιμάστε Εκτίμηση Βάθους**: Εκτελέστε `cvml-sample-depth-estimation -i sample_face.jpg` για να δημιουργήσετε έναν χρωματισμένο χάρτη βάθους — τα κοντινά αντικείμενα εμφανίζονται σε θερμά χρώματα, τα μακρινά σε ψυχρά χρώματα
- **Εξερευνήστε το Πλέγμα Προσώπου**: Εκτελέστε `cvml-sample-face-mesh -i sample_face.jpg` για να δείτε λεπτομερή παρακολούθηση γεωμετρίας προσώπου με πυκνά σημεία πλέγματος
- **Επεξεργαστείτε αρχεία βίντεο**: Χρησιμοποιήστε τις σημαίες `-i` και `-o` σε οποιοδήποτε δείγμα για επεξεργασία βίντεο (π.χ., `cvml-sample-face-detection -i video.mp4 -o output.mp4`)
- **Συγκρίνετε παραλλαγές μοντέλου**: Δοκιμάστε `-m precise` έναντι της προεπιλογής `-m fast` στην ανίχνευση προσώπου για να δείτε από κοντά τον συμβιβασμό ακρίβειας/ταχύτητας
...
- **Συνδυάστε λειτουργίες**: Συνδέστε αλυσιδωτά την ανίχνευση προσώπου με την εκτίμηση βάθους στην ίδια εφαρμογή για πλουσιότερη κατανόηση σκηνής
- **Περιηγηθείτε στον πηγαίο κώδικα**: Διαβάστε τη [Ryzen AI CVML Library στο GitHub](https://github.com/amd/RyzenAI-SW/tree/main/Ryzen-AI-CVML-Library) για τεκμηρίωση κεφαλίδων, πρόσθετα δείγματα και λεπτομέρειες API