...
Οι πράκτορες GAIA είναι AI βοηθοί που χρησιμοποιούν ένα τοπικό LLM για να συλλογίζονται και να καλούν εργαλεία που ορίζετε εσείς — όπως chatbots που μπορούν να αναλαμβάνουν δράση. Εκτελούνται **100% τοπικά** χωρίς cloud APIs, χωρίς δεδομένα που εγκαταλείπουν τον υπολογιστή σας και χωρίς να απαιτούνται κλειδιά API.

Σε αυτό το playbook, θα δημιουργήσετε έναν Hardware Advisor Agent που εντοπίζει τη RAM, το GPU και το NPU του συστήματός σας, υποβάλλει ερωτήματα στον τοπικό κατάλογο μοντέλων και προτείνει ποια LLMs μπορεί να εκτελέσει ο υπολογιστής σας. Είναι μια πρακτική εισαγωγή στο GAIA Agent SDK που παράγει κάτι άμεσα χρήσιμο.

## Τι θα Μάθετε

- Πώς να δημιουργήσετε έναν πράκτορα GAIA με προσαρμοσμένα εργαλεία
- Χρήση του LemonadeClient SDK για υποβολή ερωτημάτων σχετικά με πληροφορίες συστήματος και καταλόγους μοντέλων
- Εντοπισμός GPU/NPU ανά πλατφόρμα (Windows PowerShell και Linux lspci)
- Διαστασιολόγηση μοντέλων βάσει μνήμης χρησιμοποιώντας τον κανόνα του 70%
...
Εκτελέστε πρώτα τον έτοιμο πράκτορα για να δείτε τι δημιουργείτε. Στη συνέχεια, θα αναλύσουμε τον κώδικα βήμα προς βήμα.

### Εκτέλεση του Προκατασκευασμένου Παραδείγματος

Αυτό το playbook περιλαμβάνει τον πλήρη κώδικα [hardware_advisor_agent.py](assets/hardware_advisor_agent.py). Κατεβάστε τον σε έναν κατάλογο της επιλογής σας και εκτελέστε τον για να δείτε τον έτοιμο πράκτορα σε δράση:
...
Το υπόλοιπο του playbook θα εξηγεί πώς λειτουργεί κάθε μέρος του σεναρίου, ώστε να το κατανοήσετε από την αρχή.
<!-- @os:windows -->
...
- **LemonadeClient SDK** — APIs πληροφοριών συστήματος και καταλόγου μοντέλων
- **Εντοπισμός ανά πλατφόρμα** — Windows PowerShell / Linux lspci για πληροφορίες GPU
- **Υπολογισμοί μνήμης** — Κανόνας 70% για ασφαλή διαστασιολόγηση μοντέλων

Τα δεδομένα ρέουν μέσα από αυτά με τη σειρά: ερώτημα χρήστη → ο πράκτορας επιλέγει ένα εργαλείο → το εργαλείο καλεί LemonadeClient + εντοπισμό λειτουργικού συστήματος → ο πράκτορας συνθέτει τα αποτελέσματα σε μια σύσταση.

### LemonadeClient SDK

Το LemonadeClient παρέχει ένα ενοποιημένο API για εντοπισμό συστήματος, διαθεσιμότητα NPU/GPU και υποβολή ερωτημάτων στον κατάλογο μοντέλων.
...
**`get_system_info()`** — Επιστρέφει λειτουργικό σύστημα, CPU, RAM και διαθεσιμότητα συσκευών:
...
### Εντοπισμός GPU Ανά Πλατφόρμα

Ο πράκτορας χρησιμοποιεί εντολές του λειτουργικού συστήματος αντί για PyTorch για τον εντοπισμό GPU. Αυτό λειτουργεί χωρίς εγκατεστημένα προγράμματα οδήγησης GPU, εντοπίζει όλα τα GPU (όχι μόνο αυτά με δυνατότητα CUDA) και αποφεύγει βαριές εισαγωγές βιβλιοθηκών.

<!-- @os:windows -->

Στα Windows, ο πράκτορας χρησιμοποιεί PowerShell για υποβολή ερωτημάτων στο WMI:
...
Στο Linux, ο πράκτορας χρησιμοποιεί lspci:
...
> **Κανόνας:** Το μέγεθος του μοντέλου πρέπει να είναι μικρότερο από το 70% της διαθέσιμης RAM, ώστε να απομένει 30% περιθώριο για λειτουργίες συμπερασμού (KV cache, buffers επεξεργασίας δεσμών, αιχμές μνήμης κατά την εκτέλεση).
...
Ξεκινήστε με μια ελάχιστη δομή πράκτορα — μόνο την κλάση και ένα βασικό system prompt. Ο πράκτορας δεν έχει ακόμα εργαλεία.
...
### Βήμα 2: Εντοπισμός GPU και Υλικού

Προσθέστε τη βοηθητική μέθοδο `_get_gpu_info()` και το εργαλείο `get_hardware_info()`. Αυτό καθιστά τον πράκτορα διαδραστικό — μπορείτε πλέον να τον ρωτάτε για τις προδιαγραφές του συστήματος.
...
**Προσθέστε τη βοηθητική συνάρτηση `_get_gpu_info()`** μετά τη μέθοδο `_get_system_prompt()`:
...
**Ενημερώστε το μπλοκ `__main__`** για να ενεργοποιήσετε τη διαδραστική δοκιμή:
...
Προσθέστε το εργαλείο `list_available_models()` μέσα στη `_register_tools()`, μετά τη συνάρτηση `get_hardware_info`. Τώρα ο πράκτορας μπορεί να σας πει ποια μοντέλα είναι διαθέσιμα.
...
Προσθέστε το εργαλείο `recommend_models()` μέσα στη `_register_tools()`, μετά το `list_available_models`. Ο πράκτορας μπορεί πλέον να υπολογίζει ποια μοντέλα χωράνε στη μνήμη του συστήματός σας χρησιμοποιώντας τον κανόνα του 70%.
...
Αντικαταστήστε το απλό μπλοκ `__main__` με ένα εκλεπτυσμένο διαδραστικό CLI. Αυτό προσθέτει ένα banner, εντολές εξόδου και καλύτερο χειρισμό σφαλμάτων.
...
Το αρχείο `hardware_advisor_agent.py` σας θα πρέπει τώρα να έχει όλα αυτά τα στοιχεία:
...
- [x] Εργαλείο `get_hardware_info()` με πεδία GPU, NPU και λειτουργικού συστήματος
- [x] Εργαλείο `list_available_models()` με ετικέτες και εμπλουτισμό μεγέθους
- [x] Εργαλείο `recommend_models()` με κανόνα 70%, fits_in_ram, fits_in_gpu
...
- **Εξερευνήστε τα APIs του LemonadeClient** — Ανακαλύψτε περισσότερες δυνατότητες διαχείρισης συστήματος και μοντέλων στην [τεκμηρίωση του LemonadeClient SDK](https://amd-gaia.ai/sdk/lemonade-client)
- **Προσθέστε φωνητική αλληλεπίδραση** — Ενσωματώστε Whisper ASR και Kokoro TTS για να επιτρέψετε στους χρήστες να κάνουν ερωτήσεις υλικού μιλώντας. Δείτε τον [οδηγό Talk](https://amd-gaia.ai/guides/talk)
- **Προσθέστε υποστήριξη MCP** — Εκθέστε τον hardware advisor ως διακομιστή MCP ώστε άλλα εργαλεία να μπορούν να τον υποβάλλουν ερωτήματα. Δείτε τον [οδηγό MCP](https://amd-gaia.ai/sdk/infrastructure/mcp)
- **Επεκτείνετε τη μηχανή συστάσεων** — Λάβετε υπόψη το VRAM του GPU για offloading επιπέδων, ή προσθέστε benchmarking για εκτίμηση tokens ανά δευτερόλεπτο
- **Δημιουργήστε ένα σύστημα πολλαπλών πρακτόρων** — Συνδυάστε τον hardware advisor με έναν πράκτορα κώδικα ή συνομιλίας χρησιμοποιώντας τον [Routing Agent](https://amd-gaia.ai/guides/routing)