...
Το Ollama είναι ένα δημοφιλές ελαφρύ εργαλείο για την τοπική εκτέλεση μεγάλων γλωσσικών μοντέλων. Διαχειρίζεται τη λήψη μοντέλων, την κβαντοποίηση και την εξυπηρέτηση μέσω μιας απλής διεπαφής γραμμής εντολών και εφαρμογής επιφάνειας εργασίας, ώστε να μπορείτε να ξεκινήσετε συνομιλία με ένα LLM σε λίγα λεπτά.

Αυτό το playbook σας καθοδηγεί στην εγκατάσταση του Ollama, στη λήψη του μοντέλου GPT-OSS 20B και στη διεξαγωγή συνομιλίας με αυτό, τόσο μέσω τερματικού όσο και μέσω της εφαρμογής επιφάνειας εργασίας.
...
- Ερώτηση μοντέλων μέσω προγραμματισμού μέσω του REST API
...
1. Κατεβάστε το πρόγραμμα εγκατάστασης από το [ollama.com/download](https://ollama.com/download).
2. Εκτελέστε το πρόγραμμα εγκατάστασης `.exe` και ακολουθήστε τις οδηγίες.
3. Μόλις εγκατασταθεί, το Ollama εκτελείται ως υπηρεσία παρασκηνίου και είναι προσβάσιμο από το τερματικό, την εφαρμογή επιφάνειας εργασίας και την περιοχή ειδοποιήσεων.
...
Το Ollama διαχειρίζεται μοντέλα μέσω ενός μητρώου παρόμοιου με εικόνες container. Για να κατεβάσετε το GPT-OSS 20B:
...
Τα ονόματα μοντέλων Ollama ακολουθούν τη μορφή `name:tag`. Η ετικέτα συνήθως υποδεικνύει τον αριθμό παραμέτρων ή την παραλλαγή κβαντοποίησης. Μερικές χρήσιμες εντολές για τη διαχείριση μοντέλων:

| Εντολή | Περιγραφή |
|---------|-------------|
| `ollama list` | Εμφάνιση όλων των ληφθέντων μοντέλων |
| `ollama pull <model>` | Λήψη ενός μοντέλου χωρίς εκτέλεση |
| `ollama rm <model>` | Αφαίρεση ενός μοντέλου για ελευθέρωση χώρου στον δίσκο |
| `ollama show <model>` | Εμφάνιση μεταδεδομένων και παραμέτρων μοντέλου |

## Συνομιλία από το Τερματικό

Εκκινήστε μια διαδραστική συνεδρία συνομιλίας απευθείας από τη γραμμή εντολών:

```bash
ollama run gpt-oss:20b
```

Το Ollama φορτώνει το μοντέλο στη μνήμη και σας εμφανίζει μια προτροπή. Δοκιμάστε να το ρωτήσετε κάτι:

```
>>> What is the capital of France and why is it historically significant?
```

Το μοντέλο μεταδίδει την απάντησή του token-by-token απευθείας στο τερματικό. Πληκτρολογήστε `/bye` ή πατήστε `Ctrl+D` για να τερματίσετε τη συνεδρία.

> **Συμβουλή**: Η πρώτη εκτέλεση χρειάζεται μερικά δευτερόλεπτα για να φορτωθεί το μοντέλο στη μνήμη. Οι επόμενες προτροπές εντός της ίδιας συνεδρίας ανταποκρίνονται πολύ πιο γρήγορα καθώς το μοντέλο παραμένει φορτωμένο.

<!-- @os:windows -->
## Συνομιλία από την Εφαρμογή Επιφάνειας Εργασίας

Το Ollama διαθέτει επίσης μια εφαρμογή επιφάνειας εργασίας που παρέχει μια καθαρή διεπαφή συνομιλίας για αλληλεπίδραση με τα μοντέλα σας.

Ανοίξτε το **Ollama** από το μενού Έναρξης ή κάντε κλικ στο εικονίδιο Ollama στην περιοχή ειδοποιήσεων και επιλέξτε **Open Ollama**.

Μόλις ανοίξει η εφαρμογή:

1. Κάντε κλικ στο **New Chat** στην πλαϊνή γραμμή.
2. Επιλέξτε **gpt-oss:20b** από το αναπτυσσόμενο μενού μοντέλου στην κάτω δεξιά γωνία της περιοχής εισαγωγής συνομιλίας.
...
Η εφαρμογή επιφάνειας εργασίας διατηρεί ιστορικό των συνομιλιών σας στην πλαϊνή γραμμή, διευκολύνοντας την επανεξέταση προηγούμενων συνομιλιών.
<!-- @os:end -->

## Χρήση του REST API

Μετά την εγκατάσταση, το Ollama εκτελείται ως υπηρεσία παρασκηνίου και εκθέτει ένα REST API στο `http://localhost:11434` που μπορείτε να χρησιμοποιήσετε για να ενσωματώσετε μοντέλα στις δικές σας εφαρμογές και scripts.
...
### Παράδειγμα Python
Τώρα που μπορούμε να προσεγγίσουμε το Ollama API μέσω προγραμματισμού, ας το καλέσουμε από Python.
...
Στον ίδιο κατάλογο, χρησιμοποιήστε το VS Code ή άλλο πρόγραμμα επεξεργασίας για να δημιουργήσετε ένα αρχείο .py και αντιγράψτε τον παρακάτω κώδικα σε αυτό. Στη συνέχεια, εκτελέστε το αρχείο στο ενεργοποιημένο περιβάλλον σας με `python your_file_name.py`
...
|----------|--------|---------|
| `/api/generate` | POST | Δημιουργία κειμένου μονής στροφής |
| `/api/chat` | POST | Συνομιλία πολλαπλών στροφών με ιστορικό μηνυμάτων |
| `/api/tags` | GET | Λίστα διαθέσιμων μοντέλων |
| `/api/show` | POST | Εμφάνιση λεπτομερειών μοντέλου |
| `/api/pull` | POST | Λήψη μοντέλου από το μητρώο |

Για την πλήρη αναφορά API, δείτε την [τεκμηρίωση Ollama API](https://github.com/ollama/ollama/blob/main/docs/api.md).

## Επόμενα Βήματα

- **Δοκιμάστε διαφορετικά μοντέλα**: Περιηγηθείτε στη [βιβλιοθήκη μοντέλων Ollama](https://ollama.com/library) για να εξερευνήσετε εκατοντάδες διαθέσιμα μοντέλα, από μικρούς βοηθούς κώδικα έως μεγάλα μοντέλα συλλογισμού.
- **Δημιουργήστε προσαρμοσμένα μοντέλα**: Χρησιμοποιήστε ένα [Modelfile](https://github.com/ollama/ollama/blob/main/docs/modelfile.md) για να ορίσετε προσαρμοσμένες προτροπές συστήματος, θερμοκρασία και άλλες παραμέτρους για μια εξατομικευμένη εμπειρία.
- **Δημιουργήστε με το API**: Χρησιμοποιήστε τις βιβλιοθήκες πελάτη [Python](https://github.com/ollama/ollama-python) ή [JavaScript](https://github.com/ollama/ollama-js) για να ενσωματώσετε το Ollama στις εφαρμογές σας.
- **Σύνδεση με διεπαφές**: Συνδυάστε το Ollama με εργαλεία όπως το [Open WebUI](https://github.com/open-webui/open-webui) για μια πλούσια σε χαρακτηριστικά διεπαφή συνομιλίας με αναζήτηση, personas και μεταφόρτωση εγγράφων.

Για περισσότερες πληροφορίες, δείτε την [τεκμηρίωση Ollama](https://github.com/ollama/ollama/blob/main/README.md).