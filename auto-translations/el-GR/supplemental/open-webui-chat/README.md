...
Το [Open WebUI](https://docs.openwebui.com) είναι μια αυτοφιλοξενούμενη διεπαφή βασισμένη σε πρόγραμμα περιήγησης, που προσφέρει μια οικεία εμπειρία chatbot ενώ λειτουργεί ως frontend για έναν ή περισσότερους διακομιστές μοντέλων AI. Αντί να είναι δεμένο σε έναν πάροχο, το Open WebUI μπορεί να συνδεθεί σε **οποιοδήποτε backend που εκθέτει ένα OpenAI-compatible API**, ώστε να μπορείτε να εναλλάσσετε μοντέλα και δυνατότητες χωρίς να αλλάζετε UI.
...
Αυτή η ρύθμιση σας επιτρέπει να εξερευνήσετε την **πλήρη πολυτροπική ροή εργασίας από άκρη σε άκρη**.
...
| Backend (Model Server) | Φιλοξενεί μοντέλα και εκθέτει HTTP endpoints | Lemonade, Ollama, vLLM, llama.cpp server, OpenAI-compatible servers |
...
| **Open WebUI** | `http://localhost:8080` | Συνομιλία, ανέβασμα εικόνων, παραγωγή εικόνων — η διεπαφή που βλέπει ο χρήστης |
...
Αυτό το playbook χρειάζεται το Lemonade να εκτελείται ως backend και, σε Linux, μια μηχανή containers (Podman) για την εκτέλεση του Open WebUI. Ρυθμίστε αυτά πριν εγκαταστήσετε το Open WebUI.
...
3. Επιβεβαιώστε ότι το API είναι προσβάσιμο επισκεπτόμενοι το `http://localhost:13305/api/v1/models` στο πρόγραμμα περιήγησής σας — θα πρέπει να δείτε τα μοντέλα που κατεβάσατε να εμφανίζονται.

> Τα μοντέλα πρέπει να κατεβούν στο **Lemonade** (`localhost:13305`) πριν μπορέσουν να εμφανιστούν στο **Open WebUI** (`localhost:8080`). Εάν ένα μοντέλο δεν εμφανίζεται αργότερα στο Open WebUI, επιστρέψτε εδώ και ελέγξτε πρώτα το Lemonade.
...
Το Open WebUI απαιτεί **Python 3.12** — δεν εγκαθίσταται σε Python 3.13+. Το Windows Python Launcher (`py`) σάς επιτρέπει να εγκαταστήσετε την έκδοση 3.12 παράλληλα με οποιαδήποτε υπάρχουσα έκδοση Python χωρίς συγκρούσεις.
...
<!-- @device:halo_box -->
> **Σημείωση:** Το σύστημά σας διαθέτει προεγκατεστημένη την Python 3.13. Η εγκατάσταση της 3.12 δεν την επηρεάζει — το `python` συνεχίζει να χρησιμοποιεί την 3.13, ενώ το `py -3.12` στοχεύει την 3.12 μόνο όταν το χρειάζεστε.
<!-- @device:end -->
...
<!-- @os:linux -->
Θα χρησιμοποιήσουμε τώρα την υπηρεσία Podman για να κάνουμε containerize την εγκατάσταση του Open WebUI.
...
Αυτό κατεβάζει το image του Open WebUI και γράφει σε μόνιμο αποθηκευτικό χώρο.
...
- Το Open WebUI θα σας ζητήσει να δημιουργήσετε έναν τοπικό λογαριασμό διαχειριστή. Μόλις συνδεθείτε, θα δείτε το περιβάλλον συνομιλίας.
...
<!-- @os:linux -->
> Το container εκτελείται στο παρασκήνιο. Από τον κατάλογο που περιέχει το `compose.yml`, διαχειριστείτε το με `podman compose down` (διακοπή) και `podman compose up -d` (εκκίνηση). Οι λογαριασμοί και οι ρυθμίσεις σας παραμένουν αποθηκευμένα στον τόμο `open_webui_data`.
<!-- @os:end -->
...
Τώρα που και οι δύο υπηρεσίες εκτελούνται — το Lemonade στη διεύθυνση `localhost:13305` και το Open WebUI στη διεύθυνση `localhost:8080` — συνδέστε τις ώστε το Open WebUI να μπορεί να χρησιμοποιεί τα μοντέλα του Lemonade.
...
2. Στο πλαίσιο Settings, κάντε κλικ στο **Admin Settings** στην κάτω αριστερή γωνία.

   <p align="center">
     <img src="assets/click_admin_settings.png" alt="Select Admin Settings" width="450"/>
   </p>

3. Στην πλαϊνή μπάρα των Admin Settings, κάντε κλικ στο **Connections** (ή μεταβείτε απευθείας στο `http://localhost:8080/admin/settings/connections`).
...
4. Κάτω από το **OpenAI API**, προσθέστε μια νέα σύνδεση:
   ...
   - **API Key:** `-` (μία μόνο παύλα αρκεί για τοπική χρήση)

   <p align="center">
     <img src="assets/connection_form.png" alt="Connection details for Lemonade server" width="400"/>
   </p>

5. Βεβαιωθείτε ότι κάτω από το **"Manage OpenAI API Connections"**, είναι ενεργοποιημένη μόνο η σύνδεση `http://localhost:13305/api/v1`. Απενεργοποιήστε οποιαδήποτε άλλη σύνδεση (π.χ. την προεπιλεγμένη σύνδεση OpenAI).
...
<!-- @os:windows -->
<!-- @device:halo,stx,krk -->
1. Κάντε κλικ στο αναπτυσσόμενο μενού στην επάνω αριστερή γωνία του περιβάλλοντος εργασίας. Αυτό θα εμφανίσει τα μοντέλα Lemonade που έχετε εγκαταστήσει. Επιλέξτε ένα για να συνεχίσετε (παράδειγμα: `Qwen3-4B-Hybrid`).

    <p align="center">
      <img src="assets/model_selection.png" alt="Model Selection" width="600"/>
    </p>

2. Πληκτρολογήστε ένα μήνυμα προς το LLM και κάντε κλικ στην αποστολή (ή πατήστε Enter). Το LLM θα χρειαστεί μερικά δευτερόλεπτα για να φορτωθεί στη μνήμη και έπειτα θα δείτε την απάντηση να εμφανίζεται σταδιακά.
...
<!-- @device:rx7900xt,rx9070xt,r9700 -->
1. Κάντε κλικ στο αναπτυσσόμενο μενού στην επάνω αριστερή γωνία του περιβάλλοντος εργασίας. Αυτό θα εμφανίσει τα μοντέλα Lemonade που έχετε εγκαταστήσει. Επιλέξτε ένα για να συνεχίσετε (παράδειγμα: `Qwen3.5-4B-GGUF`).
...
4. Αυτή τη στιγμή, ανοίξτε τη `Task Manager` στο σύστημά σας. Θα δείτε **υψηλή χρήση GPU ή NPU** ανάλογα με το αν το μοντέλο που επιλέξατε είναι **Hybrid** ή **NPU** αντίστοιχα. Χρησιμοποιώντας τη διαχείριση εργασιών, μπορείτε να επιβεβαιώσετε ότι εκτελείτε το μοντέλο τοπικά.
...
Αυτό επιβεβαιώνει ότι το Open WebUI μπορεί να στέλνει αιτήματα στο Lemonade χρησιμοποιώντας το endpoint συνομιλίας συμβατό με OpenAI.
...
4. Εάν θέλετε να προσθέσετε περισσότερες παραμέτρους, προσθέστε τις στο πεδίο κειμένου ως JSON. Για παράδειγμα: `{ "steps": 4, "cfg_scale": 1 }`. Δείτε τις διαθέσιμες παραμέτρους στο [Image Generation (Stable Diffusion CPP)](https://lemonade-server.ai/models.html).
...
2. Επιλέξτε ένα **Text Generation LLM** στο αναπτυσσόμενο μενού μοντέλων (παράδειγμα: Qwen, Llama). **Μην επιλέξετε μοντέλο Stable Diffusion**, καθώς αυτό είναι επιλογέας μοντέλου συνομιλίας.
...
- Επιλέξατε ένα μοντέλο εικόνας (SDXL-Turbo / SDXL-Base-1.0) στο αναπτυσσόμενο μενού μοντέλων συνομιλίας.
- **Διόρθωση**: επιλέξτε ένα LLM για συνομιλία και χρησιμοποιήστε το διακόπτη Image + τις ρυθμίσεις Images για τη δημιουργία.
<p align="center">
...
Τώρα έχετε μια λειτουργική **«τοπική στοίβα AI»**, ένα ενιαίο περιβάλλον εργασίας που ελέγχει πολλαπλούς τύπους μοντέλων μέσω ενός τυπικού API.

Ακολουθούν τρεις επεκτάσεις που ξεκλειδώνουν εντελώς νέες ροές εργασίας:

### 1. Μετατροπή Ομιλίας σε Κείμενο με το Whisper

Δοκιμάστε να μετατρέψετε ήχο σε κείμενο χρησιμοποιώντας ένα μοντέλο Whisper και, στη συνέχεια, τροφοδοτήστε το σε ένα LLM για σύνοψη, στοιχεία δράσης ή αναδιατύπωση. Αυτό αποτελεί τη βάση για σημειώσεις συναντήσεων και βοηθούς με φωνητική καθοδήγηση.

### 2. Προγραμματισμός σε Python μέσα στο Open WebUI

Χρησιμοποιήστε την ενσωματωμένη εμπειρία εκτέλεσης κώδικα του Open WebUI για να εκτελέσετε αποσπάσματα Python, να επιθεωρήσετε τα αποτελέσματα και να επαναλάβετε γρηγορότερα—χωρίς να φύγετε από το περιβάλλον εργασίας. [Αναφορά](https://lemonade-server.ai/docs/server/apps/open-webui/#python-coding)
...
- [Βίντεο περιήγηση (Lemonade)](https://www.youtube.com/watch?v=mcf7dDybUco)
- [Βίντεο περιήγηση (Open WebUI + Lemonade)](https://www.youtube.com/watch?v=yZs-Yzl736E)