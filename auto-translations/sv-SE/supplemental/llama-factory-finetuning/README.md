...
Effektiv finjustering är avgörande för att anpassa stora språkmodeller (LLMs) till nedströmsuppgifter. LLaMA-Factory är en öppen och användarvänlig plattform som förenklar träning och finjustering av stora språkmodeller och multimodala modeller. Den låter användare anpassa hundratals förtränade modeller lokalt med minimal kodning.

Den här spelböckens lär dig hur du finjusterar LLMs med LLaMA-Factory på din lokala AMD-hårdvara.

<!-- @device:stx,krk -->
> **Obs:** Finjusteringsteknikerna i den här spelboken kräver minst **32 GB systemminne (RAM)**, varav minst **16 GB måste vara tillgängliga för GPU:n** (de 16 GB ingår i de 32 GB, inte utöver dem).
<!-- @device:end -->


<!-- @device:rx7900xt,rx9070xt,r9700 -->
<!-- @os:windows -->
> **Obs:** Finjusteringsteknikerna i den här spelboken kräver minst **16 GB totalt GPU-minne** och **32 GB systemminne (RAM)**.
> - På Windows kombinerar det totala GPU-minnet grafikkortets dedikerade VRAM med delat GPU-minne (lånat från systemminnet).
> - Därför kan kort med mindre än 16 GB dedikerat VRAM ändå köra den här spelboken genom att använda delat GPU-minne för att kompensera skillnaden.
<!-- @os:end -->

<!-- @os:linux -->
> **Obs:** Finjusteringsteknikerna i den här spelboken kräver ett grafikkort med minst **16 GB dedikerat GPU-minne** och **32 GB systemminne (RAM)**.
...
- Varaktighet: Det tar ungefär 60 minuter att köra den här spelboken (beroende på din modell-/datasetstorlek och nätverkshastighet).
...
> **Obs**: Kontrollera att Python-versionen är 3.11, 3.12 eller 3.13
...
Ladda ned källkoden från [LLaMA Factory officiella GitHub-arkiv](https://github.com/hiyouga/LlamaFactory) och installera dess beroenden.
...
Kontrollera om `llamafactory-cli` är körbart.
...
Nu när LLaMA-Factory har installerats framgångsrikt, låt oss köra finjustering på det.

## Använda LLaMA-Factory CLI för finjustering

Det här avsnittet beskriver hur du förbereder finjusteringsdataset, konfigurerar LoRA/QLoRA-parametrar och kör LoRA-finjustering.
...
I den här spelboken använder vi dataseten identity och alpaca_en_demo som exempel och konfigurerar datasetinformationen i nästa steg.
...
|-----------|------|
| Fullständiga parametrar    | [examples/train_full](https://github.com/hiyouga/LlamaFactory/tree/main/examples/train_full) |
...
Dessa exempelkonfigurationsfiler har specificerade modellparametrar, finjusteringsmetodparametrar, datasetparametrar, utvärderingsparametrar och mer. Du kan konfigurera dem efter dina egna behov. I den här spelboken använder vi [qwen3_lora_sft.yaml](https://github.com/hiyouga/LlamaFactory/blob/main/examples/train_lora/qwen3_lora_sft.yaml).

**Förklaring av nyckelparametrar:**
- `model_name_or_path` – Hugging Face-modellnamn eller lokal modellfilsökväg.
...
- `lora_rank` – Dimensionaliteten hos den lågrangsmatris som används i LoRA, typiska värden: 4, 6, 8, 16 (lägre värden = färre parametrar = snabbare finjustering; högre värden = bättre uppgiftsanpassning men högre resursanvändning).
- `lora_target` – Målmoduler för LoRA-metoden. Standard: all.
- `dataset` – Dataset att använda. Använd "," för att separera flera dataset
- `output_dir` – Utdatasökväg för finjustering
- `logging_steps` – Loggningsintervall i steg
- `save_steps` – Intervall för att spara modellkontrollpunkter.
- `overwrite_output_dir` – Om överskrivning av utdatakatalogen ska tillåtas.
...
- `lr_scheduler_type` – Schema för inlärningshastighet. Alternativ: linear, cosine, polynomial, constant, osv.
...
- Använd `fp16` istället för `bf16` för bredare AMD GPU-kompatibilitet och lägre minnesanvändning.
- Ange `dataloader_num_workers` till `0` på Windows för att undvika `"Can't pickle local object<>"`-fel orsakade av flerprocessig datainläsning.
...
**llamafactory-cli** är det officiella kommandoradsgränssnittet (CLI) för LLaMA-Factory, utvecklat för att förenkla kompletta LLM-arbetsflöden (dataförberedelse → finjustering → utvärdering → driftsättning) utan att skriva komplex kod.

För träning/finjustering är **llamafactory-cli train** det centrala underkommandot i LLaMA-Factory CLI. Det abstraherar finjusteringsarbetsflöden (dataförbehandling, hyperparameterjustering, hårdvaruoptimering) till ett enda CLI-kommando, stöder flera finjusteringsparadigm (LoRA/QLoRA/fullständig finjustering) och är optimerat för GPU:er med begränsade resurser (t.ex. QLoRA på 16 GB VRAM).
...
Efter att ha kört LLM-finjustering lagras alla genererade utdata i "output_dir", inklusive modellkontrollpunktsfiler, konfigurationsfiler och träningsstatistik.
...
**llamafactory-cli chat** är utformat för interaktiv chatt/inferens med LLMs (både basmodeller och LoRA-finjusterade modeller). LLaMA-Factory tillhandahåller exempelkonfigurationen för att köra inferens av finjusterade modeller i [examples/inference](https://github.com/hiyouga/LlamaFactory/tree/main/examples/inference). Du kan också ändra den här exempelkonfigurationen för att ändra inställningarna, till exempel inferensbackend.
...
```
Ett exempelchatt med den finjusterade modellen visas nedan:
...
`LLaMA-Factory` stöder också kodfri finjustering av LLMs via ett webbgränssnitt i webbläsaren.
...
```
`LlamaFactory Web UI` erbjuder ett strömlinjeformat gränssnitt för att hantera maskininlärningsarbetsflöden, inklusive träning, utvärdering, prediktion, chatt och export av modeller. Här är en kort introduktion till varje flik:

* **Train**: Den här fliken låter dig välja en modell och ett dataset, konfigurera träningsparametrar och starta träningsprocessen. Det är viktigt att förstå de obligatoriska och valfria parametrarna för att optimera träningskonfigurationen.
* **Evaluate & Predict**: Efter träning kan du utvärdera modellens prestanda och göra prediktioner med den här fliken. Den ger insikter om modellens noggrannhet och effektivitet på nya data.
* **Chat**: När träningen är klar, ladda modellen i Chat-fliken för att interagera med den och se resultaten av ditt arbete. Den här funktionen möjliggör realtidskommunikation med den tränade modellen.
