...
🍋 **Lemonade** est un serveur d'IA locale open source qui vous permet d'exécuter des grands modèles de langage (LLM), des générateurs d'images et des modèles audio directement sur votre propre matériel. Il expose les modèles via l'**API OpenAI**, une norme reconnue dans l'industrie, afin que toute application compatible avec OpenAI puisse instantanément fonctionner avec Lemonade. À la fin de ce playbook, vous utiliserez Lemonade pour exécuter des modèles localement sur votre machine.
...
* **Télécharger et discuter avec un LLM** à l'aide d'une seule commande.
* **Explorer l'interface web** et essayer différentes modalités telles que la vision, la reconnaissance vocale (speech-to-text) et la génération d'images.
* **Basculer entre les backends GPU** Vulkan et AMD ROCm™.
* **Créer une application Python** alimentée par un LLM local à l'aide de l'API compatible OpenAI.
<!-- @device:halo_box,halo,stx,krk -->
* **Exécuter des modèles sur le processeur neuronal AMD (NPU)** à l'aide des modes d'exécution Hybrid et FLM sur du matériel AMD Ryzen™ AI.
<!-- @device:end -->
...
<!-- @device:halo_box,halo,stx,krk -->
- [Facultatif] Un NPU AMD XDNA 2 (séries Ryzen AI 300/400/Max 300 ou Z2 Extreme) avec le pilote le plus récent installé à partir des [instructions d'installation du logiciel Ryzen AI](https://ryzenai.docs.amd.com/en/latest/inst.html#install-npu-drivers) si vous souhaitez exécuter un modèle sur le NPU.
<!-- @device:end -->
...
## Concepts fondamentaux — Fonctionnement des serveurs d'IA locale

Avant d'exécuter un modèle, il est utile de comprendre *pourquoi* les choses sont configurées ainsi. Lemonade est un **serveur de modèles local**, un processus qui charge des modèles d'IA en mémoire et les expose aux applications via HTTP, tout comme le ferait un service d'IA cloud.
...
| **Modèles partagés** | Un seul modèle chargé peut servir plusieurs applications à la fois, sans copies redondantes qui consomment votre RAM. |
...
Lemonade implémente l'**API OpenAI**, la même interface utilisée par ChatGPT, Azure OpenAI et de nombreux autres services. Le modèle de conversation est simple :

| Rôle | Qui parle |
|------|---------------|
| **system** | Instructions destinées au modèle (persona, contraintes, outils disponibles) |
| **user** | Messages de l'humain (ou de l'application) adressés au modèle |
...
<!-- @os:windows -->
Sous Windows, Lemonade App se lance automatiquement et vous pouvez commencer à discuter immédiatement. Si vous avez installé le package `minimal.msi`, l'application n'est pas incluse. Pour commencer à discuter, ouvrez votre navigateur web et accédez à `http://localhost:13305`.
<!-- @os:end -->
...
Le modèle répondra directement dans la fenêtre de discussion. **Félicitations ! Vous exécutez désormais un grand modèle de langage en local.**

![Lemonade App avec journaux affichés](../../dependencies/assets/ChatwithLogs.png)

Dans le panneau des journaux du serveur (Server Logs) de Lemonade App, vous trouverez des données de télémétrie sur les performances du modèle après chaque réponse. Par exemple :
...
Lemonade comprend une interface Web intégrée qui vous permet de :
...
Essayez de passer d'une modalité à l'autre à l'aide de l'onglet **Model Manager** de l'interface Web, où vous pouvez parcourir les modèles par Recette ou par Catégorie :
...
2. **Génération d'images :** Dans la catégorie Image, téléchargez un modèle d'image tel que `SDXL-Turbo` depuis le Model Manager, puis utilisez le Lemonade Image Generator pour saisir une invite et générer une image localement.
3. **Audio :** Dans la catégorie Audio, téléchargez un modèle audio tel que `Whisper-Tiny`, qui permet de convertir la parole en texte. Fournissez un enregistrement audio pour le transcrire localement. Pour la synthèse vocale, essayez l'un des modèles de la catégorie Speech, tel que `kokoro-v1`.

![Multi-modalité avec Lemonade](../../dependencies/assets/multi_modality.png)

### Étape 3 : Essayer un modèle avec un backend différent

Si vous survolez un modèle dans l'application Lemonade, vous verrez une icône en forme d'engrenage. En cliquant dessus, vous pouvez sélectionner des options pour le modèle, notamment choisir le backend souhaité.
...
Pour gérer vos backends installés, cliquez sur le bouton backend situé dans la colonne la plus à gauche.
...
La véritable puissance d'un serveur d'IA local réside dans le fait que n'importe quelle application peut s'y connecter avec seulement quelques lignes de code. Pour le prouver, créons un **générateur de fiches de révision** petit mais fonctionnel, où vous indiquez un sujet, il génère des fiches, et vous pouvez vous tester de manière interactive.
...
Vous devriez voir un message tel que : `Server is running on port 13305`.

Si le serveur n'est pas en cours d'exécution, démarrez-le en ouvrant l'application Lemonade. Utilisez le port par défaut **13305** (vous pouvez le confirmer ou le sélectionner depuis l'icône de la barre système).
...
> **Astuce** : Nous avons suivi des pratiques d'ingénierie standard grâce à une création d'invite minutieuse et à l'utilisation d'un système à deux modèles pour optimiser les ressources et la rapidité.
...
En environ 150 lignes de code, vous avez créé un outil d'étude entièrement fonctionnel, propulsé par un LLM local. Il n'y a pas de clé API à gérer, aucun coût d'utilisation, et aucune donnée ne quitte jamais votre machine.

> **Point clé :** Remarquez que la ligne `client = OpenAI(base_url=...) ` est la *seule* chose qui relie cette application à Lemonade plutôt qu'au cloud d'OpenAI. Le reste du code est identique à ce que vous écririez pour tout service compatible OpenAI. Si vous avez déjà utilisé la bibliothèque Python OpenAI, vous savez déjà comment créer des applications avec Lemonade.
...
|---------|-----------------|
| **Invites système** | Le message `"system"` indique au LLM de produire un résultat JSON structuré |
...
| **Gestion des erreurs** | Le `try/except` gère avec élégance les cas où la sortie du LLM n'est pas un JSON valide |

Ces mêmes modèles s'appliquent à toute application telle que les chatbots, les assistants de code, les générateurs de contenu, les outils d'automatisation.
...
Si vous disposez d'un Ryzen AI 300/400/Max 300 series ou Z2 Extreme, votre appareil dispose d'une **unité de traitement neuronal (NPU)** intégrée, une puce dédiée conçue spécifiquement pour les charges de travail d'IA. Exécuter des modèles sur le NPU est plus économe en énergie que d'utiliser le GPU, ce qui le rend idéal pour les tâches d'IA en arrière-plan, les sessions longues et l'utilisation sur batterie.
...
|------|-------------|--------|----------------|
| **Hybride (NPU + iGPU)** | Le NPU traite l'invite, l'iGPU génère les tokens | OGA (`oga-hybrid`) | Qwen3-4B-Hybrid |
...
> **Que se passe-t-il en coulisses ?** Lorsque vous envoyez un message, le NPU traite l'intégralité de votre invite en parallèle (c'est ce qu'on appelle le « prefill »). Ensuite, l'iGPU prend le relais pour générer la réponse token par token (c'est ce qu'on appelle le « decode »). Cette approche hybride exploite les points forts de chaque puce.
...
* Recommandé : quittez l'application `Lemonade App` et rouvrez-la pour que les modifications soient détectées.
* Recommandé : ouvrez `Backends Manager` et cliquez sur Install `FastFlowNPU` Backend.
<!-- @device:end -->
<!-- @os:end -->

<!-- @device:halo_box,halo,stx,krk -->
Après une installation réussie, vous devriez voir que `flm:npu` s'est terminé dans le **Download Manager** au sein de l'**application de bureau Lemonade**.
<p align="center">
...
Vous disposez d'un serveur d'IA local fonctionnant sur votre propre matériel, voici les prochaines étapes à suivre :

1. **Connectez vos applications préférées** : Lemonade fonctionne immédiatement avec [VS Code Copilot](https://marketplace.visualstudio.com/items?itemName=lemonade-sdk.lemonade-sdk), [Open WebUI](https://lemonade-server.ai/docs/server/apps/open-webui/), [Continue](https://lemonade-server.ai/docs/server/apps/continue/), [n8n](https://n8n.io/integrations/lemonade-model/), et [bien d'autres](https://lemonade-server.ai/marketplace).

2. **Explorez davantage de modèles** : Parcourez l'intégralité de la [bibliothèque de modèles](https://lemonade-server.ai/docs/server/server_models/) pour trouver des modèles optimisés pour le codage, le raisonnement, la vision, et plus encore. Utilisez l'application Lemonade ou `lemonade list` pour voir ce qui est disponible.
