...
[DeepSeek V4 Flash](https://huggingface.co/deepseek-ai/DeepSeek-V4-Flash) est la variante axée sur l'efficacité de la famille DeepSeek V4 — un modèle de type Mixture of Experts avec 284 milliards de paramètres, dont 13 milliards de paramètres actifs. Selon le [rapport technique de DeepSeek](https://huggingface.co/deepseek-ai/DeepSeek-V4-Flash), il obtient un score de 79 % sur SWE-bench Verified et de 91,6 % sur LiveCodeBench.

[ds4 (Dwarf Star 4)](https://github.com/antirez/ds4) est un moteur d'inférence dédié conçu spécifiquement pour cette architecture de modèle. Plutôt qu'un environnement d'exécution polyvalent, ds4 cible directement la famille DeepSeek V4 avec des optimisations de noyaux spécifiques à l'architecture pour le logiciel AMD ROCm™. C'est actuellement l'une des implémentations les plus performantes de DeepSeek V4 Flash sur Strix Halo.

Ce tutoriel montre comment utiliser `ds4-cockpit`, une interface utilisateur en mode terminal, pour configurer ds4, télécharger les poids du modèle et commencer à servir DeepSeek V4 Flash localement sur la plateforme de développement AMD Ryzen™ AI Halo.

## Ce que vous allez apprendre

- Comment installer et lancer l'interface utilisateur en mode terminal `ds4-cockpit`
...
L'auteur de ds4 fournit plusieurs versions quantifiées de DeepSeek V4 Flash au format GGUF. Tous les modèles ci-dessous utilisent une calibration par matrice d'importance (imatrix), qui préserve une précision plus élevée pour les parties du modèle les plus importantes pour les tâches de codage et de raisonnement.
...
| [MTP Speculative Decoding](https://huggingface.co/antirez/deepseek-v4-gguf) | ~3,6 Go | Extension facultative pour le décodage spéculatif afin d'améliorer la vitesse de génération |
...
[ds4-cockpit](https://github.com/kyuz0/strix-halo-ds4-toolbox) est une interface utilisateur en mode terminal légère qui facilite la mise en route de ds4 sur Strix Halo. Elle prend en charge la création de conteneurs toolbox, le téléchargement des poids du modèle et le démarrage des serveurs. Installez-la avec `pipx` :
...
Dans l'onglet **Interactive Toolboxes**, sélectionnez la dernière toolbox disponible/stable (par exemple `ds4-rocm-7.2.4`) et cliquez sur **Create/Update**. Cela récupère l'image du conteneur et crée l'environnement toolbox.
...
Accédez à l'onglet **Model Manager**. Sélectionnez **IQ2_XXS imatrix (~80.8 GB)** dans le menu déroulant et cliquez sur **Download**. Les fichiers du modèle seront enregistrés par défaut dans `~/ds4` (vous pouvez modifier le chemin de stockage).

> **Remarque :** Le modèle IQ2_XXS pèse environ 80 Go, le téléchargement peut donc prendre un certain temps selon votre connexion. Vous pouvez poursuivre une fois celui-ci terminé.
...
> **Cache disque KV (facultatif).** Activer **KV Disk Cache** décharge le cache KV vers le disque (dans **Host Cache Dir**, par défaut `~/.cache/ds4-kv`) afin que les invites système répétées soient restaurées depuis le SSD plutôt que recalculées. Il s'agit d'une optimisation de performance pour les workflows d'agents de codage avec des invites longues et répétées, et elle n'est **pas requise** pour exécuter le serveur.

<p align="center">
  <img src="assets/ds4-cockpit-server-mode.png" alt="Configuring and starting the ds4 server" width="800"/>
</p>

Le serveur démarre et écoute sur le port 8000, exposant un point de terminaison d'API compatible OpenAI à l'adresse `http://localhost:8000/v1`.
