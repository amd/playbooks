...
Ce playbook vous apprend à configurer un résumeur d'actualités financières alimenté par l'IA qui extrait la section économique d'AP News, récupère les titres clés et utilise un LLM local s'exécutant sur votre système pour générer un résumé destiné aux investisseurs.

## Ce que vous apprendrez

- Comment installer et lancer n8n
- Importer et configurer un flux de travail prédéfini
- Se connecter à Lemonade via l'intégration native n8n
- Comprendre les nœuds de flux de travail et le flux de données

## Qu'est-ce que Lemonade ?

[Lemonade](https://lemonade-server.ai) est une plateforme de service LLM local conçue pour le matériel AMD. Elle fournit une API compatible OpenAI qui s'exécute entièrement sur votre machine — vos données ne quittent jamais votre appareil.
...
n8n inclut un **nœud Lemonade natif** (`Lemonade Chat Model`) qui offre une intégration de premier ordre — aucune configuration manuelle n'est nécessaire. Cela rend la connexion de votre LLM local aux flux de travail d'automatisation simple et directe.
...
<!-- @os:windows -->
> **Problème de PATH** : Si `n8n --version` indique que la commande est introuvable, assurez-vous que le répertoire bin global de npm est dans le `PATH` utilisateur. Le chemin d'installation habituel est `C:\Users\<username>\AppData\Roaming\npm`.
...
<!-- @os:windows -->
Ouvrez l'interface graphique de Lemonade en cliquant sur l'icône Lemonade. Faites un clic droit sur l'icône dans la barre d'état système pour ouvrir l'application. Vous pouvez ensuite ajouter des modèles, des backends et charger les modèles préinstallés.
<!-- @os:end -->
...
> **Conseil** : Si vous êtes bloqué hors de votre compte, essayez `n8n user-management:reset`

### Étape 2 : Importer le flux de travail

Nous avons fourni un flux de travail prédéfini que vous pouvez importer directement :

1. Téléchargez le fichier de flux de travail suivant : [financial-news-workflow.json](assets/financial-news-workflow.json)
2. Cliquez sur **Start from Scratch** pour ouvrir l'éditeur de flux de travail. Vous pouvez également cliquer sur le bouton + en haut à gauche, puis sur **Add workflow**.
...
| **Extract News Headlines & Text** | Nœud HTML qui extrait les titres, les sélections de la rédaction, les principales actualités et les nouvelles régionales à l'aide de sélecteurs CSS |
| **Clean Extracted News Data** | Nœud Set qui combine toutes les données extraites dans un seul champ de texte |
...
<!-- @device:halo_box -->
> Ce flux de travail utilise GPT-OSS-120B qui est préinstallé dans Lemonade. Vous pouvez le remplacer par d'autres modèles chargés dans les paramètres du nœud Lemonade Chat Model.
<!-- @device:end -->
...
3. Observez l'exécution de chaque nœud de gauche à droite — ils deviennent verts une fois terminés
...
Cliquez sur le nom du flux de travail en haut et renommez-le si vous le souhaitez. Les flux de travail sont enregistrés automatiquement au fur et à mesure de votre travail.

## Prochaines étapes

- **Automatisation planifiée** : Remplacez le déclencheur manuel par un **Schedule Trigger** pour une exécution quotidienne
- **Envoyer des notifications** : Ajoutez un nœud **Discord**, **Slack** ou **Email** pour recevoir des résumés
...
n8n propose des centaines de modèles de flux de travail prédéfinis. Parcourez la bibliothèque officielle de modèles à l'adresse :
