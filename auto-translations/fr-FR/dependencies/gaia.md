...
Vérifiez que GAIA v0.16.2 ou une version ultérieure est installée :

```bash
gaia --version
```

> **Important** : Assurez-vous que Lemonade Server est en cours d'exécution avant d'utiliser GAIA. GAIA nécessite que Lemonade Server soit démarré manuellement.

Pour plus d'informations, consultez la [documentation GAIA](https://amd-gaia.ai).