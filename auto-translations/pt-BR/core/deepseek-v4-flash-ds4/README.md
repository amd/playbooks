...
[ds4 (Dwarf Star 4)](https://github.com/antirez/ds4) é um mecanismo de inferência dedicado, construído especificamente para essa arquitetura de modelo. Em vez de um runtime de propósito geral, o ds4 tem como alvo diretamente a família DeepSeek V4, com otimizações de kernel específicas para a arquitetura do software AMD ROCm™. Atualmente, é uma das implementações de melhor desempenho do DeepSeek V4 Flash no Strix Halo.

Este tutorial mostra como usar o `ds4-cockpit`, uma interface de terminal, para configurar o ds4, baixar os pesos do modelo e iniciar o fornecimento local do DeepSeek V4 Flash na AMD Ryzen™ AI Halo Developer Platform.
...
> - **VRAM dedicada na BIOS (UMA frame buffer) definida no mínimo**, para que o pool de memória compartilhada possa ser o maior possível.
> - O pool de memória compartilhada da GPU **definido para pelo menos 110 GB**: execute `amd-ttm --set 110` (veja a etapa de configuração de memória acima) e reinicie. Valores mais baixos falham com falta de memória quando o modelo é carregado com um contexto de 126k. Se o seu sistema tiver menos memória disponível, diminua o valor de **Context** no Server Mode em vez disso.

O ds4-cockpit usa toolboxes em containers para executar o mecanismo ds4. Instale `podman`, `distrobox` e `pipx`:
...
O autor do ds4 fornece várias versões quantizadas do DeepSeek V4 Flash no formato GGUF. Todos os modelos abaixo usam calibração por matriz de importância (imatrix), que preserva maior precisão nas partes do modelo mais importantes para tarefas de codificação e raciocínio.
...
Na aba **Interactive Toolboxes**, selecione o toolbox mais recente disponível (por exemplo, `ds4-rocm-7.2.4`) e clique em **Create/Update**. Isso baixa a imagem do container e cria o ambiente do toolbox.

> **Dica**: A versão do toolbox mudará ao longo do tempo à medida que novas versões do ROCm forem lançadas. Escolha a mais recente disponível na lista.
...
Vá para a aba **Model Manager**. Selecione **IQ2_XXS imatrix (~80.8 GB)** no menu suspenso e clique em **Download**. Os arquivos do modelo serão salvos em `~/ds4` por padrão (você pode alterar o caminho de armazenamento).
...
Vá para a aba **Server Mode**. Selecione o modelo baixado e o toolbox, depois configure o tamanho do contexto (por exemplo, 126000), o host e a porta (8000). Quando estiver pronto, clique em **Start ds4-server**.
...
Você pode conectar qualquer interface de chat que suporte o formato de API da OpenAI. Por exemplo, para usar o HuggingFace ChatUI:
...
O servidor ds4 expõe endpoints compatíveis tanto com OpenAI quanto com Anthropic, então a maioria dos agentes de codificação pode se conectar a ele diretamente. Por exemplo, para adicioná-lo ao agente de codificação `pi`, adicione o seguinte bloco em `~/.pi/agent/models.json`:
...
- **Clustering multi-node**: Se você tiver dois dispositivos Halo, o ds4 oferece suporte à distribuição do modelo Q4 (~153 GB) entre as duas máquinas por meio de pipeline parallelism. Consulte a [documentação do ds4-toolbox](https://github.com/kyuz0/strix-halo-ds4-toolbox#distributed-inference-pipeline-parallelism) para instruções de configuração.
- **Speculative decoding (MTP)**: Baixe os pesos do MTP (~3,6 GB) e passe `--mtp` para o servidor para obter maior velocidade de geração.
- **KV cache disk offloading**: Para fluxos de trabalho de agentes de codificação, ative `--kv-disk-dir` para que os prompts de sistema repetidos sejam restaurados a partir do SSD em vez de serem recalculados a cada vez.
