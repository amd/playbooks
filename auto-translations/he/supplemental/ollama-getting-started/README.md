...
Ollama הוא כלי קל משקל פופולרי להרצת מודלים של שפה גדולה באופן מקומי. הוא מטפל בהורדת מודלים, כימות והגשה מאחורי ממשק שורת פקודה פשוט ואפליקציית שולחן עבודה, כך שתוכלו לעבור מאפס לשיחה עם LLM תוך דקות.

ספר המשחקים הזה מדריך אתכם בהתקנת Ollama, משיכת מודל GPT-OSS 20B וניהול שיחה איתו, הן דרך הטרמינל והן דרך אפליקציית שולחן העבודה.

## מה תלמדו

- כיצד להתקין ולהפעיל את Ollama במערכת שלכם
- משיכה והרצה של מודל GPT-OSS 20B באופן מקומי
- שיחה עם מודלים באמצעות ה-CLI
- שאילתת מודלים באופן תכנותי דרך ה-REST API
...
1. הורידו את תוכנית ההתקנה מ-[ollama.com/download](https://ollama.com/download).
2. הריצו את תוכנית ההתקנה `.exe` ופעלו לפי ההוראות.
3. לאחר ההתקנה, Ollama פועל כשירות רקע ונגיש מהטרמינל, אפליקציית שולחן העבודה ומגש המערכת.

אמתו את ההתקנה על ידי פתיחת טרמינל והרצת:
...
אמורים לראות את מספר הגרסה המותקנת מודפס בקונסולה.
<!-- @os:end -->
...
Ollama מנהל מודלים דרך רישום הדומה לתמונות קונטיינר. להורדת GPT-OSS 20B:

```bash
ollama pull gpt-oss:20b
```

פעולה זו מורידה את משקלי המודל למחשב המקומי שלכם (כ-12 GB). ההורדה מתרחשת פעם אחת בלבד, והרצות עתידיות טוענות את המודל מהדיסק.

ניתן לאשר שהמודל זמין עם:

```bash
ollama list
```

אמורים לראות את `gpt-oss:20b` בפלט יחד עם גודלו ותאריך השינוי האחרון שלו.
...
שמות מודלים ב-Ollama עוקבים אחר הפורמט `name:tag`. התג בדרך כלל מציין את ספירת הפרמטרים או גרסת הכימות. כמה פקודות שימושיות לניהול מודלים:

| פקודה | תיאור |
|---------|-------------|
| `ollama list` | הצגת כל המודלים שהורדו |
| `ollama pull <model>` | הורדת מודל ללא הרצתו |
| `ollama rm <model>` | הסרת מודל לפינוי שטח דיסק |
| `ollama show <model>` | הצגת מטא-נתונים ופרמטרים של המודל |

## שיחה מהטרמינל

הפעילו סשן שיחה אינטראקטיבי ישירות משורת הפקודה:

```bash
ollama run gpt-oss:20b
```

Ollama טוען את המודל לזיכרון ומוריד אתכם לפרומפט. נסו לשאול אותו משהו:

```
>>> What is the capital of France and why is it historically significant?
```

המודל מזרים את תגובתו אסימון-אחר-אסימון ישירות בטרמינל. הקלידו `/bye` או לחצו `Ctrl+D` לסיום הסשן.

> **טיפ**: ההרצה הראשונה לוקחת כמה שניות לטעינת המודל לזיכרון. פרומפטים עוקבים באותו סשן מגיבים הרבה יותר מהר מכיוון שהמודל נשאר טעון.
...
פתחו את **Ollama** מתפריט התחל או לחצו על סמל Ollama במגש המערכת ובחרו **Open Ollama**.

לאחר פתיחת האפליקציה:

1. לחצו על **New Chat** בסרגל הצד.
2. בחרו **gpt-oss:20b** מתפריט הנפתח של המודל בפינה הימנית התחתונה של אזור קלט השיחה.
3. הקלידו הודעה ולחצו Enter להתחלת השיחה.
...
לאחר ההתקנה, Ollama פועל כשירות רקע וחושף REST API על `http://localhost:11434` שניתן להשתמש בו לשילוב מודלים באפליקציות ובסקריפטים שלכם.
...
### דוגמה ב-Python
כעת שניתן לגשת ל-Ollama API באופן תכנותי, בואו נקרא לו מ-Python.
...
<!-- @os:end -->
#### יצירת קובץ Python
באותה ספרייה, השתמשו ב-VS Code או בעורך אחר ליצירת קובץ .py והעתיקו אליו את הקוד הבא. לאחר מכן, הריצו את הקובץ בסביבה המופעלת שלכם עם `python your_file_name.py`
...
| נקודת קצה | שיטה | מטרה |
|----------|--------|---------|
| `/api/generate` | POST | יצירת טקסט בתור יחיד |
| `/api/chat` | POST | שיחה רב-תורנית עם היסטוריית הודעות |
| `/api/tags` | GET | רשימת מודלים זמינים |
| `/api/show` | POST | הצגת פרטי מודל |
| `/api/pull` | POST | משיכת מודל מהרישום |

לעיון מלא ב-API, ראו את [תיעוד Ollama API](https://github.com/ollama/ollama/blob/main/docs/api.md).

## צעדים הבאים

- **נסו מודלים שונים**: עיינו ב-[ספריית המודלים של Ollama](https://ollama.com/library) לחקירת מאות מודלים זמינים, מעוזרי קידוד קטנים ועד מודלי היסק גדולים.
- **צרו מודלים מותאמים אישית**: השתמשו ב-[Modelfile](https://github.com/ollama/ollama/blob/main/docs/modelfile.md) להגדרת פרומפטי מערכת מותאמים, טמפרטורה ופרמטרים אחרים לחוויה מותאמת.
- **בנו עם ה-API**: השתמשו בספריות הלקוח של [Python](https://github.com/ollama/ollama-python) או [JavaScript](https://github.com/ollama/ollama-js) לשילוב Ollama באפליקציות שלכם.
- **התחברו לממשקי קצה**: שלבו את Ollama עם כלים כמו [Open WebUI](https://github.com/open-webui/open-webui) לממשק שיחה עשיר בתכונות עם חיפוש, פרסונות והעלאת מסמכים.
