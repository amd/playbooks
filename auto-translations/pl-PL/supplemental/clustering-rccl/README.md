...
Twój system Ryzen™ AI Halo jest już zdolny do lokalnego uruchamiania dużych modeli językowych. Klastrowanie idzie o krok dalej, łącząc pamięć GPU wielu systemów w ramach sieci lokalnej, dając Ci dostęp do jeszcze większych modeli o silniejszym rozumowaniu, lepszej generacji kodu i głębszym rozumieniu wielojęzycznym — wszystko całkowicie na Twoim własnym sprzęcie.

Ten poradnik nauczy Cię, jak sklastrować dwa systemy Ryzen AI Halo za pomocą RCCL (ROCm Communication Collectives Library) z vLLM oraz jak uruchomić Qwen3.5-397B, model o 397 miliardach parametrów, na obu maszynach z akceleracją ROCm.

## Czego się nauczysz

- Jak rozszerzyć alokację VRAM w systemach Ryzen AI Halo
- Uruchamianie vLLM ze wsparciem ROCm
- Konfigurowanie RCCL do wnioskowania z równoległością tensorową na wielu węzłach w dwóch systemach Ryzen AI Halo
...
| Przełącznik Ethernet 10 Gb/s | 1 | Centralny przełącznik umożliwiający komunikację wielowęzłową Ryzen AI Halo (co najmniej 2 porty) |
...
Podłącz każdą jednostkę Ryzen AI Halo do przełącznika Ethernet za pomocą kabla Cat 7 (lub wyższego). Ustanawia to łącze 10 Gb/s używane do szybkiej komunikacji między węzłami.
...
> **Uwaga**: Zastąp `<IFNAME>` nazwą interfejsu z wyniku kroku [1. Określenie interfejsów sieciowych](#1-determine-network-interfaces)
...
Tę ilość można zwiększyć, zmieniając ustawienie stron Translation Table Manager (TTM) w jądrze, zgodnie z poniższymi instrukcjami. AMD zaleca ustawienie minimalnej dedykowanej pamięci VRAM w BIOS-ie (0,5 GB).

* Zainstaluj narzędzie pipx i dodaj ścieżkę do pakietów zainstalowanych przez pipx do systemowej ścieżki wyszukiwania.
...
Twój system Ryzen AI Halo jest dostarczany z vLLM spakowanym wewnątrz gotowego obrazu kontenera, który uruchamiasz za pomocą Podman, darmowego narzędzia open source do konteneryzacji.

### 1. Utworzenie katalogu do pobierania modeli

Podczas serwowania modelu Qwen3.5-397B w tym poradniku, vLLM automatycznie pobierze wagi modelu do Twojego systemu. Aby upewnić się, że te wagi są dostępne wewnątrz kontenera, najpierw utwórz katalog na modele, który kontener będzie mógł zamontować:
...
Poniższe polecenie uruchamia kontener i przenosi Cię do interaktywnej powłoki. Montuje ono utworzony wcześniej katalog na modele oraz przekazuje Twoją wartość `IFNAME` do `NCCL_SOCKET_IFNAME` i `GLOO_SOCKET_IFNAME`, informując RCCL (bibliotekę używaną przez vLLM do koordynacji GPU w klastrze), którego interfejsu ma używać.
...
vLLM wykorzystuje Ray do orkiestracji klastra oraz RCCL do obsługi komunikacji GPU-GPU między węzłami. Jedna maszyna pełni rolę **węzła głównego** (Maszyna 1), koordynującego wnioskowanie. Druga dołącza jako **węzeł roboczy** (Maszyna 2), udostępniając swoją pamięć GPU i moc obliczeniową.

> **Uwaga**: Ray jest opcjonalną zależnością dla vLLM i jest dostępny wyłącznie wewnątrz wstępnie skonfigurowanego kontenera Podman.
...
### Krok 1: Uruchomienie węzła głównego Ray (Maszyna 1)
...
| `--language-model-only` | Pomija wczytywanie pomocniczych komponentów modelu (np. enkodera wizyjnego) |
| `--reasoning-parser` | Włącza strukturalne parsowanie danych wyjściowych rozumowania modelu |

Pełny opis użycia parametrów znajduje się w [dokumentacji vLLM](https://docs.vllm.ai/en/latest/configuration/engine_args/).

## Dostęp do modelu

vLLM udostępnia API kompatybilne z OpenAI, dzięki czemu możesz połączyć dowolnego kompatybilnego klienta lub interfejs z klastrem. Jedną z popularnych opcji jest [Open WebUI](https://github.com/open-webui/open-webui), który zapewnia interfejs czatu działający w przeglądarce.
...
- **Skaluj do czterech węzłów**: Dodaj dwa kolejne systemy Ryzen AI Halo jako dodatkowe workery Ray, aby dzielić modele na jeszcze większą liczbę GPU. Wymaga to przełącznika Ethernet z co najmniej czterema portami, po jednym dla każdego węzła. Wykonaj [Krok 2: Dołącz do klastra](#step-2-join-the-cluster-machine-2) na każdym dodatkowym workerze i odpowiednio zwiększ `--tensor-parallel-size`
