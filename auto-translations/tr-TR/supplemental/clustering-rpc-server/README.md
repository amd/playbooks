...
Ryzen™ AI Halo'nuz, büyük dil modellerini yerel olarak çalıştırma konusunda zaten yeterli kapasiteye sahiptir. Kümeleme, birden fazla sistemin GPU belleğini yerel bir ağ üzerinden birleştirerek bunu bir adım öteye taşır ve size daha güçlü akıl yürütme, daha iyi kod üretimi ve daha derin çok dilli anlayışa sahip, tamamen kendi donanımınızda çalışan çok daha büyük modellere erişim sağlar.

Bu playbook, llama.cpp'nin RPC motorunu kullanarak iki Ryzen AI Halo sistemini nasıl kümeleyeceğinizi ve AMD ROCm™ hızlandırması ile 358B parametreli bir model olan GLM 4.7'yi her iki makinede birden nasıl çalıştıracağınızı öğretir.

## Neler Öğreneceksiniz

- Ryzen AI Halo sistemlerinde VRAM ayırmayı nasıl genişletebileceğinizi
- llama.cpp'yi ROCm ve RPC desteğiyle nasıl kuracağınızı
- Bir RPC çalışanının (worker) nasıl yapılandırılacağını ve iki düğüm üzerinde dağıtık çıkarımın nasıl başlatılacağını
- 358B parametreli bir modeli ağa bağlı iki Ryzen AI Halo sisteminde nasıl çalıştıracağınızı
...
Bu miktar, aşağıdaki talimatlar izlenerek çekirdeğin Çeviri Tablosu Yöneticisi (TTM) sayfa ayarı değiştirilerek artırılabilir. AMD, BIOS'ta minimum ayrılmış VRAM'in (0.5 GB) ayarlanmasını önerir.
...
Bu playbook, yıldız topolojisinde bağlanan ve her birimin doğrudan anahtara (switch) kablolandığı iki Ryzen AI Halo birimi ile bir Ethernet anahtarı gerektirir.
...
- **Desktop Development with C++** iş yükü ile [Visual Studio Build Tools](https://visualstudio.microsoft.com/visual-cpp-build-tools/)
...
Her bir makinede, ağ arayüzünün adını bulun ve not edin (aşağıda `IFNAME` olarak anılacaktır). Çalıştırın:
...
> **Not**: Hız `10000Mb/s`'den düşükse veya bağlantı kurulmuyorsa kablo bağlantısını kontrol edin ve anahtar portunun 10Gbps olarak ayarlandığından emin olun. Bazı anahtarlar, otomatik müzakerenin devre dışı bırakılmasını ve bağlantı hızının manuel olarak ayarlanmasını gerektirir; lütfen anahtarınızın belgelerine başvurun.
...
Her bir makinede, ağ arayüzlerinizin bağlantı hızını kontrol edin:

```powershell
Get-NetAdapter | Select-Object Name, Status, LinkSpeed
```

Ethernet arayüzünüz `Up` durumda olmalı ve `10 Gbps` hızında çalışmalıdır:
...
> **Not**: Hız `10 Gbps`'den düşükse veya bağlantı kurulmuyorsa kablo bağlantısını kontrol edin ve anahtar portunun 10Gbps olarak ayarlandığından emin olun. Bazı anahtarlar, otomatik müzakerenin devre dışı bırakılmasını ve bağlantı hızının manuel olarak ayarlanmasını gerektirir; lütfen anahtarınızın belgelerine başvurun.
...
Lemonade SDK, gfx1151 (Strix Halo / Ryzen AI Max+ 395) ve diğer güncel Radeon mimarilerini hedefleyen, AMD ROCm 7 hızlandırmalı llama.cpp'nin gecelik (nightly) derlemelerini sağlar.

<!-- @os:windows -->
#### 1. Adım: Önceden Derlenmiş İkili Dosyaları İndirin
...
#### 2. Adım: İkili Dosyaları Ayıklayın
...
Bu dizin artık Ryzen AI Halo sisteminiz için önceden derlenmiş, ROCm özellikli `llama-cli.exe`, `llama-server.exe` ve `rpc-server.exe` derlemelerini içermektedir.

#### 3. Adım: GPU Algılamayı Doğrulayın
...
#### 2. Adım: İkili Dosyaları Ayıklayın ve Hazırlayın
...
#### 2. Adım: GPU Algılamayı Doğrulayın
...
#### 3. Adım: HIP'i Kullanıcı Yolunuza Ekleyin

Yukarıdaki derleme adımı `%HIP_PATH%\bin` değerini yalnızca mevcut oturum için ayarladı. HIP kitaplıklarını herhangi bir terminalde (yalnızca x64 Native Tools Command Prompt'ta değil) kullanılabilir hale getirmek için, bunu kullanıcı `PATH` değerinize kalıcı olarak ekleyin:
...
Bu kılavuz, Unsloth'tan [Q4_K_XL](https://huggingface.co/unsloth/GLM-4.7-GGUF/tree/main/UD-Q4_K_XL) niceleme formatında, 358B parametreli bir model olan [GLM 4.7](https://huggingface.co/zai-org/GLM-4.7)'yi kullanır. Bu nicelemede model yaklaşık 205GB depolama alanı gerektirir ve iki Ryzen AI Halo düğümünün birleşik GPU belleğine sığar.
...
llama.cpp RPC (Remote Procedure Call) motoru, tek bir llama.cpp örneğinin model katmanlarını ağ üzerinden uzak işçilere aktarmasına olanak tanır. Bir makine **denetleyici** (Makine 1) olarak hareket ederek belirteçleştirme, zamanlama ve orkestrasyonu yönetir. Diğer makine, GPU belleğini ve hesaplama gücünü denetleyiciye sunan hafif bir **RPC sunucusu** (Makine 2) çalıştırır.

Yükleme sırasında, llama.cpp modeli her iki düğüme de parçalar. Yüklendikten sonra, çıkarım tek bir hızlandırıcı üzerinde çalışıyormuş gibi devam eder. RPC, sahne arkasında tensör aktarımlarını ve senkronizasyonu yönetir.

### 1. Adım: RPC Sunucusunu Başlatın (Makine 2)
...
### 2. Adım: Modeli Başlatın (Makine 1)

Makine 2'de RPC sunucusu çalışırken, Makine 1'den `llama-cli` veya `llama-server` kullanarak çıkarımı başlatın.

#### llama-cli

`llama-cli`, modelle doğrudan etkileşim kurmak için terminal tabanlı bir arayüz sunar. Kıyaslama, hata ayıklama ve düşük seviyeli deneyler için idealdir.
...
Çalışmaya başladığında, `llama-cli` model yükleme ilerlemesini gösterir ve modelle doğrudan sohbet edebileceğiniz etkileşimli bir istem ekranına geçer:

![İki düğümde GLM 4.7 çalıştıran llama-cli](assets/llama-cli-example.png)
#### llama-server
...
- **Üçüncü taraf uygulamaları bağlayın**: `llama-server`, OpenAI uyumlu bir API sunar. Kümenize bağlanmak için herhangi bir OpenAI uyumlu uygulamayı (Open WebUI gibi) herhangi bir yer tutucu API anahtarıyla (ör. `none`) `http://<HOST_IP>:8081` adresine yönlendirin
...
- **Dört düğüme ölçeklendirin**: 1 trilyon parametre ölçeğindeki modellere erişmek için ek RPC çalışanları olarak iki Ryzen AI Halo sistemi daha ekleyin. `--rpc` parametresine ek uç noktaları virgülle ayrılmış bir liste olarak geçirin (ör. `--rpc <IP1>:50053,<IP2>:50053,<IP3>:50053`)