...
高效微调对于将大语言模型（LLM）适配到下游任务至关重要。LLaMA Factory 是一个开源且用户友好的平台，可简化大语言模型和多模态模型的训练与微调。它允许用户在本地以最少的编码量自定义数百种预训练模型。

本操作手册将教您如何在本地 AMD 硬件上使用 LLaMA Factory 微调 LLM。

<!-- @device:stx,krk -->
> **注意：** 本操作手册中的微调技术要求至少 **32 GB 系统内存**，且其中至少 **16 GB 可供 GPU 使用**（该 16 GB 是 32 GB 中的一部分，而非在其基础上额外提供）。
<!-- @device:end -->


<!-- @device:rx7900xt,rx9070xt,r9700 -->
<!-- @os:windows -->
> **注意：** 本操作手册中的微调技术要求至少 **16 GB 的 GPU 总内存**和 **32 GB 系统内存**。
> - 在 Windows 上，GPU 总内存是显卡专用显存与共享 GPU 内存（从系统内存中借用）之和。
> - 因此，专用显存不足 16 GB 的显卡仍可通过借用共享 GPU 内存来弥补差额，从而运行本操作手册。
<!-- @os:end -->

<!-- @os:linux -->
> **注意：** 本操作手册中的微调技术要求显卡至少具备 **16 GB 专用 GPU 内存**，以及 **32 GB 系统内存**。
> - 在 Linux 上，训练完全在显卡的专用显存中运行。
> - 当显存耗尽时，不会回退使用共享 GPU 内存（系统内存）。
> - 专用显存不足 16 GB 的显卡在 Linux 上训练时会出现内存不足的情况，即使系统拥有充足的内存也是如此。
<!-- @os:end -->
<!-- @device:end -->
...
- 时长：运行本操作手册大约需要 60 分钟（具体取决于您的模型/数据集大小及网络速度）。
...
**为您的用户授予 GPU 设备访问权限**（需注销并重新登录才能生效）：
...
LLaMA Factory 依赖于 PyTorch。按照上述要求，您应该已经安装了它。
...
LLaMA Factory 支持 Alpaca 格式和 ShareGPT 格式的微调数据集。所有可用的数据集均已在 [dataset_info.json](https://github.com/hiyouga/LlamaFactory/blob/main/data/dataset_info.json) 中定义。如果您使用自定义数据集，请务必在 `dataset_info.json` 中添加数据集描述，并在训练前指定数据集名称。详细信息请参阅其[官方文档](https://llamafactory.readthedocs.io/en/latest/getting_started/data_preparation.html)。
...
这些示例配置文件已经指定了模型参数、微调方法参数、数据集参数、评估参数等。您可以根据自身需求进行配置。在本手册中，我们将使用 [qwen3_lora_sft.yaml](https://github.com/hiyouga/LlamaFactory/blob/main/examples/train_lora/qwen3_lora_sft.yaml)。
...
- `lora_rank` - LoRA 中使用的低秩矩阵的维度，典型值：4、6、8、16（值越小，参数越少，微调速度越快；值越大，任务适应性越好，但资源消耗更高）。
...
- 将 `lora_rank` 从 `8` 设置为 `6`，以降低微调过程中的内存占用。
...
对于训练/微调，**llamafactory-cli train** 是 LLaMA Factory CLI 的核心子命令。它将微调工作流程（数据预处理、超参数调优、硬件优化）抽象为单个 CLI 命令，支持多种微调范式（LoRA/QLoRA/全参数微调），并针对低资源 GPU 进行了优化（例如在 16GB 显存上运行 QLoRA）。
...
**llamafactory-cli chat** 专为与 LLM（包括基础模型和经过 LoRA 微调的模型）进行交互式对话/推理而设计。LLaMA Factory 在 [examples/inference](https://github.com/hiyouga/LlamaFactory/tree/main/examples/inference) 中提供了运行微调模型推理的示例配置。您也可以修改此示例配置以更改设置，例如推理后端。

使用以下命令测试 Qwen3 微调后的模型：
...
在生产环境中，需要将预训练模型与 LoRA 适配器合并并导出为单一模型。这个合并后的模型可以作为普通的 Hugging Face 模型文件使用。LLaMA Factory 在 [examples/merge_lora](https://github.com/hiyouga/LlamaFactory/tree/main/examples/merge_lora) 中提供了示例配置。

使用以下命令导出 Qwen3 微调后的模型：
...
`LLaMA-Factory` 还支持通过浏览器中的网页界面进行零代码大语言模型微调。
...
```
`LlamaFactory Web UI` 提供了一个简洁的界面，用于管理机器学习工作流程，包括训练、评估、预测、对话和导出模型。以下是各个选项卡的简要介绍：

* **Train（训练）**：此选项卡允许您选择模型和数据集、配置训练参数并启动训练过程。理解必需参数和可选参数对于优化训练设置至关重要。
* **Evaluate & Predict（评估与预测）**：训练完成后，您可以使用此选项卡评估模型的性能并进行预测。它可以帮助您深入了解模型在新数据上的准确性和有效性。
* **Chat（对话）**：训练完成后，在 Chat 选项卡中加载模型即可与其交互并查看您的成果。此功能支持与训练好的模型进行实时交流。
* **Export（导出）**：此选项卡便于导出训练好的模型以供部署或进一步使用。您可以将模型保存为适用于不同应用场景的各种格式。

如需详细指导，建议参阅 [LlamaFactory GitHub 仓库](https://github.com/hiyouga/LlamaFactory#fine-tuning-with-llama-board-gui-powered-by-gradio)和 [LlamaFactory ReadTheDocs](https://llamafactory.readthedocs.io/en/latest) 上的官方文档。此外，[Wiki LLaMA Board Web UI](https://deepwiki.com/xtong-zhang/Chain-of-Focus/3.2-llama-board-web-ui) 也提供了关于该界面及其功能的宝贵见解。

## 后续步骤
- 尝试使用 `gpt-oss` 等不同模型以及其他先进模型。
- 在微调后的模型上试验不同的后端
