...
[Ryzen AI CVML 库](https://ryzenai.docs.amd.com/en/latest/ryzen_ai_libraries.html#ryzen-ai-cvml-library)是 AMD 的 C++ 计算机视觉与机器学习工具包，提供强大的设备端感知能力——包括深度估计、人脸检测和人脸网格追踪。该库基于 Ryzen AI 驱动程序构建，能够自动选择最佳可用硬件（GPU 或 NPU）进行推理，让您无需关心模型训练或框架集成即可为 C++ 应用程序添加 AI 功能。所有处理均在本地系统上完成，非常适合对隐私敏感、低延迟的应用场景。

本手册将指导您如何设置 Ryzen AI CVML 库、构建附带的示例应用程序，并在示例图像上运行人脸检测。

## 您将学到的内容

- 如何安装前置条件并在系统上设置 Ryzen AI CVML 库
- CVML C++ API 的工作原理：上下文、特征对象和图像缓冲区
- 如何使用 CMake 和 OpenCV 构建并运行附带的示例应用程序
- 如何在图像上运行人脸检测，并显示边界框和关键点
...
<!-- @os:windows -->
- [OpenCV 4.11](https://github.com/opencv/opencv/releases/tag/4.11.0) — 下载 `opencv-4.11.0-windows.exe`，运行后解压到本地文件夹（例如 `C:\opencv`）
- [CMake](https://cmake.org/download/) — 下载 Windows x86-64 MSI 安装程序，安装时选择 **"Add CMake to the system PATH for all users"**
- [Ryzen AI NPU 驱动程序](https://ryzenai.docs.amd.com/en/latest/inst.html) — 安装最新可用版本
- [Visual Studio 2022 Community](https://aka.ms/vs/17/release/vs_community.exe)，并选择"使用 C++ 的桌面开发"工作负载（包含 MSVC 编译器、Windows SDK 和 C++ 构建工具）
<!-- @os:end -->
...
- CMake — 通过 apt 安装：
  ...
  ```
- Ubuntu 22.04 或 24.04（内核版本 >= 6.11.0-21-generic）
...
| 文件夹 | 内容 |
|--------|----------|
| `cmake/` | CMake `find_package` 函数的打包信息 |
| `include/` | C++ 头文件（`cvml-depth-estimation.h`、`cvml-face-detector.h`、`cvml-face-mesh.h` 等） |
| `windows/` | Windows 的二进制文件（编译时 `.LIB` 和运行时 `.DLL`/`.GRAPHLIB`/`.AMODEL` 文件） |
| `linux/` | Linux 的二进制文件（编译和运行时 `.SO` 文件） |
| `samples/` | 包含源代码的各个示例应用程序 |
...
共享库安装在 `<build>/install/lib/` 下。在后续步骤中，将 `install` 目录用作 `OPENCV_INSTALL_ROOT`。
...
如果您运行的是 Ubuntu 24.04，请安装额外的必需软件包：
...
CVML 库提供了简洁的 C++ API，每种感知功能（深度估计、人脸检测、人脸网格）都有其对应的头文件和特征对象。您无需直接操作原始模型——库会自动处理模型加载、预处理和推理。
...
| **人脸检测** | `cvml-face-detector.h` | 检测人脸，并提供边界框、关键点（眼睛、鼻子、嘴巴）和置信度分数 |
| **人脸网格** | `cvml-face-mesh.h` | 使用密集网格点追踪详细的面部几何形状 |
...
2. **创建特征对象** — 针对上下文实例化特定功能（例如 `amd::cvml::DepthEstimation`）。
3. **封装输入数据** — 使用 `amd::cvml::Image` 封装您的 RGB 图像缓冲区，无需复制数据。
4. **执行** — 调用特征的处理方法并读取结果。
...
库会自动为每个操作选择最佳硬件（GPU 或 NPU）。您也可以显式设置后端：
...
> **注意：** 使用 ONNX 后端进行 NPU 操作的功能在首次运行时可能会出现较长的启动延迟。后续运行将会更快。

> **注意：** 如果目标系统上未安装 NPU 驱动程序，Ryzen AI CVML 库将自动回退到 GPU 后端进行推理操作。

## 构建示例应用程序

CVML 库包含每种功能的即用型示例应用程序。让我们一次性构建所有示例。

1. 将 `OPENCV_INSTALL_ROOT` 环境变量设置为指向您的 OpenCV 安装目录：
...
将弹出一个窗口，显示带有边界框、置信度分数和面部关键点（眼睛、鼻子、嘴角）的图像。
...
| 模型 | 速度 | 精度 | 最适用于 |
|-------|-------|----------|----------|
| `fast`（默认） | 更高帧率 | 良好 | 实时摄像头应用 |
| `precise` | 较低帧率 | 最佳 | 照片分析、高精度需求 |
...
其中 `AMD_CVML_SDK_ROOT` 指向 Ryzen AI CVML 库文件夹的根目录。然后为您想要使用的功能包含相应的头文件：
...
对于以下每个示例，请先按照上方[运行人脸检测](#running-face-detection)部分的相同模式导航到其可执行文件所在的文件夹（例如，在 Windows 上执行 `cd build\cvml-sample-depth-estimation\Release`，在 Linux 上执行 `cd build/cvml-sample-depth-estimation`）。在 Windows 上，请在每个命令后附加 `.exe`（例如 `cvml-sample-depth-estimation.exe`）。

- **尝试深度估计**：运行 `cvml-sample-depth-estimation -i sample_face.jpg` 以生成彩色深度图——较近的物体显示为暖色，较远的物体显示为冷色
- **探索人脸网格**：运行 `cvml-sample-face-mesh -i sample_face.jpg` 以查看带有详细网格点的密集面部几何追踪
- **处理视频文件**：在任意示例上使用 `-i` 和 `-o` 标志来处理视频（例如 `cvml-sample-face-detection -i video.mp4 -o output.mp4`）
- **比较模型变体**：在人脸检测上尝试 `-m precise` 与默认的 `-m fast`，亲身体验精度/速度的权衡
- **构建您自己的应用程序**：使用 CMake 集成和 C++ API 将 CVML 功能添加到您自己的 C++ 应用程序中
- **组合功能**：在同一应用程序中将人脸检测与深度估计结合使用，以获得更丰富的场景理解
- **浏览源代码**：阅读 [GitHub 上的 Ryzen AI CVML 库](https://github.com/amd/RyzenAI-SW/tree/main/Ryzen-AI-CVML-Library)，获取头文件文档、更多示例和 API 详情