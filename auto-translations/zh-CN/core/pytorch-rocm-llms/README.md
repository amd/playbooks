...
<!-- @github-only -->
> [!IMPORTANT]
> 本行动手册使用了 GitHub 无法渲染的特殊标签。请访问 [amd.com/playbooks](https://amd.com/playbooks) 以正确预览此内容。
<!-- @github-only:end -->
...
> **注意：** 当您加载模型时，Hugging Face Transformers 会首先检查其本地缓存（Linux 上为 `~/.cache/huggingface/hub`，Windows 上为 `C:\Users\<user>\.cache\huggingface\hub`）。如果模型未被缓存，它会自动从 huggingface.co 下载。首次运行可能需要几分钟，具体取决于模型大小和网络速度。
