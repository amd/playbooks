...
GAIA 是 AMD 的开源框架,用于构建在配备 Ryzen AI 加速的 AMD 硬件上本地运行的 AI 智能体。

#### 安装 GAIA

<!-- @device:halo_box -->
<!-- @os:windows -->
1. 在 Windows 上,在您选择的目录中打开终端,然后按照以下命令创建 venv。
...
<!-- @os:linux -->
1. 在 Linux 上,在您选择的目录中打开终端,然后按照以下命令创建 venv。
...
这将安装 Lemonade Server、下载默认模型并验证设置。
...
> **重要提示**:请确保在使用 GAIA 之前已运行 Lemonade Server。GAIA 需要手动启动 Lemonade Server。

有关更多信息,请参阅 [GAIA 文档](https://amd-gaia.ai)。