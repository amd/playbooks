...
Программный стек AMD ROCm™ и PyTorch создают единую экосистему для локального ИИ. Он работает как на Windows, так и на Linux с официальной поддержкой широкого спектра устройств, включая APU Ryzen™ AI и GPU Radeon™.

Этот сборник инструкций научит вас запускать низколатентный, выразительный и конфиденциальный перевод речи в речь полностью на устройстве.

## Чему вы научитесь

- Как настроить среду для перевода речи в речь
- Как писать код на Python для загрузки и использования моделей речь-в-речь
- Как запускать и экспериментировать с пользовательским интерфейсом Gradio

## Зачем использовать перевод речи в речь в реальном времени?

- Устраняет барьеры между переводом и языковыми различиями
- Передаёт тон, эмоции и намерения без неловких пауз
- Обеспечивает глобальное сотрудничество и ускоряет принятие решений
...
<!-- @os:linux -->
<!-- @device:halo_box -->
На Linux откройте терминал и выполните следующую команду для создания venv с уже установленными ROCm+PyTorch:
...
На Linux откройте терминал и выполните следующую команду для создания venv:
...
<!-- @os:windows -->
<!-- @device:halo_box -->
На Windows откройте терминал в выбранном каталоге и выполните следующие команды для создания venv с уже установленными ROCm+PyTorch:
...
<!-- @device:halo,stx,krk,rx7900xt,rx9070xt,r9700 -->
На Windows откройте терминал в выбранном каталоге и выполните следующие команды для создания venv:
...
Ознакомьтесь с [карточкой модели](https://huggingface.co/facebook/seamless-m4t-v2-large/tree/main) на Hugging Face для получения дополнительной информации.
Это техническая архитектура моделей речь-в-речь:
<p align="center">
...
Этот сборник инструкций включает готовые к использованию скрипты. Пожалуйста, загрузите все их в тот же каталог, где была создана ваша среда.

| Скрипт | Описание | Использование |
|--------|-------------|-------|
| [infer.py](assets/infer.py) | Базовая генерация текста с помощью LLM | `python infer.py` |
| [input1.wav](assets/input1.wav) | Пример аудиофайла | N/A |
| [lang_list.py](assets/lang_list.py) | Файл поддержки языков | N/A |
| [gradio_demo.py](assets/gradio_demo.py) | Удобный интерфейс для перевода речи | `python gradio_demo.py --no-share` |


### Начало работы с infer.py

Для выполнения скрипта запустите 
```bash
python infer.py
```
> **Примечание**: Вы можете увидеть некоторые предупреждения. Это ожидаемое поведение.
...
Эта функция принимает идентификатор модели и загружает модель, если она ещё не загружена. Затем она возвращает процессор и модель для использования в следующей функции.
```python
...
**Фрагмент 3: Входной аудиоклип в формате .wav и его предобработка**

Эта функция загружает аудиоклип и выполняет его ресемплирование до целевой частоты.
```python
...
Эта функция выполняет инференс с помощью модели и возвращает сгенерированный результат.
```python
...
Эта функция сохраняет массив аудиоданных в файл .WAV. 
```python
...
### Запуск демонстрации с пользовательским интерфейсом Gradio:

Теперь, когда вы запустили базовый пример скрипта, следующие инструкции предоставляют удобный интерфейс, который основывается на написанном нами коде и упрощает живой перевод речи в речь.
...
```
Затем откройте веб-браузер по адресу `http://127.0.0.1:7860` для доступа к интерфейсу.
...
- Поделитесь своей демонстрацией с другими: добавьте --share для создания публичной ссылки, доступной для всех удалённо, или разверните постоянно с помощью Hugging Face Spaces

## Ресурсы

Ниже приведены дополнительные ресурсы для получения дополнительной информации о переводе речи в речь:  
* Репозиторий находится здесь: https://huggingface.co/facebook/seamless-m4t-v2-large 
* Академические исследования по теме «Seamless: Multilingual Expressive and Streaming Speech Translation»
* Публикация и развёртывание Gradio: [Руководство по публикации приложения](https://www.gradio.app/guides/sharing-your-app) и [Развёртывание в Hugging Face Spaces](https://shafiqulai.github.io/blogs/blog_5.html)