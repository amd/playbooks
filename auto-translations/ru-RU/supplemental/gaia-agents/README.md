...
Агенты GAIA — это AI-ассистенты, которые используют локальную LLM для рассуждений и вызова определённых вами инструментов — как чат-боты, способные выполнять действия. Они работают **на 100% локально**: без облачных API, без передачи данных за пределы вашего устройства и без необходимости в API-ключах.

В этом руководстве вы создадите агента Hardware Advisor, который определяет объём RAM, GPU и NPU вашей системы, запрашивает локальный каталог моделей и рекомендует, какие LLM может запустить ваша машина. Это практическое введение в GAIA Agent SDK, результатом которого станет нечто сразу же полезное.

## Что вы узнаете

- Как создать агента GAIA с пользовательскими инструментами
- Использование LemonadeClient SDK для запроса информации о системе и каталогов моделей
- Платформо-зависимое определение GPU/NPU (Windows PowerShell и Linux lspci)
- Подбор размера модели на основе объёма памяти с использованием правила 70%
- Создание интерактивного CLI для запросов об оборудовании на естественном языке
...
Сначала запустите готового агента, чтобы увидеть, что именно вы создаёте. Затем мы разберём код шаг за шагом.

### Запуск готового примера

В этом руководстве уже содержится полный файл [hardware_advisor_agent.py](assets/hardware_advisor_agent.py). Загрузите его в выбранный каталог и запустите, чтобы увидеть готового агента в действии:
...
**Попробуйте спросить:** «What size LLM can I run?»
...
Оставшаяся часть руководства посвящена объяснению того, как работает каждая часть скрипта, чтобы вы могли понять его с нуля.
<!-- @os:windows -->
...
Агент Hardware Advisor объединяет три компонента:

- **LemonadeClient SDK** — API для получения информации о системе и каталога моделей
- **Платформо-зависимое определение** — Windows PowerShell / Linux lspci для получения информации о GPU
- **Вычисления памяти** — правило 70% для безопасного подбора размера модели

Данные проходят через них последовательно: запрос пользователя → агент выбирает инструмент → инструмент вызывает LemonadeClient + определение ОС → агент синтезирует результаты в рекомендацию.

### LemonadeClient SDK

LemonadeClient предоставляет унифицированный API для определения системы, доступности NPU/GPU и запросов к каталогу моделей.
...
### Платформо-зависимое определение GPU

Агент использует команды, встроенные в ОС, а не PyTorch для определения GPU. Это работает без установленных драйверов GPU, обнаруживает все GPU (не только совместимые с CUDA) и позволяет избежать импорта тяжёлых библиотек.
...
### Правило 70% для памяти

> **Правило:** Размер модели должен быть менее 70% доступной RAM, чтобы оставить 30% запаса для операций вывода (KV-кэш, буферы пакетной обработки, пиковые скачки потребления памяти во время выполнения).
...
## Пошаговое написание кода агента (необязательно)

Вы создадите **один файл** с именем `hardware_advisor_agent.py` и будете постепенно добавлять функциональность. Каждый шаг основывается на предыдущем.
...
**Замените метод `_register_tools()`** инструментом `get_hardware_info`:
...
**Обновите блок `__main__`** для включения интерактивного тестирования:
...
Запустите и попробуйте спросить «Show me my system specs»:
...
Добавьте инструмент `list_available_models()` внутри `_register_tools()`, после функции `get_hardware_info`. Теперь агент может сообщать вам, какие модели доступны.
...
Запустите и попробуйте спросить «What models are available?»:
...
Добавьте инструмент `recommend_models()` внутри `_register_tools()`, после `list_available_models`. Теперь агент может вычислять, какие модели помещаются в память вашей системы, используя правило 70%.
...
Запустите и попробуйте спросить «What size LLM can I run?»:
...
### Шаг 5: Производственный CLI

Замените простой блок `__main__` на полноценный интерактивный CLI. Это добавляет баннер, команды выхода и улучшенную обработку ошибок.

**Замените весь блок `if __name__ == "__main__":` на:**
...
Ваш файл `hardware_advisor_agent.py` должен теперь содержать все следующие компоненты:
...
- [x] Вспомогательный метод `_get_gpu_info()` (Windows PowerShell + Linux lspci)
- [x] Инструмент `get_hardware_info()` с полями GPU, NPU и ОС
- [x] Инструмент `list_available_models()` с метками и обогащением информацией о размере
- [x] Инструмент `recommend_models()` с правилом 70%, fits_in_ram, fits_in_gpu
- [x] Функция `main()` с интерактивным CLI

**Проверьте работу с помощью следующих запросов:**
...
- **Изучите API LemonadeClient** — Откройте для себя больше возможностей управления системой и моделями в [документации LemonadeClient SDK](https://amd-gaia.ai/sdk/lemonade-client)
...
- **Добавьте поддержку MCP** — Откройте доступ к советнику по оборудованию как к MCP-серверу, чтобы другие инструменты могли его запрашивать. См. [руководство MCP](https://amd-gaia.ai/sdk/infrastructure/mcp)
- **Расширьте движок рекомендаций** — Учитывайте VRAM GPU для выгрузки слоёв или добавьте бенчмаркинг для оценки количества токенов в секунду
- **Создайте мультиагентную систему** — Объедините советника по оборудованию с агентом кода или агентом чата с помощью [Routing Agent](https://amd-gaia.ai/guides/routing)