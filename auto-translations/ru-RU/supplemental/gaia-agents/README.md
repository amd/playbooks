...
<!-- @github-only -->
> [!IMPORTANT]
> В этом плейбуке используются специальные теги, которые GitHub не может отобразить. Пожалуйста, посетите [amd.com/playbooks](https://amd.com/playbooks), чтобы корректно просмотреть этот контент.
<!-- @github-only:end -->

## Обзор

Агенты GAIA — это ИИ-ассистенты, которые используют локальную LLM для рассуждений и вызова определённых вами инструментов — подобно чат-ботам, способным выполнять действия. Они работают **на 100% локально**, без облачных API, без передачи данных за пределы вашего компьютера и без необходимости в API-ключах.

В этом плейбуке вы создадите агента Hardware Advisor Agent, который определяет объём RAM, GPU и NPU вашей системы, запрашивает локальный каталог моделей и рекомендует, какие LLM ваш компьютер сможет запустить. Это практическое введение в GAIA Agent SDK, результатом которого станет сразу полезный инструмент.

## Чему вы научитесь

- Как создать агента GAIA с пользовательскими инструментами
- Использование LemonadeClient SDK для запроса информации о системе и каталогах моделей
- Определение GPU/NPU в зависимости от платформы (Windows PowerShell и Linux lspci)
- Подбор размера модели на основе доступной памяти по правилу 70%
- Создание интерактивного CLI для запросов о характеристиках оборудования на естественном языке
...
Сначала запустите готового агента, чтобы увидеть, что вы будете создавать. Затем мы разберём код шаг за шагом.

### Запуск готового примера

В этот плейбук включён полный файл [hardware_advisor_agent.py](assets/hardware_advisor_agent.py). Загрузите его в выбранный вами каталог и запустите, чтобы увидеть готового агента в действии:
...
**Попробуйте спросить:** «Какую по размеру LLM я могу запустить?»
...
**Поздравляем** — вы создали агента!

Оставшаяся часть плейбука объяснит, как работает каждая часть скрипта, чтобы вы могли понять его с самых основ.
<!-- @os:windows -->
...
Агент Hardware Advisor Agent объединяет три компонента:

- **LemonadeClient SDK** — API для получения информации о системе и каталога моделей
- **Определение специфичное для платформы** — Windows PowerShell / Linux lspci для получения информации о GPU
- **Расчёты памяти** — правило 70% для безопасного подбора размера модели

Данные проходят через эти компоненты последовательно: запрос пользователя → агент выбирает инструмент → инструмент вызывает LemonadeClient + определение ОС → агент синтезирует результаты в рекомендацию.

### LemonadeClient SDK

LemonadeClient предоставляет унифицированный API для определения системы, доступности NPU/GPU и запросов каталога моделей.
...
**`get_system_info()`** — возвращает ОС, CPU, RAM и доступность устройств:
...
**`list_models(show_all=True)`** — возвращает полный каталог моделей:
...
**`get_model_info(model_id)`** — возвращает оценки размера для конкретной модели:
...
### Определение GPU в зависимости от платформы

Агент использует нативные команды ОС, а не PyTorch, для определения GPU. Это работает без установленных драйверов GPU, обнаруживает все GPU (а не только те, что поддерживают CUDA), и избегает импорта тяжёлых библиотек.

<!-- @os:windows -->

В Windows агент использует PowerShell для запроса WMI:
...
В Linux агент использует lspci:
...
> **Правило:** Размер модели должен составлять менее 70% доступной RAM, чтобы оставить 30% резерва для операций вывода (KV-кэш, буферы пакетной обработки, всплески использования памяти во время выполнения).
...
## Пошаговое написание кода агента (опционально)

Вы создадите **один файл** с именем `hardware_advisor_agent.py` и будете постепенно добавлять функциональность. Каждый шаг строится на предыдущем.

### Шаг 1: Скелет агента

Начните с минимальной структуры агента — только класс и базовый системный промпт. У агента пока нет инструментов.
...
### Шаг 2: Определение GPU и оборудования

Добавьте вспомогательный метод `_get_gpu_info()` и инструмент `get_hardware_info()`. Это делает агента интерактивным — теперь вы можете запрашивать у него информацию о характеристиках системы.
...
**Добавьте вспомогательный метод `_get_gpu_info()`** после метода `_get_system_prompt()`:
...
**Замените метод `_register_tools()`** на инструмент `get_hardware_info`:
...
**Обновите блок `__main__`**, чтобы включить интерактивное тестирование:
...
Запустите и попробуйте спросить «Покажи характеристики моей системы»:
...
### Шаг 3: Каталог моделей

Добавьте инструмент `list_available_models()` внутри `_register_tools()`, после функции `get_hardware_info`. Теперь агент может сообщить, какие модели доступны.
...
Запустите и попробуйте спросить «Какие модели доступны?»:
...
### Шаг 4: Умные рекомендации

Добавьте инструмент `recommend_models()` внутри `_register_tools()`, после `list_available_models`. Теперь агент может рассчитать, какие модели поместятся в память вашей системы, используя правило 70%.
...
Запустите и попробуйте спросить «Какую по размеру LLM я могу запустить?»:
...
### Шаг 5: CLI для продакшена

Замените простой блок `__main__` на отполированный интерактивный CLI. Это добавит баннер, команды выхода и улучшенную обработку ошибок.

**Замените весь блок `if __name__ == "__main__":`** на следующий:
...
Теперь ваш `hardware_advisor_agent.py` должен содержать все следующие компоненты:

- [x] Импорты: `from typing import Any, Dict` и `from gaia import Agent, tool`
- [x] Класс `HardwareAdvisorAgent` с `__init__` и системным промптом
- [x] Вспомогательная функция `_get_gpu_info()` (Windows PowerShell + Linux lspci)
- [x] Инструмент `get_hardware_info()` с полями GPU, NPU и OS
- [x] Инструмент `list_available_models()` с метками и обогащением информацией о размере
- [x] Инструмент `recommend_models()` с правилом 70%, `fits_in_ram` и `fits_in_gpu`
- [x] Функция `main()` с интерактивным CLI

**Проверьте эти запросы, чтобы убедиться, что всё работает:**
...
> **Совет**: Полная реализация доступна в файле [hardware_advisor_agent.py](assets/hardware_advisor_agent.py).

## Дальнейшие шаги

- **Изучите API LemonadeClient** — узнайте больше о возможностях управления системой и моделями в [документации SDK LemonadeClient](https://amd-gaia.ai/sdk/lemonade-client)
- **Добавьте голосовое взаимодействие** — интегрируйте Whisper ASR и Kokoro TTS, чтобы пользователи могли задавать вопросы об оборудовании голосом. См. [руководство Talk](https://amd-gaia.ai/guides/talk)
- **Добавьте поддержку MCP** — предоставьте hardware advisor в виде сервера MCP, чтобы другие инструменты могли обращаться к нему с запросами. См. [руководство по MCP](https://amd-gaia.ai/sdk/infrastructure/mcp)
- **Расширьте систему рекомендаций** — учитывайте объём видеопамяти GPU для выгрузки слоёв или добавьте бенчмаркинг для оценки токенов в секунду
- **Постройте многоагентную систему** — объедините hardware advisor с агентом кода или чат-агентом с помощью [Routing Agent](https://amd-gaia.ai/guides/routing)