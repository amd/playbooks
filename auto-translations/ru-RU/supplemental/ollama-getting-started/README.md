...
<!-- @github-only -->
> [!IMPORTANT]
> В этом руководстве используются специальные теги, которые GitHub не может отобразить. Пожалуйста, посетите [amd.com/playbooks](https://amd.com/playbooks) для корректного просмотра этого материала.
<!-- @github-only:end -->

## Обзор

Ollama — это популярный легковесный инструмент для локального запуска больших языковых моделей. Он берёт на себя загрузку моделей, квантование и обслуживание, скрывая всё это за простым интерфейсом командной строки и desktop-приложением, поэтому вы можете начать общаться с LLM за считанные минуты.

Это руководство проведёт вас через установку Ollama, загрузку модели GPT-OSS 20B и общение с ней как через терминал, так и через desktop-приложение.

## Чему вы научитесь

- Как установить и запустить Ollama в вашей системе
- Загрузить и запустить модель GPT-OSS 20B локально
- Общаться с моделями через CLI
- Программно обращаться к моделям через REST API
...
1. Загрузите установщик с [ollama.com/download](https://ollama.com/download).
2. Запустите установщик `.exe` и следуйте подсказкам.
3. После установки Ollama работает как фоновая служба и доступна из терминала, desktop-приложения и системного трея.
...
Вы должны увидеть номер установленной версии, выведенный в консоль.
<!-- @os:end -->
...
## Загрузка вашей первой модели

Ollama управляет моделями через реестр, похожий на реестры образов контейнеров. Чтобы загрузить GPT-OSS 20B:

```bash
ollama pull gpt-oss:20b
```

Это загрузит веса модели на ваш локальный компьютер (примерно 12 ГБ). Загрузка происходит только один раз, при последующих запусках модель загружается с диска.

Вы можете убедиться, что модель доступна, с помощью:

```bash
ollama list
```

Вы должны увидеть `gpt-oss:20b` в выводе вместе с её размером и датой последнего изменения.
...
Названия моделей в Ollama следуют формату `name:tag`. Тег обычно указывает на количество параметров или вариант квантования. Вот несколько полезных команд для управления моделями:

| Команда | Описание |
|---------|-------------|
| `ollama list` | Показать все загруженные модели |
| `ollama pull <model>` | Загрузить модель без запуска |
| `ollama rm <model>` | Удалить модель для освобождения места на диске |
| `ollama show <model>` | Показать метаданные и параметры модели |

## Общение через терминал

Запустите интерактивную сессию чата прямо из командной строки:

```bash
ollama run gpt-oss:20b
```

Ollama загрузит модель в память и переведёт вас в режим ввода запросов. Попробуйте что-нибудь спросить:

```
>>> What is the capital of France and why is it historically significant?
```

Модель выводит ответ токен за токеном прямо в терминале. Введите `/bye` или нажмите `Ctrl+D`, чтобы завершить сессию.

> **Совет**: Первый запуск занимает несколько секунд для загрузки модели в память. Последующие запросы в рамках той же сессии обрабатываются гораздо быстрее, так как модель остаётся загруженной.

<!-- @os:windows -->
## Общение через desktop-приложение

Ollama также поставляется с desktop-приложением, которое предоставляет удобный интерфейс чата для взаимодействия с вашими моделями.

Откройте **Ollama** из меню «Пуск» или щёлкните значок Ollama в системном трее и выберите **Open Ollama**.

После открытия приложения:

1. Нажмите **New Chat** на боковой панели.
2. Выберите **gpt-oss:20b** в выпадающем списке моделей в правом нижнем углу области ввода сообщения.
3. Введите сообщение и нажмите Enter, чтобы начать общение.

<p align="center">
  <img src="assets/ollama_app.png" alt="Ollama desktop app chatting with gpt-oss:20b" width="600"/>
</p>

Desktop-приложение хранит историю ваших разговоров на боковой панели, что позволяет легко возвращаться к предыдущим чатам.
<!-- @os:end -->

## Использование REST API

После установки Ollama работает как фоновая служба и предоставляет REST API по адресу `http://localhost:11434`, который можно использовать для интеграции моделей в собственные приложения и скрипты.
...
Ответ представляет собой объект JSON, содержащий вывод модели в поле `response`.


### Пример на Python
Теперь, когда мы можем обращаться к API Ollama программно, давайте вызовем его из Python.
...
В том же каталоге используйте VS Code или другой редактор, чтобы создать файл .py, и скопируйте в него следующий код. Затем запустите файл в активированном окружении с помощью `python your_file_name.py`
...
|----------|--------|---------|
| `/api/generate` | POST | Однократная генерация текста |
| `/api/chat` | POST | Многоходовой разговор с историей сообщений |
| `/api/tags` | GET | Список доступных моделей |
| `/api/show` | POST | Показать сведения о модели |
| `/api/pull` | POST | Загрузить модель из реестра |

Полное описание API см. в [документации Ollama API](https://github.com/ollama/ollama/blob/main/docs/api.md).
## Дальнейшие шаги

- **Попробуйте другие модели**: просмотрите [библиотеку моделей Ollama](https://ollama.com/library), чтобы изучить сотни доступных моделей — от небольших помощников для написания кода до крупных моделей для рассуждений.
- **Создавайте пользовательские модели**: используйте [Modelfile](https://github.com/ollama/ollama/blob/main/docs/modelfile.md) для настройки системных подсказок, temperature и других параметров под ваши задачи.
- **Разрабатывайте с использованием API**: используйте клиентские библиотеки [Python](https://github.com/ollama/ollama-python) или [JavaScript](https://github.com/ollama/ollama-js), чтобы интегрировать Ollama в свои приложения.
- **Подключайте интерфейсы**: используйте Ollama вместе с такими инструментами, как [Open WebUI](https://github.com/open-webui/open-webui), чтобы получить многофункциональный интерфейс чата с поиском, персонами и загрузкой документов.

Дополнительную информацию можно найти в [документации Ollama](https://github.com/ollama/ollama/blob/main/README.md).