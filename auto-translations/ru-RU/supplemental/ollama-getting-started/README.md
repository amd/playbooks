...
Ollama — это популярный лёгкий инструмент для локального запуска больших языковых моделей. Он берёт на себя загрузку моделей, квантизацию и обслуживание через простой интерфейс командной строки и настольное приложение, позволяя перейти от нуля до общения с LLM за считанные минуты.

Этот playbook проведёт вас через установку Ollama, загрузку модели GPT-OSS 20B и общение с ней — как через терминал, так и через настольное приложение.
...
- Обращаться к моделям программно через REST API
...
2. Запустите установщик `.exe` и следуйте инструкциям.
3. После установки Ollama работает как фоновая служба и доступна из терминала, настольного приложения и системного трея.
...
Ollama управляет моделями через реестр, аналогичный реестру образов контейнеров. Чтобы загрузить GPT-OSS 20B:

```bash
ollama pull gpt-oss:20b
```

Это загружает веса модели на ваш локальный компьютер (приблизительно 12 ГБ). Загрузка происходит только один раз, а при последующих запусках модель загружается с диска.

Вы можете убедиться, что модель доступна, с помощью команды:
...
Имена моделей Ollama имеют формат `name:tag`. Тег обычно указывает количество параметров или вариант квантизации. Некоторые полезные команды для управления моделями:

| Команда | Описание |
|---------|-------------|
| `ollama list` | Показать все загруженные модели |
| `ollama pull <model>` | Загрузить модель без её запуска |
| `ollama rm <model>` | Удалить модель для освобождения места на диске |
| `ollama show <model>` | Отобразить метаданные и параметры модели |

## Общение через терминал

Запустите интерактивный сеанс чата непосредственно из командной строки:

```bash
ollama run gpt-oss:20b
```

Ollama загружает модель в память и переводит вас в режим ввода запросов. Попробуйте задать ей вопрос:

```
>>> What is the capital of France and why is it historically significant?
```

Модель передаёт свой ответ токен за токеном прямо в терминале. Введите `/bye` или нажмите `Ctrl+D`, чтобы завершить сеанс.

> **Совет**: При первом запуске загрузка модели в память занимает несколько секунд. Последующие запросы в рамках того же сеанса выполняются значительно быстрее, поскольку модель остаётся загруженной.

<!-- @os:windows -->
## Общение через настольное приложение

Ollama также поставляется с настольным приложением, предоставляющим удобный интерфейс чата для взаимодействия с вашими моделями.

Откройте **Ollama** из меню «Пуск» или нажмите на значок Ollama в системном трее и выберите **Open Ollama**.

После открытия приложения:

1. Нажмите **New Chat** на боковой панели.
2. Выберите **gpt-oss:20b** из выпадающего списка моделей в правом нижнем углу области ввода чата.
...
Настольное приложение хранит историю ваших разговоров на боковой панели, что позволяет легко возвращаться к предыдущим чатам.
<!-- @os:end -->

## Использование REST API

После установки Ollama работает как фоновая служба и предоставляет REST API по адресу `http://localhost:11434`, который вы можете использовать для интеграции моделей в собственные приложения и скрипты.
...
Ответ представляет собой JSON-объект, содержащий вывод модели в поле `response`.


### Пример на Python
Теперь, когда мы можем обращаться к Ollama API программно, давайте вызовем его из Python.
...
В том же каталоге используйте VS Code или другой редактор, чтобы создать файл .py и скопировать в него следующий код. Затем запустите файл в активированном окружении с помощью `python your_file_name.py`
...
|----------|--------|---------|
| `/api/generate` | POST | Генерация текста в один ход |
| `/api/chat` | POST | Многоходовой разговор с историей сообщений |
| `/api/tags` | GET | Список доступных моделей |
| `/api/show` | POST | Отображение сведений о модели |
| `/api/pull` | POST | Загрузка модели из реестра |

Полную справку по API см. в [документации Ollama API](https://github.com/ollama/ollama/blob/main/docs/api.md).

## Следующие шаги

- **Попробуйте другие модели**: Просмотрите [библиотеку моделей Ollama](https://ollama.com/library), чтобы изучить сотни доступных моделей — от небольших помощников по написанию кода до крупных моделей рассуждения.
- **Создавайте собственные модели**: Используйте [Modelfile](https://github.com/ollama/ollama/blob/main/docs/modelfile.md), чтобы задать пользовательские системные подсказки, температуру и другие параметры для индивидуального опыта.
- **Разрабатывайте с помощью API**: Используйте клиентские библиотеки для [Python](https://github.com/ollama/ollama-python) или [JavaScript](https://github.com/ollama/ollama-js), чтобы интегрировать Ollama в ваши приложения.
- **Подключайтесь к фронтендам**: Совместите Ollama с такими инструментами, как [Open WebUI](https://github.com/open-webui/open-webui), для получения многофункционального интерфейса чата с поиском, персонажами и загрузкой документов.

Для получения дополнительной информации ознакомьтесь с [документацией Ollama](https://github.com/ollama/ollama/blob/main/README.md).