...
> [!NOTE]
> Для этого сценария требуется минимум **32 ГБ** оперативной памяти.
<!-- @device:end -->

n8n — это платформа для автоматизации рабочих процессов, которая позволяет соединять приложения и сервисы с помощью визуального редактора на основе узлов.

Этот сценарий научит вас настраивать инструмент для составления финансовых новостных сводок на базе ИИ, который собирает данные из раздела бизнес-новостей AP News, извлекает ключевые заголовки и использует локальную LLM, работающую на вашей системе, для создания сводки, ориентированной на инвесторов.
...
[Lemonade](https://lemonade-server.ai) — это платформа для локального запуска LLM, разработанная для оборудования AMD. Она предоставляет API, совместимый с OpenAI, который полностью работает на вашем компьютере — ваши данные никогда не покидают устройство.

В этом сценарии мы используем Lemonade для запуска локальной LLM, к которой подключается n8n для выполнения задач на основе ИИ.
...
<!-- @os:windows -->
> **Проблема с PATH**: Если команда `n8n --version` выдаёт сообщение о том, что команда не найдена, убедитесь, что глобальная папка bin npm добавлена в пользовательский `PATH`. Обычно путь установки находится по адресу `C:\Users\<username>\AppData\Roaming\npm`. 
> Добавьте этот путь в пользовательский PATH (Изменить системные переменные среды > Переменные среды > Изменить путь пользователя) и перезапустите терминал. 
...
> **Совет**: Не закрывайте окно терминала во время работы с n8n. Закрытие может остановить сервер.
...
<!-- @os:linux -->
Откройте графический интерфейс Lemonade, щёлкнув значок Lemonade на панели задач. Здесь вы можете просматривать модели, бэкенды и загружать предустановленные модели.
<!-- @os:end -->

<!-- @os:windows -->
Откройте графический интерфейс Lemonade, щёлкнув значок Lemonade. Щёлкните правой кнопкой мыши по значку в трее, чтобы открыть приложение. Затем вы можете добавлять модели, бэкенды и загружать предустановленные модели.
<!-- @os:end -->

>**Совет**: После запуска графический интерфейс Lemonade также доступен по адресу http://localhost:13305

Также вы можете открыть терминал и выполнить команду `lemonade list`, чтобы посмотреть, какие модели установлены. Затем выполните:
...
2. Создайте новую локальную учётную запись, указав электронную почту, или войдите, если у вас уже есть аккаунт
3. После входа вы увидите панель управления n8n

> **Совет**: Если вы потеряли доступ к своей учётной записи, попробуйте выполнить команду `n8n user-management:reset`
...
2. Нажмите **Start from Scratch**, чтобы открыть редактор рабочего процесса. Либо нажмите кнопку **+** в левом верхнем углу, а затем **Add workflow**.
3. Нажмите на меню **...** (три точки) в верхней правой панели и выберите **Import from file**
...
| **Extract News Headlines & Text** | HTML-узел, извлекающий заголовки, редакторскую подборку, главные новости и региональные новости с помощью CSS-селекторов |
| **Clean Extracted News Data** | Узел Set, объединяющий все извлечённые данные в единое текстовое поле |
...
Перед запуском рабочего процесса необходимо подключить его к вашему локальному серверу Lemonade:
...
<!-- @device:halo_box -->
> Этот рабочий процесс использует GPT-OSS-120B, которая предустановлена в Lemonade. Вы можете заменить её на другие загруженные модели в настройках узла Lemonade Chat Model.
<!-- @device:end -->
...
Нажмите на название рабочего процесса вверху и переименуйте его при желании. Рабочие процессы автоматически сохраняются по мере работы.

## Дальнейшие шаги

- **Настройте автоматизацию по расписанию**: замените Manual Trigger на **Schedule Trigger** для ежедневного запуска
- **Настройте уведомления**: добавьте узел **Discord**, **Slack** или **Email**, чтобы получать сводки
- **Попробуйте другие модели**: измените модель в узле Lemonade Chat Model, чтобы поэкспериментировать с разными LLM
- **Настройте извлечение данных**: измените CSS-селекторы узла HTML Extract, чтобы нацелиться на другие разделы новостей
...
Выполните поиск по запросам "AI", "LLM" или "automation", чтобы найти рабочие процессы, которые можно импортировать и настроить под себя.
