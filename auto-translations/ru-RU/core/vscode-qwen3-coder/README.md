...
> [!NOTE]
> Для этого руководства требуется минимум **32ГБ** оперативной памяти.
<!-- @device:end -->

## Обзор

Кодовые агенты — это мощные инструменты, которые расширяют возможности разработчиков за счёт взаимодействия с ИИ-агентами на основе больших языковых моделей (LLM). Их можно встраивать в среду разработки, например в терминал или VS Code, что обеспечивает бесшовную интеграцию в рабочий процесс разработчика.

В этом руководстве показано, как использовать Cline, VS Code и LM Studio для запуска кодового агента полностью на вашей локальной машине.

## Что вы узнаете

* Как запустить VS Code с кодовым агентом Cline для помощи в задачах разработки ПО.
* Как настроить Cline для взаимодействия с LM Studio для локального инференса кодовых агентов.
* Как использовать локальных кодовых агентов для решения реальных задач разработки ПО.
...
Мы будем использовать LM Studio для обслуживания LLM, на которой работает кодовый агент.
...
![Пустая папка в VS Code](assets/open-cline-test.png)

Теперь мы готовы отправить запрос локальному кодовому агенту.
...
После генерации программного обеспечения работа агента завершена, и вы можете запустить приложение. В данном случае агент записал данные в три файла: `index.html`, `script.js` и `styles.css`. Просто дважды щёлкнув по HTML-файлу, мы можем загрузить сгенерированный веб-сайт и взаимодействовать с ним.
...
- **Документация**: Достаточно попросить агента `Add a README`, и он сгенерирует файл `README.md`, документирующий веб-сайт.
- **Анимация**: Попросите модель `Add an animation that visually represents a large language model running on a laptop.`, чтобы добавить на сайт анимацию.

Мы рекомендуем читателю попробовать сгенерировать другие приложения с помощью этой настройки. Ниже приведены несколько интересных примеров, которые мы опробовали:

- **Ретро-игры для аркадных автоматов**: Попробуйте другие запросы. Агенту также может быть интересно создать ретро-игры на Python с использованием пакета `PyGame`, используя следующий запрос:

```code
Create a simple pong game using the PyGame python package.
```

- **Анализ данных**: Одна из областей, где кодовые агенты особенно полезны, — это написание скриптов и анализ данных. Вот запрос, демонстрирующий способность локальной модели генерировать программное обеспечение для анализа данных с визуализацией цен акций:
...
Ниже приведены дополнительные ресурсы, чтобы узнать больше о кодовых агентах, Cline и запуске рабочих нагрузок на 
...
* Блог Cline о запуске кодовых агентов локально на ИИ ПК: https://cline.bot/blog/local-models-amd