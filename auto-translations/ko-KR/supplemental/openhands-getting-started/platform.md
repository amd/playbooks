<!--
Copyright Advanced Micro Devices, Inc.

SPDX-License-Identifier: MIT
-->

<!-- auto-translated-disclaimer v1 -->
> [!WARNING]
> **기계 번역.** 이 페이지는 영어에서 자동으로 번역되었으며 사람이 검토하지 않았습니다. 오류가 포함될 수 있으며, 일부 단계, 명령어, 다운로드 또는 제품 가용성이 사용자의 언어나 지역에 따라 다를 수 있습니다. 이상한 부분이 있다면 원본 영어 플레이북을 정확한 정보의 출처로 참고하시기 바랍니다.
<!-- auto-translated-disclaimer:end -->

# 플랫폼 구성

이 문서는 이 플레이북을 실행하기 위한 예상 플랫폼 구성을 설명합니다.

## 필수 앱/프레임워크

### Windows/Linux

- **Lemonade Server**는
  [Lemonade 설치 가이드](https://lemonade-server.ai/docs/guide/install/)에 따라 설치해야 합니다.
- **Node.js 22.12 이상** 및 `npm`은 `agent-canvas` CLI에서 사용됩니다.
- **uv**는 Agent Canvas가 에이전트 서버 환경을 관리하는 데 사용하는 Python 패키지 관리자입니다.
  [uv 설치 가이드](https://docs.astral.sh/uv/getting-started/installation/)에서 설치하세요.

## 필수 모델

### Windows/Linux

플레이북을 시작하기 전에 다음 모델이 Lemonade Server에서 사용 가능해야 합니다.

| 모델 유형 | 모델 ID | 참고 사항 |
| --- | --- | --- |
| GGUF 채팅 모델 | `Qwen3.6-35B-A3B-GGUF` | Lemonade Server가 `http://127.0.0.1:13305/api/v1`에서 제공합니다. 메모리가 32GB 미만인 장치에서는 더 작은 GGUF 모델을 사용하세요. |

다음 명령어로 모델을 시작하세요:

```bash
lemonade config set llamacpp.backend=vulkan
lemonade config set ctx_size=65536
lemonade run "Qwen3.6-35B-A3B-GGUF"
```
