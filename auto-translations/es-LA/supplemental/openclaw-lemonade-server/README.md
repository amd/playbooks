...
Juntos, forman una pila de agente de IA totalmente local: Lemonade se encarga de la inferencia del modelo, y OpenClaw proporciona el bucle de agente que convierte las salidas del modelo en acciones reales.

> **Antes de continuar:** OpenClaw es un agente de IA altamente autónomo. Darle a cualquier agente de IA acceso a tu sistema puede resultar en resultados impredecibles o no deseados. Procede solo si comprendes los riesgos y te sientes cómodo con software autónomo actuando en tu nombre.

---

## Lo que aprenderás

Al final de esta guía práctica podrás:
...
El modelo recomendado para esta guía práctica es **Qwen3.6-35B-A3B-GGUF** de Unsloth, un sólido modelo MoE con una ventana de contexto de 263k tokens que es muy adecuado para cargas de trabajo de agentes. Este modelo utiliza la cuantización UD-Q4_K_XL. Descárgalo ahora:
...
El modelo tiene una longitud de contexto predeterminada de 262,144 tokens. Si encuentras errores de memoria insuficiente (OOM), considera reducir la ventana de contexto. Sin embargo, dado que Qwen3.6 aprovecha el contexto extendido para tareas complejas, recomendamos mantener una longitud de contexto de al menos 128K tokens para preservar las capacidades de razonamiento.

> **Consejo: Desactiva el modo de razonamiento para respuestas de agente más rápidas:** Qwen3.6-35B-A3B se ejecuta en modo de razonamiento por defecto, lo que agrega latencia antes de cada respuesta. Para los bucles de agente, esta sobrecarga se acumula rápidamente. El repositorio [lemonade-sdk/recipes](https://github.com/lemonade-sdk/recipes/blob/main/coding-agents/Qwen3.6-35B-A3B-NoThinking.json) proporciona una configuración lista para usar que desactiva el modo de razonamiento. Para usarla, descarga el archivo e impórtalo:
>
> ```bash
...
Ejecutamos OpenClaw dentro de WSL (recomendado) y lo conectamos a Lemonade ejecutándose de forma nativa en Windows. Esto te brinda un entorno de shell de Linux para OpenClaw mientras mantienes la aceleración de GPU de Lemonade en el lado de Windows.
...
WSL2 se ejecuta en una red virtual. Lemonade en Windows se vincula a `127.0.0.1`, al cual WSL no puede acceder directamente. Un proxy de puerto de Windows reenvía el tráfico desde la IP de la puerta de enlace de WSL hacia el localhost de Windows.
...
**Agrega el proxy de puerto** (ejecuta en PowerShell como Administrador, reemplazando `<WSL-Gateway-IP>` con la IP de la puerta de enlace de tu WSL):
...
> La regla `netsh portproxy` sobrevive a los reinicios, pero la IP de la puerta de enlace de WSL puede cambiar después de ejecutar `wsl --shutdown`. Si Lemonade se vuelve inaccesible desde WSL después de un reinicio, obtén la IP de puerta de enlace actualizada y actualiza el proxy con esta nueva IP.
...
El indicador `--no-onboard` omite el asistente de configuración interactivo; configurarás el backend del modelo manualmente en el siguiente paso, lo que te da un control preciso sobre qué modelo y servidor se utilizan.
...
> ```
>
> `reserveTokensFloor` es un *piso* (una protección mínima), no la reserva en sí; configurar solo el piso no tiene ningún efecto. `reserveTokensFloor: 0` desactiva la protección para que se acepte el valor más bajo de `reserveTokens`.
>
> **Cuándo aplicar esto:** Usa esta configuración si la ventana de contexto efectiva de tu modelo está por debajo de ~37k, ya sea porque el modelo es pequeño (por ejemplo, 8k, 16k, 32k) o porque intencionalmente lo has limitado a un valor menor (por ejemplo, cargar un modelo de 128k pero configurar el contexto en 16k en Lemonade). Sin esto, OpenClaw entrará en un bucle infinito de compactación al iniciar.
>
> **Modelos de contexto grande con contexto completo:** Puedes omitir esto por completo. Los valores predeterminados funcionan bien, la compactación se activará mucho antes de que se llene la ventana y el modelo tendrá amplio espacio para generar respuestas largas. Si de todas formas lo aplicas, ten en cuenta que `reserveTokens: 4096` limita la longitud de la respuesta a ~4k tokens, lo que puede cortar la generación de archivos extensos o planes detallados.
>
...
### (Recomendado) Habilita el sandboxing de Docker

OpenClaw puede enrutar todas las operaciones de archivos y código del agente a través de un contenedor Docker aislado en lugar de ejecutarlas directamente en tu host. Esto limita el alcance de cualquier acción no deseada al entorno aislado, dejando intactos el sistema de archivos y la red de tu host.
...
Los contenedores del sandbox **no tienen acceso a la red** de forma predeterminada. Consulta la [referencia de sandboxing](https://docs.openclaw.ai/gateway/sandboxing) para conocer las opciones de montajes vinculados (bind mounts) y anulaciones de red.
...
[Firecrawl](https://docs.firecrawl.dev/introduction) proporciona un servicio autoalojado de rastreo web y extracción de contenido que puede evitar estos obstáculos y desbloquear todo el potencial de la automatización con OpenClaw.
...
### 1. Registra el servicio del sistema
...
[SELF-HOST Firecrawl](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md) es ideal para quienes necesitan control total sobre sus entornos de scraping y procesamiento de datos, aunque implica el costo adicional de mantenimiento y configuración.

Comienza clonando el repositorio:
```bash
git clone https://github.com/firecrawl/firecrawl.git
```
Crea el archivo `.env` en el directorio raíz `/firecrawl`: 
```bash
# ===== Required ENVS ======
PORT=3002
HOST=0.0.0.0
...
> Siempre puedes ampliar el stack agregando servicios adicionales de Firecrawl según sea necesario. La lista completa de servicios disponibles se encuentra en el [docker-compose.yaml](https://github.com/firecrawl/firecrawl/blob/main/docker-compose.yaml) oficial de Firecrawl.
...
### Cómo obtener tu `OPENCLAW_GATEWAY_TOKEN`

Una vez que el servicio esté en funcionamiento, notarás que se ha creado un nuevo directorio `.openclaw` en tu carpeta de inicio (~/.openclaw). Este directorio está bloqueado por defecto, por lo que deberás desbloquearlo para obtener tu token de gateway.
...
3. Abre el panel del gateway en tu navegador en http://127.0.0.1:18789. Pega tu token cuando se te solicite para autenticarte.
...
Para abrir el panel, ejecuta esto en una segunda terminal mientras el gateway sigue en ejecución:

```bash
openclaw dashboard
```

Debido a que el gateway se enlaza a loopback, el panel se autentica automáticamente cuando se abre desde la misma máquina; no se necesita ingresar un token ni aprobar el dispositivo para el acceso local. Deberías ver el panel de OpenClaw con tu modelo de Lemonade listado como el backend activo.

> Si has habilitado el sandboxing, puedes verificarlo pidiéndole al agente que ejecute `run hostname` desde el panel. Si ves un ID corto de contenedor en lugar del nombre de host de tu máquina, el sandbox está funcionando.

**Felicitaciones, has construido un stack de agente de IA completamente local desde cero.**

> **¿Necesitas el token del gateway?** Ejecuta `openclaw dashboard --no-open` para imprimir la URL del panel con el token incrustado (también intenta copiarlo a tu portapapeles). Alternativamente, el token se encuentra en `gateway.auth.token` dentro de `~/.openclaw/openclaw.json`.
>
> **Aprobar un dispositivo remoto:** Cuando abres el panel desde una segunda máquina o teléfono, el navegador muestra un ID de solicitud. De vuelta en la máquina que ejecuta el gateway, ejecuta:
> ```bash
...
Si no tienes una cuenta de Discord, regístrate en [discord.com](https://discord.com). También necesitas un servidor donde seas administrador; crea uno haciendo clic en el ícono **+** en la barra lateral de Discord y seleccionando **Crear mi propio servidor**. Un servidor privado está bien.
...
Habilita el Modo de desarrollador en Discord (**Configuración de usuario/ Avanzado/ Modo de desarrollador**), luego:
- Haz clic derecho en el ícono de tu servidor: **Copiar ID de servidor**
...
Guarda el token de tu bot como una variable de entorno, luego crea un único archivo de parche que habilite Discord, haga referencia al token y agregue tu servidor a la lista de permitidos. Reemplaza `<server_id>` y `<user_id>` con los IDs recolectados anteriormente.
...
> **No dependas de pedirle al agente que configure esto.** Cuando el sandboxing está habilitado, el agente no puede escribir en `~/.openclaw/openclaw.json` desde dentro del sandbox; usa los comandos de la CLI mencionados arriba en el host en su lugar.

Reinicia la puerta de enlace (gateway) para que tome la nueva configuración de canal:

```bash
openclaw gateway run --bind loopback --port 18789
```

Deberías ver `logged in to discord as <bot-name>` en la salida de la puerta de enlace en unos segundos.
...
Agrega la configuración del canal a `~/.openclaw/openclaw.json` (o parchéala desde el panel de control):
...
Reinicia la puerta de enlace, luego envíale a tu bot cualquier mensaje en Telegram. Aprueba el emparejamiento:
...
1. **Resumidor del mercado bursátil**: Programa OpenClaw para obtener datos de APIs financieras a intervalos fijos, resumir los movimientos del día con tu modelo local y enviar un resumen a tu teléfono cada mañana a través del canal que elijas.

2. **Monitor de ajuste fino (fine-tuning)**: Inicia un trabajo de entrenamiento de forma remota a través de Telegram o Discord, luego haz que el agente siga el registro de entrenamiento y reporte valores de pérdida periódicos, uso de GPU y uso de disco de vuelta a tu teléfono. Si la ejecución se estanca o el VRAM tiene un pico, te enteras de inmediato sin necesidad de estar frente a la máquina.
