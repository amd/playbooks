...
<!-- @github-only -->
> [!IMPORTANT]
> Este playbook usa etiquetas especiales que GitHub no puede renderizar. Visita [amd.com/playbooks](https://amd.com/playbooks) para obtener una vista previa correcta de este contenido.
<!-- @github-only:end -->
...
Tu Ryzen™ AI Halo ya es capaz de ejecutar modelos de lenguaje grandes de forma local. La agrupación en clúster lleva esto un paso más allá al combinar la memoria de GPU de varios sistemas a través de una red local, dándote acceso a modelos aún más grandes con razonamiento más sólido, mejor generación de código y una comprensión multilingüe más profunda, todo completamente en tu propio hardware.
...
- Cómo iniciar vLLM con soporte para ROCm
...
| Switch Ethernet de 10Gbps | 1 | Switch central que permite la comunicación multi-nodo entre las unidades Ryzen AI Halo (al menos 2 puertos) |
...
En cada máquina, busca el nombre de su interfaz de red y anótalo (se hará referencia a él en el resto de las instrucciones como `IFNAME`). Ejecuta:
...
> **Nota**: Si la velocidad es menor a `10000Mb/s` o el enlace no se activa, revisa la conexión del cable y confirma que el puerto del switch esté configurado en 10Gbps. Algunos switches requieren que se desactive la autonegociación y que la velocidad del enlace se configure manualmente; consulta la documentación de tu switch.
...
En Linux, ROCm utiliza un grupo de memoria del sistema compartido, y este grupo está configurado de forma predeterminada en la mitad de la memoria del sistema.

Esta cantidad se puede aumentar cambiando la configuración de páginas del Translation Table Manager (TTM) del kernel, siguiendo las instrucciones a continuación. AMD recomienda configurar la VRAM dedicada mínima en el BIOS (0.5 GB).

* Instala la utilidad pipx y agrega la ruta de los wheels instalados con pipx a la ruta de búsqueda del sistema.
...
## Inicialización del contenedor de vLLM
...
Cuando pongas en servicio el modelo Qwen3.5-397B en este playbook, vLLM descargará automáticamente los pesos del modelo en tu sistema. Para asegurarte de que esos pesos sean accesibles desde dentro del contenedor, primero crea un directorio de modelos que el contenedor pueda montar:

```bash
mkdir -p ~/.local/share/vLLM/models
```

### 2. Iniciar el contenedor de vLLM

El comando a continuación inicia el contenedor y te ubica en un shell interactivo. Monta el directorio de modelos que acabas de crear y pasa tu `IFNAME` a `NCCL_SOCKET_IFNAME` y `GLOO_SOCKET_IFNAME`, indicándole a RCCL (la biblioteca que vLLM usa para coordinar las GPU en todo el clúster) qué interfaz usar.
...
vLLM usa Ray para orquestar el clúster y RCCL para gestionar la comunicación de GPU a GPU entre nodos. Una máquina actúa como el **nodo principal** (Máquina 1), coordinando la inferencia. La otra se une como **nodo de trabajo** (Máquina 2), aportando su memoria de GPU y capacidad de cómputo.

> **Nota**: Ray es una dependencia opcional para vLLM y solo está disponible desde dentro del contenedor Podman preconfigurado.

Al iniciarse, vLLM fragmenta el modelo entre ambos nodos usando paralelismo tensorial. Una vez cargado, la inferencia se realiza como si se ejecutara en un solo acelerador.
...
> **Buscar `<MACHINE_2_IP>`**: En la Máquina 2, ejecuta `hostname -I | awk '{print $1}'` para encontrar su dirección IP local.
...
| `--tensor-parallel-size` | Número de GPU entre las que se fragmenta el modelo (establecer en el total de GPU del clúster) |
...
| `--reasoning-parser` | Habilita el análisis de salida de razonamiento estructurado para el modelo |
...
> **Buscar `<MACHINE_1_IP>`**: En la Máquina 1, ejecuta `hostname -I | awk '{print $1}'` para encontrar su dirección IP local. Si accedes a Open WebUI desde la propia Máquina 1, puedes usar `http://localhost:7000/v1`.

![Configuración de conexión de Open WebUI para el endpoint de vLLM](assets/openwebui-connection.png)

Una vez conectado, selecciona el modelo en el menú desplegable de modelos de Open WebUI y comienza a chatear. El modelo ahora se está ejecutando en ambos nodos de tu Ryzen AI Halo:
...
- **Escala a cuatro nodos**: Agrega dos sistemas Ryzen AI Halo adicionales como workers de Ray adicionales para fragmentar modelos entre aún más GPU. Esto requiere un switch Ethernet con al menos cuatro puertos, uno para cada nodo. Sigue el [Paso 2: Unirse al clúster](#step-2-join-the-cluster-machine-2) en cada worker adicional y aumenta `--tensor-parallel-size` según corresponda
