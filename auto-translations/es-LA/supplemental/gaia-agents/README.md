...
Los agentes GAIA son asistentes de IA que utilizan un LLM local para razonar y llamar a herramientas que tú defines — como chatbots que pueden tomar acción. Se ejecutan **100% localmente** sin APIs en la nube, sin que los datos salgan de tu máquina y sin necesidad de claves de API.

En este playbook, crearás un Agente Asesor de Hardware que detecta la RAM, GPU y NPU de tu sistema, consulta el catálogo de modelos local y recomienda qué LLMs puede ejecutar tu máquina. Es una introducción práctica al SDK de Agentes GAIA que produce algo inmediatamente útil.

## Lo que aprenderás

- Cómo crear un agente GAIA con herramientas personalizadas
- Usar el SDK LemonadeClient para consultar información del sistema y catálogos de modelos
- Detección de GPU/NPU específica por plataforma (Windows PowerShell y Linux lspci)
...
> **Nota**: Si VS Code no está instalado, puedes instalarlo con el Centro de Desarrolladores de Ryzen AI.
...
Primero pon en marcha el agente terminado para que puedas ver qué estás construyendo. Luego, recorreremos el código paso a paso.

### Ejecutar el ejemplo preconstruido

Este playbook incluye el archivo completo [hardware_advisor_agent.py](assets/hardware_advisor_agent.py). Descárgalo en un directorio de tu elección y ejecútalo para ver el agente terminado en acción:
...
**Prueba preguntando:** "¿Qué tamaño de LLM puedo ejecutar?"
...
**¡Felicitaciones!** — ¡has construido un agente!
...
- **Detección específica por plataforma** — Windows PowerShell / Linux lspci para información de GPU
- **Cálculos de memoria** — Regla del 70% para dimensionamiento seguro de modelos

Los datos fluyen a través de estos en secuencia: consulta del usuario → el agente selecciona una herramienta → la herramienta llama a LemonadeClient + detección del SO → el agente sintetiza los resultados en una recomendación.

### SDK LemonadeClient

El LemonadeClient proporciona una API unificada para detección del sistema, disponibilidad de NPU/GPU y consultas al catálogo de modelos.
...
**`get_system_info()`** — Devuelve el SO, CPU, RAM y disponibilidad de dispositivos:
...
**`get_model_info(model_id)`** — Devuelve estimaciones de tamaño para un modelo específico:
...
### Detección de GPU específica por plataforma

El agente utiliza comandos nativos del SO en lugar de PyTorch para la detección de GPU. Esto funciona sin controladores de GPU instalados, detecta todas las GPUs (no solo las compatibles con CUDA) y evita importaciones de bibliotecas pesadas.
...
> **Regla:** El tamaño del modelo debe ser menor al 70% de la RAM disponible para dejar un 30% de margen para las operaciones de inferencia (caché KV, búferes de procesamiento por lotes, picos de memoria en tiempo de ejecución).
...
Crearás **un archivo** llamado `hardware_advisor_agent.py` y agregarás funciones progresivamente. Cada paso se basa en el anterior.

### Paso 1: Esqueleto del agente

Comienza con una estructura mínima del agente — solo la clase y un prompt de sistema básico. El agente aún no tiene herramientas.
...
Ejecuta y prueba preguntando "Muéstrame las especificaciones de mi sistema":
...
Ejecuta y prueba preguntando "¿Qué modelos están disponibles?":
...
Ejecuta y prueba preguntando "¿Qué tamaño de LLM puedo ejecutar?":
...
Reemplaza el bloque `__main__` simple con una CLI interactiva pulida. Esto agrega un banner, comandos para salir y mejor manejo de errores.
...
- [x] Importaciones: `from typing import Any, Dict` y `from gaia import Agent, tool`
- [x] Clase `HardwareAdvisorAgent` con `__init__` y prompt de sistema
- [x] Auxiliar `_get_gpu_info()` (Windows PowerShell + Linux lspci)
- [x] Herramienta `get_hardware_info()` con campos de GPU, NPU y SO
- [x] Herramienta `list_available_models()` con etiquetas y enriquecimiento de tamaño
- [x] Herramienta `recommend_models()` con regla del 70%, fits_in_ram, fits_in_gpu
...
- **Explorar las APIs de LemonadeClient** — Descubre más capacidades de gestión de sistemas y modelos en la [documentación del SDK LemonadeClient](https://amd-gaia.ai/sdk/lemonade-client)
- **Agregar interacción por voz** — Integra Whisper ASR y Kokoro TTS para que los usuarios puedan hacer preguntas sobre hardware hablando. Consulta la [guía de Talk](https://amd-gaia.ai/guides/talk)
- **Agregar soporte MCP** — Expón el asesor de hardware como un servidor MCP para que otras herramientas puedan consultarlo. Consulta la [guía de MCP](https://amd-gaia.ai/sdk/infrastructure/mcp)
- **Ampliar el motor de recomendaciones** — Considera la VRAM de la GPU para descargar capas, o agrega benchmarking para estimar tokens por segundo
- **Construir un sistema multiagente** — Combina el asesor de hardware con un agente de código o un agente de chat usando el [Agente de Enrutamiento](https://amd-gaia.ai/guides/routing)