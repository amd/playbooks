...
[Ryzen AI CVML Library](https://ryzenai.docs.amd.com/en/latest/ryzen_ai_libraries.html#ryzen-ai-cvml-library) — це набір інструментів AMD для комп'ютерного зору та машинного навчання на C++, що забезпечує потужні можливості сприйняття на пристрої — включаючи оцінку глибини, виявлення облич та відстеження сітки обличчя. Побудована на основі драйверів Ryzen AI, бібліотека автоматично вибирає найкраще доступне апаратне забезпечення (GPU або NPU) для виведення, дозволяючи додавати функції штучного інтелекту до C++ застосунків без необхідності займатися навчанням моделей або інтеграцією фреймворків. Вся обробка відбувається локально на вашій системі, що робить її ідеальною для застосунків, чутливих до конфіденційності та з низькою затримкою.

Цей посібник навчить вас, як налаштувати Ryzen AI CVML Library, зібрати включені зразки застосунків та запустити виявлення облич на зразковому зображенні.

## Що ви дізнаєтесь

- Як встановити передумови та налаштувати Ryzen AI CVML Library у вашій системі
- Як працює CVML C++ API: контексти, об'єкти функцій та буфери зображень
- Як зібрати та запустити включені зразки застосунків за допомогою CMake та OpenCV
- Як запустити виявлення облич на зображенні з обмежувальними рамками та орієнтирами
- Як інтегрувати функції CVML у власні C++ застосунки

<!-- @device:halo_box -->
## Перевірте наявність оновлень програмного забезпечення
...
<!-- @os:windows -->
- [OpenCV 4.11](https://github.com/opencv/opencv/releases/tag/4.11.0) — завантажте `opencv-4.11.0-windows.exe`, запустіть його та розпакуйте до локальної папки (наприклад, `C:\opencv`)
- [CMake](https://cmake.org/download/) — завантажте інсталятор MSI для Windows x86-64 та під час встановлення виберіть **"Add CMake to the system PATH for all users"**
- [Ryzen AI NPU driver](https://ryzenai.docs.amd.com/en/latest/inst.html) — встановіть останню доступну версію
- [Visual Studio 2022 Community](https://aka.ms/vs/17/release/vs_community.exe) з робочим навантаженням "Desktop development with C++" (включає компілятор MSVC, Windows SDK та інструменти збірки C++)
<!-- @os:end -->

<!-- @os:linux -->
- OpenCV 4.11 — необхідно зібрати з вихідного коду (пакети apt для Ubuntu 22.04 та 24.04 не надають версію 4.11). Дивіться розділ [Збірка OpenCV з вихідного коду](#building-opencv-from-source) нижче.
- CMake — встановіть через apt:
  ```bash
  sudo apt install cmake
  ```
- Ubuntu 22.04 або 24.04 (ядро >= 6.11.0-21-generic)
- [Ryzen AI NPU driver](https://ryzenai.docs.amd.com/en/latest/linux.html#install-npu-drivers) (інсталятор для Linux — необхідний для виведення на NPU)
...
Створіть обліковий запис AMD на [account.amd.com](https://account.amd.com), якщо у вас його немає, потім увійдіть, щоб завантажити Ryzen AI CVML Library за посиланням на портал нижче:

```
https://account.amd.com/en/forms/downloads/xef.html?filename=72293_Ryzen_AI_Library_26.05.20.zip
```

Після завантаження розпакуйте пакет до локального каталогу (наприклад, `C:\RyzenAI-Library` у Windows або `~/RyzenAI-Library` у Linux) та встановіть змінну середовища `AMD_CVML_SDK_ROOT` на місце розпакування:
...
Пакет бібліотеки містить таку структуру:

| Папка | Вміст |
|--------|----------|
| `cmake/` | Інформація про пакування для функції `find_package` CMake |
| `include/` | Заголовні файли C++ (`cvml-depth-estimation.h`, `cvml-face-detector.h`, `cvml-face-mesh.h` тощо) |
| `windows/` | Бінарні файли для Windows (файли `.LIB` для компіляції та файли `.DLL`/`.GRAPHLIB`/`.AMODEL` для виконання) |
...
Завантажте, налаштуйте та зберіть OpenCV 4.11.0 з модулями contrib (довідка: [Посібник зі встановлення OpenCV для Linux](https://docs.opencv.org/4.11.0/d7/d9f/tutorial_linux_install.html#tutorial_linux_install_quick_build_contrib)):
...
CVML Library надає простий C++ API, де кожна функція сприйняття (оцінка глибини, виявлення облич, сітка обличчя) має власний заголовний файл та об'єкт функції. Ви не працюєте з необробленими моделями — бібліотека автоматично обробляє завантаження моделей, попередню обробку та виведення.

### Доступні функції

| Функція | Заголовний файл | Опис |
|---------|------------|-------------|
| **Оцінка глибини** | `cvml-depth-estimation.h` | Генерує карти глибини для кожного пікселя з RGB-зображень |
| **Виявлення облич** | `cvml-face-detector.h` | Виявляє обличчя з обмежувальними рамками, орієнтирами (очі, ніс, рот) та оцінками впевненості |
| **Сітка обличчя** | `cvml-face-mesh.h` | Відстежує детальну геометрію обличчя з щільними точками сітки |

### Модель програмування

Кожен застосунок CVML слідує одному й тому ж чотириетапному шаблону:

1. **Створіть контекст** — `amd::cvml::Context` керує спільними ресурсами, такими як журналювання та вибір бекенду виведення.
2. **Створіть об'єкт функції** — Створіть екземпляр конкретної функції (наприклад, `amd::cvml::DepthEstimation`) на основі контексту.
3. **Обгорніть вхідні дані** — Використовуйте `amd::cvml::Image` для інкапсуляції вашого буфера RGB-зображення без копіювання даних.
4. **Виконайте** — Викличте метод обробки функції та зчитайте результати.
...
Бібліотека автоматично вибирає найкраще апаратне забезпечення (GPU або NPU) для кожної операції. Ви також можете явно встановити бекенд:
...
> **Примітка:** Функції, що використовують бекенд ONNX для операцій NPU, можуть мати більшу затримку запуску під час першого запуску. Наступні запуски будуть швидшими.

> **Примітка:** Якщо драйвер NPU не встановлено в цільовій системі, бібліотека Ryzen AI CVML автоматично перейде на бекенд GPU для операцій виведення.

## Збірка зразків застосунків

CVML Library включає готові до збірки зразки застосунків для кожної функції. Давайте зберемо їх усі одразу.
...
2. Зберіть зразки за допомогою CMake:
   ...
   Після успішної збірки виконувані файли знаходяться в:
...
3. Перед запуском будь-якого зразка переконайтеся, що файли середовища виконання CVML доступні:
...
Зразок виявлення облич виявляє обличчя на зображенні, у відео або в прямому ефірі з камери. Він малює обмежувальні рамки, оцінки впевненості та п'ять орієнтирів обличчя (два ока, ніс та два краї рота) на кожному виявленому обличчі.
...
Потім завантажте зразкове зображення для використання як вхідних даних (фото від [Jopwell](https://www.pexels.com/photo/man-in-gray-crew-neck-shirt-smiling-on-focus-photo-895863/), безкоштовне для використання через Pexels):

```bash
curl -L -o sample_face.jpg "https://images.pexels.com/photos/895863/pexels-photo-895863.jpeg?cs=srgb&dl=pexels-jopwell-895863.jpg&fm=jpg"
```

**Запустіть виявлення облич на зразковому зображенні:**
...
З'явиться вікно із зображенням, на якому навколо виявлених облич намальовані обмежувальні рамки, оцінки впевненості та точки орієнтирів обличчя (очі, ніс, краї рота).
...
**Використовуйте точну модель** для вищої точності (за рахунок швидкості):
...
| Модель | Швидкість | Точність | Найкраще для |
|-------|-------|----------|----------|
| `fast` (за замовчуванням) | Вищий FPS | Добра | Застосунки з камерою в реальному часі |
| `precise` | Нижчий FPS | Найкраща | Аналіз фотографій, потреби у високій точності |
...
Щоб використовувати CVML Library у власному C++ проєкті, додайте її через `find_package` CMake:
...
Де `AMD_CVML_SDK_ROOT` вказує на кореневу папку Ryzen AI CVML Library. Потім включіть відповідний заголовний файл для потрібної функції:
...
Для кожного зразка нижче спочатку перейдіть до папки з його виконуваним файлом, дотримуючись того ж шаблону, що й у розділі [Запуск виявлення облич](#running-face-detection) вище (наприклад, `cd build\cvml-sample-depth-estimation\Release` у Windows або `cd build/cvml-sample-depth-estimation` у Linux). У Windows додайте `.exe` до кожної команди (наприклад, `cvml-sample-depth-estimation.exe`).

- **Спробуйте оцінку глибини**: Запустіть `cvml-sample-depth-estimation -i sample_face.jpg`, щоб згенерувати кольорову карту глибини — ближчі об'єкти відображаються теплими кольорами, далекі — холодними
- **Дослідіть сітку обличчя**: Запустіть `cvml-sample-face-mesh -i sample_face.jpg`, щоб побачити детальне відстеження геометрії обличчя з докладними точками сітки
- **Обробляйте відеофайли**: Використовуйте прапорці `-i` та `-o` у будь-якому зразку для обробки відео (наприклад, `cvml-sample-face-detection -i video.mp4 -o output.mp4`)
- **Порівняйте варіанти моделей**: Спробуйте `-m precise` проти стандартного `-m fast` у виявленні облич, щоб на власні очі побачити компроміс між точністю та швидкістю
- **Створіть власний застосунок**: Використовуйте інтеграцію CMake та C++ API, щоб додати функції CVML до власних C++ застосунків
- **Комбінуйте функції**: Поєднайте виявлення облич з оцінкою глибини в одному застосунку для більш глибокого розуміння сцени
- **Перегляньте вихідний код**: Прочитайте [Ryzen AI CVML Library на GitHub](https://github.com/amd/RyzenAI-SW/tree/main/Ryzen-AI-CVML-Library) для документації заголовних файлів, додаткових зразків та деталей API