...
Ollama — це популярний легковісний інструмент для локального запуску великих мовних моделей. Він керує завантаженням моделей, квантуванням та обслуговуванням через простий інтерфейс командного рядка та настільний застосунок, тож ви можете за лічені хвилини перейти від нуля до спілкування з LLM.

Цей посібник проведе вас через встановлення Ollama, завантаження моделі GPT-OSS 20B та спілкування з нею як через термінал, так і через настільний застосунок.

## Що ви дізнаєтесь

- Як встановити та запустити Ollama у вашій системі
- Завантажити та запустити модель GPT-OSS 20B локально
- Спілкуватися з моделями за допомогою CLI
- Програмно надсилати запити до моделей через REST API
...
1. Завантажте інсталятор з [ollama.com/download](https://ollama.com/download).
2. Запустіть `.exe` інсталятор і дотримуйтесь підказок.
3. Після встановлення Ollama працює як фонова служба та доступна з терміналу, настільного застосунку та системного трею.

Перевірте встановлення, відкривши термінал і виконавши:
...
Ви маєте побачити номер встановленої версії, виведений у консоль.
<!-- @os:end -->
...
## Завантаження вашої першої моделі

Ollama керує моделями через реєстр, подібний до образів контейнерів. Щоб завантажити GPT-OSS 20B:

```bash
ollama pull gpt-oss:20b
```

Це завантажить ваги моделі на ваш локальний комп'ютер (приблизно 12 ГБ). Завантаження відбувається лише один раз, а подальші запуски завантажують модель з диска.

Ви можете перевірити доступність моделі за допомогою:

```bash
ollama list
```

Ви маєте побачити `gpt-oss:20b` у виведенні разом з її розміром та датою останньої зміни.
...
Назви моделей Ollama мають формат `name:tag`. Тег зазвичай вказує на кількість параметрів або варіант квантування. Ось декілька корисних команд для керування моделями:

| Команда | Опис |
|---------|-------------|
| `ollama list` | Показати всі завантажені моделі |
| `ollama pull <model>` | Завантажити модель без запуску |
| `ollama rm <model>` | Видалити модель для звільнення місця на диску |
| `ollama show <model>` | Показати метадані та параметри моделі |

## Спілкування через термінал

Запустіть інтерактивну сесію чату безпосередньо з командного рядка:

```bash
ollama run gpt-oss:20b
```

Ollama завантажить модель у пам'ять і відкриє запрошення. Спробуйте запитати щось:

```
>>> What is the capital of France and why is it historically significant?
```

Модель транслює свою відповідь токен за токеном безпосередньо в терміналі. Введіть `/bye` або натисніть `Ctrl+D`, щоб вийти з сесії.

> **Порада**: Перший запуск займає кілька секунд для завантаження моделі в пам'ять. Подальші запити в межах тієї ж сесії обробляються значно швидше, оскільки модель залишається завантаженою.

<!-- @os:windows -->
## Спілкування через настільний застосунок

Ollama також постачається з настільним застосунком, який надає зручний інтерфейс чату для взаємодії з вашими моделями.

Відкрийте **Ollama** з меню Пуск або натисніть на іконку Ollama в системному треї та виберіть **Open Ollama**.

Коли застосунок відкрито:

1. Натисніть **New Chat** на боковій панелі.
2. Виберіть **gpt-oss:20b** зі спадного списку моделей у нижньому правому куті області введення чату.
3. Введіть повідомлення та натисніть Enter, щоб почати спілкування.

<p align="center">
  <img src="assets/ollama_app.png" alt="Ollama desktop app chatting with gpt-oss:20b" width="600"/>
</p>

Настільний застосунок зберігає історію ваших розмов на боковій панелі, що дозволяє легко повертатися до попередніх чатів.
<!-- @os:end -->

## Використання REST API

Після встановлення Ollama працює як фонова служба та надає REST API за адресою `http://localhost:11434`, який ви можете використовувати для інтеграції моделей у власні застосунки та скрипти.
...
### Генерація відповіді в терміналі
...
Відповідь являє собою JSON-об'єкт, що містить вивід моделі в полі `response`.


### Приклад на Python
Тепер, коли ми можемо звертатися до API Ollama програмно, викличемо його з Python.

#### Створення віртуального середовища в терміналі
...
У тій самій директорії скористайтеся VS Code або іншим редактором, щоб створити файл .py, і скопіюйте в нього наступний код. Потім запустіть файл у вашому активованому середовищі за допомогою `python your_file_name.py`
...
|----------|--------|---------|
| `/api/generate` | POST | Генерація тексту в один хід |
| `/api/chat` | POST | Багатоходова розмова з історією повідомлень |
| `/api/tags` | GET | Список доступних моделей |
| `/api/show` | POST | Показати деталі моделі |
| `/api/pull` | POST | Завантажити модель з реєстру |

Повну довідку з API дивіться в [документації Ollama API](https://github.com/ollama/ollama/blob/main/docs/api.md).
## Наступні кроки

- **Спробуйте різні моделі**: перегляньте [бібліотеку моделей Ollama](https://ollama.com/library), щоб дослідити сотні доступних моделей — від невеликих помічників з кодування до великих моделей для складних міркувань.
- **Створюйте власні моделі**: використовуйте [Modelfile](https://github.com/ollama/ollama/blob/main/docs/modelfile.md), щоб налаштувати власні системні підказки, температуру та інші параметри для індивідуального досвіду.
- **Розробляйте з використанням API**: використовуйте клієнтські бібліотеки [Python](https://github.com/ollama/ollama-python) або [JavaScript](https://github.com/ollama/ollama-js), щоб інтегрувати Ollama у свої застосунки.
- **Підключайтеся до фронтендів**: поєднуйте Ollama з такими інструментами, як [Open WebUI](https://github.com/open-webui/open-webui), для отримання багатофункціонального інтерфейсу чату з пошуком, персонажами та завантаженням документів.

Для отримання додаткової інформації перегляньте [документацію Ollama](https://github.com/ollama/ollama/blob/main/README.md).