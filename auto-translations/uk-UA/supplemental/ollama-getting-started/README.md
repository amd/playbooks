...
Ollama — це популярний легкий інструмент для локального запуску великих мовних моделей. Він забезпечує завантаження моделей, квантизацію та обслуговування через простий інтерфейс командного рядка та настільний застосунок, тому ви можете перейти від нуля до спілкування з LLM за лічені хвилини.
...
- Надсилати запити до моделей програмно через REST API
...
2. Запустіть інсталятор `.exe` та дотримуйтесь інструкцій.
3. Після встановлення Ollama працює як фонова служба та доступна з термінала, настільного застосунку та системного трею.
...
Ви повинні побачити номер встановленої версії, виведений у консоль.
<!-- @os:end -->
...
Ollama керує моделями через реєстр, подібний до реєстру контейнерних образів. Щоб завантажити GPT-OSS 20B:

```bash
ollama pull gpt-oss:20b
```

Це завантажує ваги моделі на ваш локальний комп'ютер (приблизно 12 ГБ). Завантаження відбувається лише один раз, а наступні запуски завантажують модель з диска.

Ви можете підтвердити доступність моделі за допомогою:

```bash
ollama list
```

Ви повинні побачити `gpt-oss:20b` у виводі разом із її розміром та датою останньої зміни.
...
Назви моделей Ollama мають формат `name:tag`. Тег зазвичай вказує на кількість параметрів або варіант квантизації. Деякі корисні команди для керування моделями:

| Команда | Опис |
|---------|-------------|
| `ollama list` | Показати всі завантажені моделі |
| `ollama pull <model>` | Завантажити модель без її запуску |
| `ollama rm <model>` | Видалити модель для звільнення місця на диску |
| `ollama show <model>` | Відобразити метадані та параметри моделі |

## Спілкування через термінал

Запустіть інтерактивний сеанс чату безпосередньо з командного рядка:

```bash
ollama run gpt-oss:20b
```

Ollama завантажує модель у пам'ять і переводить вас до запрошення. Спробуйте щось запитати:

```
>>> What is the capital of France and why is it historically significant?
```

Модель передає свою відповідь токен за токеном безпосередньо в терміналі. Введіть `/bye` або натисніть `Ctrl+D`, щоб завершити сеанс.

> **Порада**: Перший запуск займає кілька секунд для завантаження моделі в пам'ять. Наступні запити в межах того самого сеансу відповідають значно швидше, оскільки модель залишається завантаженою.
...
Відкрийте **Ollama** з меню «Пуск» або клацніть значок Ollama в системному треї та виберіть **Open Ollama**.

Після відкриття застосунку:

1. Натисніть **New Chat** на бічній панелі.
2. Виберіть **gpt-oss:20b** зі спадного меню моделей у нижньому правому куті області введення чату.
3. Введіть повідомлення та натисніть Enter, щоб розпочати спілкування.

<p align="center">
  <img src="assets/ollama_app.png" alt="Ollama desktop app chatting with gpt-oss:20b" width="600"/>
</p>

Настільний застосунок зберігає історію ваших розмов на бічній панелі, що дозволяє легко повертатися до попередніх чатів.
<!-- @os:end -->

## Використання REST API

Після встановлення Ollama працює як фонова служба та надає REST API на `http://localhost:11434`, який ви можете використовувати для інтеграції моделей у власні застосунки та скрипти.
...
Відповідь є JSON-об'єктом, що містить вивід моделі в полі `response`.


### Приклад на Python
Тепер, коли ми можемо програмно звертатися до Ollama API, давайте викличемо його з Python.
...
У тому самому каталозі використайте VS Code або інший редактор, щоб створити файл .py та скопіюйте в нього наведений нижче код. Потім запустіть файл у вашому активованому середовищі за допомогою `python your_file_name.py`
...
| `/api/show` | POST | Відображення деталей моделі |
| `/api/pull` | POST | Завантаження моделі з реєстру |

Повну довідку по API дивіться в [документації Ollama API](https://github.com/ollama/ollama/blob/main/docs/api.md).

## Наступні кроки

- **Спробуйте різні моделі**: Перегляньте [бібліотеку моделей Ollama](https://ollama.com/library), щоб дослідити сотні доступних моделей — від невеликих помічників з написання коду до великих моделей для міркування.
- **Створюйте власні моделі**: Використовуйте [Modelfile](https://github.com/ollama/ollama/blob/main/docs/modelfile.md), щоб задати власні системні підказки, температуру та інші параметри для персоналізованого досвіду.
- **Розробляйте з API**: Використовуйте клієнтські бібліотеки [Python](https://github.com/ollama/ollama-python) або [JavaScript](https://github.com/ollama/ollama-js) для інтеграції Ollama у ваші застосунки.
- **Підключайте до інтерфейсів**: Поєднуйте Ollama з такими інструментами, як [Open WebUI](https://github.com/open-webui/open-webui), для багатофункціонального інтерфейсу чату з пошуком, персонажами та завантаженням документів.

Для отримання додаткової інформації ознайомтеся з [документацією Ollama](https://github.com/ollama/ollama/blob/main/README.md).