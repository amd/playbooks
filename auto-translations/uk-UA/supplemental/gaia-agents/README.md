...
Агенти GAIA — це AI-асистенти, які використовують локальну LLM для міркування та виклику визначених вами інструментів — як чат-боти, що можуть виконувати дії. Вони працюють **на 100% локально** без хмарних API, без передачі даних за межі вашого комп'ютера та без необхідності в API-ключах.

У цьому посібнику ви побудуєте агента Hardware Advisor, який визначає RAM, GPU та NPU вашої системи, запитує локальний каталог моделей і рекомендує, які LLM може запустити ваш комп'ютер. Це практичне введення в GAIA Agent SDK, яке дає одразу корисний результат.

## Що ви дізнаєтесь

- Як створити агента GAIA з власними інструментами
- Використання LemonadeClient SDK для запиту системної інформації та каталогів моделей
- Платформо-залежне визначення GPU/NPU (Windows PowerShell та Linux lspci)
- Розрахунок розміру моделі на основі пам'яті за правилом 70%
...
> **Примітка**: Якщо VS Code не встановлено, ви можете встановити його через Ryzen AI Developer Center.
...
Спочатку запустіть готового агента, щоб побачити, що ви будуєте. Потім ми крок за кроком розберемо код.

### Запуск готового прикладу

Цей посібник містить повний файл [hardware_advisor_agent.py](assets/hardware_advisor_agent.py). Завантажте його до обраної директорії та запустіть, щоб побачити готового агента в дії:
...
**Вітаємо** — ви побудували агента!

Решта посібника пояснює, як працює кожна частина скрипту, щоб ви могли зрозуміти його з нуля.
<!-- @os:windows -->
...
Агент Hardware Advisor поєднує три компоненти:

- **LemonadeClient SDK** — API для системної інформації та каталогу моделей
- **Платформо-залежне визначення** — Windows PowerShell / Linux lspci для інформації про GPU
- **Розрахунки пам'яті** — правило 70% для безпечного визначення розміру моделі

Дані проходять через них послідовно: запит користувача → агент обирає інструмент → інструмент викликає LemonadeClient + визначення ОС → агент синтезує результати в рекомендацію.

### LemonadeClient SDK

LemonadeClient надає уніфікований API для визначення системи, доступності NPU/GPU та запитів до каталогу моделей.
...
### Платформо-залежне визначення GPU

Агент використовує команди, вбудовані в ОС, а не PyTorch для визначення GPU. Це працює без встановлених драйверів GPU, визначає всі GPU (не лише ті, що підтримують CUDA), та уникає важких імпортів бібліотек.

<!-- @os:windows -->

На Windows агент використовує PowerShell для запиту WMI:
...
На Linux агент використовує lspci:
...
### Правило 70% для пам'яті

> **Правило:** Розмір моделі має бути менше 70% доступної RAM, щоб залишити 30% накладних витрат для операцій виведення (KV-кеш, буфери пакетної обробки, пікові витрати пам'яті під час виконання).
...
## Покрокове написання коду агента (необов'язково)

Ви створите **один файл** під назвою `hardware_advisor_agent.py` і поступово додаватимете функції. Кожен крок будується на попередньому.

### Крок 1: Скелет агента

Почніть з мінімальної структури агента — лише клас і базовий системний промпт. Агент ще не має інструментів.
...
### Крок 2: Визначення GPU та апаратного забезпечення

Додайте допоміжний метод `_get_gpu_info()` та інструмент `get_hardware_info()`. Це робить агента інтерактивним — тепер ви можете запитувати його про характеристики системи.
...
**Оновіть блок `__main__`** для увімкнення інтерактивного тестування:
...
Додайте інструмент `list_available_models()` всередині `_register_tools()`, після функції `get_hardware_info`. Тепер агент може розповісти вам, які моделі доступні.
...
Додайте інструмент `recommend_models()` всередині `_register_tools()`, після `list_available_models`. Тепер агент може розраховувати, які моделі вміщуються в пам'ять вашої системи за правилом 70%.
...
### Крок 5: Виробничий CLI

Замініть простий блок `__main__` на відполірований інтерактивний CLI. Це додає банер, команди виходу та кращу обробку помилок.
...
Ваш файл `hardware_advisor_agent.py` тепер має містити всі ці компоненти:
...
- [x] Допоміжний метод `_get_gpu_info()` (Windows PowerShell + Linux lspci)
- [x] Інструмент `get_hardware_info()` з полями GPU, NPU та ОС
- [x] Інструмент `list_available_models()` з мітками та збагаченням розміру
- [x] Інструмент `recommend_models()` з правилом 70%, fits_in_ram, fits_in_gpu
...
> **Порада**: Повна реалізація доступна у файлі [hardware_advisor_agent.py](assets/hardware_advisor_agent.py).

## Наступні кроки

- **Дослідіть API LemonadeClient** — Відкрийте більше можливостей для управління системою та моделями в [документації LemonadeClient SDK](https://amd-gaia.ai/sdk/lemonade-client)
- **Додайте голосову взаємодію** — Інтегруйте Whisper ASR та Kokoro TTS, щоб користувачі могли задавати питання про апаратне забезпечення голосом. Дивіться [посібник Talk](https://amd-gaia.ai/guides/talk)
- **Додайте підтримку MCP** — Опублікуйте hardware advisor як MCP-сервер, щоб інші інструменти могли його запитувати. Дивіться [посібник MCP](https://amd-gaia.ai/sdk/infrastructure/mcp)
- **Розширте рушій рекомендацій** — Враховуйте VRAM GPU для вивантаження шарів або додайте бенчмаркінг для оцінки кількості токенів за секунду
- **Побудуйте мультиагентну систему** — Поєднайте hardware advisor з агентом коду або агентом чату за допомогою [Routing Agent](https://amd-gaia.ai/guides/routing)