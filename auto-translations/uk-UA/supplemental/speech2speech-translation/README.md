...
Програмне забезпечення AMD ROCm™ та стек PyTorch створюють єдину екосистему для ШІ на пристрої. Воно працює як на Windows, так і на Linux з офіційною підтримкою широкого спектру пристроїв, включаючи APU Ryzen™ AI та GPU Radeon™.

Цей посібник навчить вас запускати низькозатримкове, виразне та приватне перетворення мовлення в мовлення повністю на периферії.

## Що ви дізнаєтесь

- Як налаштувати середовище для перетворення мовлення в мовлення
...
- Усуває перешкоди між перекладом та мовними бар'єрами
- Передає тон, емоції та наміри без незручних пауз
- Забезпечує глобальну співпрацю та прискорює прийняття рішень
...
<!-- @os:linux -->
<!-- @device:halo_box -->
На Linux відкрийте термінал і виконайте наступну команду для створення venv з уже встановленими ROCm+Pytorch:
...
На Linux відкрийте термінал і виконайте наступну команду для створення venv:
...
<!-- @os:windows -->
<!-- @device:halo_box -->
На Windows відкрийте термінал у вибраному каталозі та виконайте команди для створення venv з уже встановленими ROCm+Pytorch:
...
## Налаштування демонстрації перетворення мовлення в мовлення
...
Цей посібник містить готові до використання скрипти. Будь ласка, завантажте всі їх до того самого каталогу, де ви створили середовище.

| Скрипт | Опис | Використання |
|--------|-------------|-------|
| [infer.py](assets/infer.py) | Базова генерація тексту за допомогою LLM | `python infer.py` |
| [input1.wav](assets/input1.wav) | Приклад аудіофайлу | N/A |
| [lang_list.py](assets/lang_list.py) | Файл підтримки мов | N/A |
| [gradio_demo.py](assets/gradio_demo.py) | Зручний інтерфейс для перекладу мовлення | `python gradio_demo.py --no-share` |
...
python infer.py
```
> **Примітка**: Ви можете побачити деякі попередження. Це очікувана поведінка.
 
  
#### Пояснення коду
**Фрагмент 1: Імпорт необхідних залежностей**
...
**Фрагмент 3: Вхідний аудіокліп у форматі .wav та його попередня обробка**

Ця функція завантажує аудіокліп і перетворює його частоту дискретизації до цільового значення.
```python
...
Тепер, коли ви запустили базовий приклад скрипту, наступні інструкції надають зручний інтерфейс, який базується на написаному нами коді та спрощує живий переклад мовлення в мовлення.
...
```
Потім відкрийте веб-браузер за адресою `http://127.0.0.1:7860` для доступу до інтерфейсу.
...
- Комбінуйте десятки мов для швидкого перекладу.
- Поділіться своєю демонстрацією з іншими: додайте --share для створення публічного посилання, доступного будь-кому віддалено, або розгорніть постійно за допомогою Hugging Face Spaces
...
* Спільний доступ та розгортання Gradio: [Посібник із спільного доступу до застосунку](https://www.gradio.app/guides/sharing-your-app) та [Розгортання на Hugging Face Spaces](https://shafiqulai.github.io/blogs/blog_5.html)