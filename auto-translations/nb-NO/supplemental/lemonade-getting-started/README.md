...
<!-- @github-only -->
> [!IMPORTANT]
> Denne veiledningen bruker spesielle tagger som GitHub ikke kan gjengi. Besøk [amd.com/playbooks](https://amd.com/playbooks) for å forhåndsvise dette innholdet korrekt.
<!-- @github-only:end -->
...
* **Utforske web-grensesnittet** og prøve ulike modaliteter som visjon, tale-til-tekst og bildegenerering.
* **Bytte mellom GPU-backends** mellom Vulkan og AMD ROCm™-programvare.
...
- **Python 3.10–3.13** (brukt i Python-app-delen)
- En internettforbindelse (kablet eller trådløs)
<!-- @device:halo_box,halo,stx,krk -->
- [Valgfritt] En AMD XDNA 2 NPU (Ryzen AI 300/400/Max 300-serien eller Z2 Extreme) med den nyeste driveren installert fra [Ryzen AI Software installasjonsinstruksjoner](https://ryzenai.docs.amd.com/en/latest/inst.html#install-npu-drivers) hvis du vil kjøre en modell på NPU-en.
<!-- @device:end -->
...
| **Ansvarsatskillelse** | Modellhåndtering, strømming og feiltoleranse håndteres av serveren, slik at utviklere kan fokusere på appen sin. |
...
Dette betyr at ethvert bibliotek eller enhver app som støtter OpenAI kan snakke med Lemonade ved å peke det mot `http://localhost:13305/api/v1` mens Lemonade Server kjører.

## Hovedaktivitet — Din første lokale AI-samtale

La oss laste ned en LLM og ha en samtale med den, hvor AI-en kjører helt og holdent på din egen maskin.

### Trinn 1: Last ned og kjør en modell

Lemonade leveres med et kuratert modellbibliotek. La oss starte med **Gemma-4-E2B-it**, en dyktig og kompakt modell som inkluderer visjonsstøtte. Åpne en terminal og kjør:
...
Modellen vil svare direkte i chattevinduet. **Gratulerer! Du kjører en stor språkmodell lokalt.**
...
### Trinn 2: Utforsk webgrensesnittet og de ulike modalitetene

Lemonade har et innebygd webgrensesnitt der du kan:
...
Prøv å bytte mellom ulike modaliteter ved hjelp av fanen **Model Manager** i webgrensesnittet, der du kan bla gjennom modeller etter Recipe eller Category:

1. **Vision:** `Gemma-4-E2B-it-GGUF`-modellen du allerede har lastet inn, støtter vision. Lim inn et bilde i chatteboksen og be modellen om å beskrive det.
2. **Bildegenerering:** I kategorien Image kan du laste ned en bildemodell, som `SDXL-Turbo`, fra Model Manager, og deretter bruke Lemonade Image Generator til å skrive inn en prompt og generere et bilde lokalt.
3. **Lyd:** I kategorien Audio kan du laste ned en lydmodell, som `Whisper-Tiny`, som kan gjøre tale om til tekst. Angi en lydopptak for å transkribere det lokalt. For tekst-til-tale kan du prøve en av modellene i kategorien Speech, som `kokoro-v1`.

![Multi-Modality with Lemonade](../../dependencies/assets/multi_modality.png)

### Trinn 3: Prøv en modell med en annen backend

Hvis du holder musepekeren over en modell i Lemonade-appen, ser du et tannhjulikon. Ved å klikke på dette kan du velge alternativer for modellen, blant annet ønsket backend.
...
For å administrere de installerte backendene dine klikker du på backend-knappen i kolonnen lengst til venstre.
...
Den virkelige styrken til en lokal AI-server er at hvilken som helst applikasjon kan koble seg til den med bare noen få linjer med kode. For å bevise dette skal vi bygge en liten, men fungerende **generator for studiekort (flashcards)**, der du oppgir et emne, den genererer studiekort, og du kan teste deg selv interaktivt.

### Trinn 4: Start serveren

Kontroller at Lemonade-serveren kjører. Den starter vanligvis automatisk i bakgrunnen etter installasjon. For å kontrollere dette, kjør:
...
Hvis serveren ikke kjører, starter du den ved å åpne Lemonade-appen. Bruk standardporten **13305** (du kan bekrefte eller velge denne fra systemstatusfeltikonet).

### Trinn 5: Installer OpenAI Python-klienten

I en terminal oppretter du et venv og installerer OpenAI Python-klienten med følgende kommandoer:
<!-- @os:linux -->
```bash
...
### Trinn 6: Bygg flashcard-appen

La oss laste ned en annen modell for å generere kode: `Qwen3.5-35B-A3B-GGUF`. Dette er en stor (~20 GB) og ytelsessterk modell som passer best for systemer med 32 GB+ RAM. Hvis du har mindre RAM tilgjengelig, kan du i stedet prøve `Qwen3.5-9B-GGUF` (~6 GB).
...
Vi bruker Qwen3.5-35B-A3B-GGUF (en større modell som er bedre til å skrive kode) til å generere Python-appen vår, og selve appen kaller opp Gemma-4-E2B-it-GGUF (den mindre modellen du allerede har lastet ned) under kjøring. Koden kan deretter kopieres til en fil etter eget valg for å kjøres i Python.
...
> **Tips**: Vi har fulgt standard ingeniørpraksis gjennom grundig utforming av prompten og ved å bruke et to-modellsystem for å optimalisere ressurser og hastighet.

For enkelhets skyld har vi lagt ved eksempelresultat i [`flashcards.py`](assets/flashcards.py). Du kan gjerne laste den ned til katalogen din. Uansett bør du nå ha en Python-fil som kan kjøres.
...
I løpet av rundt 150 linjer med kode har du bygget et fullt fungerende studieverktøy drevet av en lokal LLM. Det finnes ingen API-nøkkel å administrere, ingen bruksutgifter, og ingen data forlater noensinne maskinen din.

> **Nøkkelinnsikt:** Legg merke til at linjen `client = OpenAI(base_url=...) ` er det *eneste* som knytter denne appen til Lemonade i stedet for OpenAIs sky. Resten av koden er identisk med det du ville skrevet mot en hvilken som helst OpenAI-kompatibel tjeneste. Hvis du noen gang har brukt OpenAI Python-biblioteket, vet du allerede hvordan du bygger apper med Lemonade.

### Hva dette demonstrerer

Denne lille appen viser flere virkelige integrasjonsmønstre:

| Mønster | Hvor det forekommer |
|---------|-----------------|
| **Systemprompter** | `"system"`-meldingen ber LLM-en om å produsere strukturert JSON |
| **Strukturert utdata** | Appen tolker LLM-ens svar som JSON for å bygge studiekort |
| **Statsløse forespørsler** | Hvert kall til `generate_flashcards()` er uavhengig |
| **Feilhåndtering** | `try/except` håndterer på en elegant måte tilfeller der LLM-ens utdata ikke er gyldig JSON |

Disse samme mønstrene skalerer til enhver applikasjon, som chatboter, kodeassistenter, innholdsgeneratorer og automatiseringsverktøy.

#### Bonusutfordring

* For en ekstra utfordring kan du prøve å oppdatere appen slik at studiekortene leses opp for brukeren, ved å ta utgangspunkt i eksempelet gitt [her](https://github.com/lemonade-sdk/lemonade/blob/main/examples/api_text_to_speech.py).
...
Hybridmodeller deler arbeidet mellom NPU-en og iGPU-en for en god balanse mellom hastighet og effektivitet. I Lemonade-appen velger du en modell fra `Ryzen AI LLM`-listen, for eksempel `Qwen3-4B-Hybrid`, eller kjører den med følgende kommando:
...
> **Hva skjer i bakgrunnen?** Når du sender en melding, behandler NPU-en hele prompten din parallelt (dette kalles "prefill"). Deretter tar iGPU-en over og genererer svaret ett token om gangen (dette kalles "decode"). Denne hybride tilnærmingen utnytter styrkene til hver brikke.

### Steg 9: Kjør en FLM-modell

FastFlowLM (FLM)-modeller er spesifikt optimalisert for AMDs XDNA2 NPU-arkitektur og kan være svært raske i forhold til størrelsen. Velg for eksempel `qwen3.5-4b-FLM` fra `FastFlowLM NPU`-listen, eller bruk følgende kommando:
...
<!-- @os:linux -->
<!-- @device:halo_box,halo,stx,krk -->
Når `Lemonade`-appen startes for første gang, er `FastFlowNPU`-backenden ikke aktivert som standard.
...
For en spesifikk modell, last ned ønsket modell fra [modellsiden](https://fastflowlm.com/docs/models/qwen/) og valider den ved å bruke Shell-kommandoen som er oppgitt i dokumentasjonen.
```
...
Flashcard-appen fra steg 6 fungerer også med NPU-modeller, bare endre modellnavnet:
...
2. **Bla gjennom flere modeller**: Utforsk hele [modellbiblioteket](https://lemonade-server.ai/docs/server/server_models/) for å finne modeller optimalisert for koding, resonnering, syn og mer. Bruk Lemonade-appen eller `lemonade list` for å se hva som er tilgjengelig.
...
5. **Bidra**: Lemonade er open source. Sjekk ut [bidragsguiden](https://github.com/lemonade-sdk/lemonade/blob/main/docs/contribute.md) og se etter [Good First Issues](https://github.com/lemonade-sdk/lemonade/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22).