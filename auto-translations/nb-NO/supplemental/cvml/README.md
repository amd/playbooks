...
<!-- @github-only -->
> [!IMPORTANT]
> Dette spilleboken bruker spesielle tagger som GitHub ikke kan gjengi. Besøk [amd.com/playbooks](https://amd.com/playbooks) for å forhåndsvise innholdet korrekt.
<!-- @github-only:end -->

## Oversikt

[Ryzen AI CVML Library](https://ryzenai.docs.amd.com/en/latest/ryzen_ai_libraries.html#ryzen-ai-cvml-library) er et AMD C++ verktøysett for datamaskinvisjon og maskinlæring som gir kraftige, lokale persepsjonsevner — inkludert dybdeestimering, ansiktsdeteksjon og ansiktsmesh-sporing. Biblioteket er bygget på toppen av Ryzen AI-driverne og velger automatisk den beste tilgjengelige maskinvaren (GPU eller NPU) for inferens, slik at du kan legge til AI-funksjoner i C++-applikasjoner uten å bekymre deg for modelltrening eller rammeverksintegrasjon. All behandling skjer lokalt på systemet ditt, noe som gjør det ideelt for personvernfølsomme applikasjoner med lav ventetid.

Denne spilleboken lærer deg hvordan du setter opp Ryzen AI CVML Library, bygger de medfølgende eksempelapplikasjonene og kjører ansiktsdeteksjon på et eksempelbilde.

## Hva du vil lære

- Hvordan installere forutsetninger og sette opp Ryzen AI CVML Library på systemet ditt
- Hvordan CVML C++ API fungerer: kontekster, funksjonsobjekter og bildebuffere
- Hvordan bygge og kjøre de medfølgende eksempelapplikasjonene ved hjelp av CMake og OpenCV
- Hvordan kjøre ansiktsdeteksjon på et bilde med avgrensningsbokser og landemerker
...
- [Visual Studio 2022 Community](https://aka.ms/vs/17/release/vs_community.exe) med arbeidsmengden "Desktop development with C++" (inkluderer MSVC-kompilator, Windows SDK og C++-byggeverktøy)
<!-- @os:end -->

<!-- @os:linux -->
- OpenCV 4.11 — må bygges fra kildekode (apt-pakker på Ubuntu 22.04 og 24.04 leverer ikke versjon 4.11). Se [Bygge OpenCV fra kildekode](#building-opencv-from-source) nedenfor.
- CMake — installer via apt:
  ```bash
  sudo apt install cmake
  ```
- Ubuntu 22.04 eller 24.04 (kjerne >= 6.11.0-21-generic)
- [Ryzen AI NPU driver](https://ryzenai.docs.amd.com/en/latest/linux.html#install-npu-drivers) (Linux-installasjonsprogram — kreves for NPU-inferens)
- Vulkan SDK (installert i [Vulkan SDK](#vulkan-sdk)-avsnittet nedenfor)
<!-- @os:end -->
...
Etter nedlasting pakker du ut pakken til en lokal katalog (f.eks. `C:\RyzenAI-Library` på Windows eller `~/RyzenAI-Library` på Linux) og setter miljøvariabelen `AMD_CVML_SDK_ROOT` til den utpakkede plasseringen:
...
Last ned, konfigurer og bygg OpenCV 4.11.0 med contrib-modulene (referanse: [OpenCV Linux-installasjonsopplæring](https://docs.opencv.org/4.11.0/d7/d9f/tutorial_linux_install.html#tutorial_linux_install_quick_build_contrib)):
...
#### Ytterligere Ubuntu 24.04-avhengigheter
...
CVML Library tilbyr et enkelt C++ API der hver persepsjonsfunksjon (dybdeestimering, ansiktsdeteksjon, ansiktsmesh) har sin egen headerfil og funksjonsobjekt. Du arbeider ikke med rå modeller — biblioteket håndterer modellinnlasting, forbehandling og inferens automatisk.

### Tilgjengelige funksjoner

| Funksjon | Headerfil | Beskrivelse |
|---------|------------|-------------|
| **Dybdeestimering** | `cvml-depth-estimation.h` | Genererer per-piksel dybdekart fra RGB-bilder |
| **Ansiktsdeteksjon** | `cvml-face-detector.h` | Oppdager ansikter med avgrensningsbokser, landemerker (øyne, nese, munn) og konfidenspoeng |
| **Ansiktsmesh** | `cvml-face-mesh.h` | Sporer detaljert ansiktsgeometri med tette meshpunkter |

### Programmeringsmodell

Enhver CVML-applikasjon følger det samme firetrinns mønsteret:

1. **Opprett en kontekst** — `amd::cvml::Context` administrerer delte ressurser som logging og valg av inferensbakende.
2. **Opprett et funksjonsobjekt** — Instansier den spesifikke funksjonen (f.eks. `amd::cvml::DepthEstimation`) mot konteksten.
3. **Pakk inn inndata** — Bruk `amd::cvml::Image` til å kapsle inn RGB-bildebufferen din uten å kopiere data.
4. **Utfør** — Kall funksjonens behandlingsmetode og les resultatene.
...
Biblioteket velger automatisk den beste maskinvaren (GPU eller NPU) for hver operasjon. Du kan også angi bakenden eksplisitt:
...
> **Merk:** Funksjoner som bruker ONNX-bakenden for NPU-operasjoner kan oppleve lengre oppstartsventetid ved første kjøring. Påfølgende kjøringer vil være raskere.

> **Merk:** Hvis NPU-driveren ikke er installert på målsystemet, vil Ryzen AI CVML-biblioteket automatisk falle tilbake til GPU-bakenden for inferensoperasjoner.

## Bygge eksempelapplikasjonene

CVML Library inkluderer klare eksempelapplikasjoner for hver funksjon. La oss bygge dem alle på én gang.
...
2. Bygg eksemplene med CMake:
   ...
   Etter en vellykket bygging befinner de kjørbare filene seg i:
...
3. Før du kjører noe eksempel, sørg for at CVML-kjøretidsfilene er tilgjengelige:
...
## Kjøre ansiktsdeteksjon

Ansiktsdeteksjonseksempelet oppdager ansikter i et bilde, en video eller et direktekamerabilde. Det tegner avgrensningsbokser, konfidenspoeng og fem ansiktslandemerker (to øyne, nese og to munnkanter) på hvert oppdaget ansikt.

Naviger først til mappen med den kjørbare ansiktsdeteksjonsfilen:
...
Last deretter ned et eksempelbilde å bruke som inndata (foto av [Jopwell](https://www.pexels.com/photo/man-in-gray-crew-neck-shirt-smiling-on-focus-photo-895863/), gratis å bruke via Pexels):

```bash
curl -L -o sample_face.jpg "https://images.pexels.com/photos/895863/pexels-photo-895863.jpeg?cs=srgb&dl=pexels-jopwell-895863.jpg&fm=jpg"
```

**Kjør ansiktsdeteksjon på eksempelbildet:**
...
Et vindu vil vises med bildet med avgrensningsbokser rundt oppdagede ansikter, konfidenspoeng og ansiktslandemerkepunkter (øyne, nese, munnkanter).

<p align="center">
  <img src="assets/human_face_output.png" alt="Face detection output showing bounding box, confidence score, and facial landmarks" width="600"/>
</p>

**Lagre den annoterte utdataen til en fil:**
...
Ansiktsdeteksjonsfunksjonen tilbyr to modellvarianter:

| Modell | Hastighet | Nøyaktighet | Best for |
|-------|-------|----------|----------|
| `fast` (standard) | Høyere FPS | God | Sanntids kameraapplikasjoner |
...
For å bruke CVML Library i ditt eget C++-prosjekt, legg det til via CMakes `find_package`:
...
Der `AMD_CVML_SDK_ROOT` peker til roten av Ryzen AI CVML Library-mappen. Inkluder deretter den aktuelle headeren for funksjonen du ønsker:
...
For hvert eksempel nedenfor, naviger til den kjørbare mappen først, og følg samme mønster som i avsnittet [Kjøre ansiktsdeteksjon](#running-face-detection) ovenfor (f.eks. `cd build\cvml-sample-depth-estimation\Release` på Windows eller `cd build/cvml-sample-depth-estimation` på Linux). På Windows, legg til `.exe` på hver kommando (f.eks. `cvml-sample-depth-estimation.exe`).

- **Prøv dybdeestimering**: Kjør `cvml-sample-depth-estimation -i sample_face.jpg` for å generere et fargelagt dybdekart — objekter nær kameraet vises i varme farger, fjerne objekter i kalde farger
- **Utforsk ansiktsmesh**: Kjør `cvml-sample-face-mesh -i sample_face.jpg` for å se tett ansiktsgeometri-sporing med detaljerte meshpunkter
...
- **Sammenlign modellvarianter**: Prøv `-m precise` mot standard `-m fast` på ansiktsdeteksjon for å se nøyaktighets-/hastighetsavveiningen i praksis
- **Bygg din egen app**: Bruk CMake-integrasjonen og C++ API til å legge til CVML-funksjoner i dine egne C++-applikasjoner
- **Kombiner funksjoner**: Koble ansiktsdeteksjon med dybdeestimering i samme applikasjon for rikere sceneforståelse
- **Bla gjennom kildekoden**: Les [Ryzen AI CVML Library på GitHub](https://github.com/amd/RyzenAI-SW/tree/main/Ryzen-AI-CVML-Library) for headerdokumentasjon, ytterligere eksempler og API-detaljer