...
Denne spilleboken lærer deg hvordan du finjusterer LLM-er ved hjelp av LLaMA-Factory på din lokale AMD-maskinvare.

<!-- @device:stx,krk -->
> **Merk:** Finjusteringsteknikkene i denne spilleboken krever minst **32 GB systemminne (RAM)**, der minst **16 GB må være tilgjengelig for GPU-en** (de 16 GB er en del av de 32 GB, ikke i tillegg til dem).
<!-- @device:end -->


<!-- @device:rx7900xt,rx9070xt,r9700 -->
<!-- @os:windows -->
> **Merk:** Finjusteringsteknikkene i denne spilleboken krever minst **16 GB totalt GPU-minne** og **32 GB systemminne (RAM)**.
> - På Windows kombinerer totalt GPU-minne grafikkortets dedikerte VRAM med delt GPU-minne (lånt fra systemminnet).
> - Derfor kan kort med mindre enn 16 GB dedikert VRAM likevel kjøre denne spilleboken ved å bruke delt GPU-minne for å dekke differansen.
<!-- @os:end -->

<!-- @os:linux -->
> **Merk:** Finjusteringsteknikkene i denne spilleboken krever et grafikkort med minst **16 GB dedikert GPU-minne** og **32 GB systemminne (RAM)**.
> - På Linux kjører treningen utelukkende i grafikkortets dedikerte VRAM.
> - Den faller ikke tilbake til delt GPU-minne (systemminne) når VRAM er fullt.
...
- Varighet: Det vil ta omtrent 60 minutter å kjøre denne spilleboken (avhengig av modell-/datasett-størrelse og nettverkshastighet).
...
Last ned kildekoden fra [LLaMA Factory offisielle GitHub-repositorium](https://github.com/hiyouga/LlamaFactory), og installer avhengighetene.
...
Kontroller om `llamafactory-cli` er kjørbart.
...
Denne delen dekker hvordan du forbereder finjusteringsdatasett, konfigurerer LoRA/QLoRA-parametere og kjører LoRA-finjustering.

### Forberedelse av datasett

LLaMA-Factory støtter finjusteringsdatasett i Alpaca-format og ShareGPT-format. Alle tilgjengelige datasett er definert i [dataset_info.json](https://github.com/hiyouga/LlamaFactory/blob/main/data/dataset_info.json). Hvis du bruker et egendefinert datasett, må du sørge for å legge til en datasettbeskrivelse i `dataset_info.json` og angi datasettnavnet før trening. Detaljer finnes i dokumentasjonen [her](https://llamafactory.readthedocs.io/en/latest/getting_started/data_preparation.html).

I denne spilleboken vil vi bruke datasettene identity og alpaca_en_demo som eksempel, og konfigurere datasettinformasjonen i neste trinn.
...
Disse eksempelkonfigurasjonsfilene har angitt modellparametere, parametere for finjusteringsmetode, datasettparametere, evalueringsparametere og mer. Du kan konfigurere dem etter egne behov. I denne spilleboken vil vi bruke [qwen3_lora_sft.yaml](https://github.com/hiyouga/LlamaFactory/blob/main/examples/train_lora/qwen3_lora_sft.yaml).

**Forklaring av nøkkelparametere:**
- `model_name_or_path` - Hugging Face-modellnavn eller lokal modellfilsti.
- `stage` - Treningsstadium. Alternativer: rm (reward modeling), pt (pretrain), sft (Supervised Fine-Tuning), PPO, DPO, KTO, ORPO.
- `do_train` - true for trening, false for evaluering
- `finetuning_type` - Finjusteringsmetode. Alternativer: freeze, lora, full
- `lora_rank` - Dimensjonaliteten til lavrangsmatrisen brukt i LoRA, typiske verdier: 4, 6, 8, 16 (lavere verdier = færre parametere = raskere finjustering; høyere verdier = bedre oppgavetilpasning, men høyere ressursbruk).
...
- `logging_steps` - Loggingsintervall i trinn
- `save_steps` - Lagringsintervall for modellsjekkpunkt.
- `overwrite_output_dir` - Om utdatamappen skal tillates overskrevet.
- `per_device_train_batch_size` - Treningsbatchstørrelse per enhet.
- `gradient_accumulation_steps` - Antall trinn for gradientakkumulering.
- `learning_rate` - Læringsrate
- `num_train_epochs` - Antall treningsepoker
- `lr_scheduler_type` - Læringsrateskjema. Alternativer: linear, cosine, polynomial, constant, osv.
...
<!-- @os:windows -->
Vi vil oppdatere standard LoRA-finjusteringskonfigurasjon for bedre kompatibilitet med AMD Ryzen™ og AMD Radeon™ GPU-er:
...
- Sett `dataloader_num_workers` til `0` på Windows for å unngå `"Can't pickle local object<>"`-feil forårsaket av flerprosesskjøring av datalasting.
...
**llamafactory-cli** er det offisielle kommandolinjegrensesnittet (CLI) for LLaMA-Factory, utviklet for å forenkle ende-til-ende LLM-arbeidsflyter (dataforberedelse → finjustering → evaluering → distribusjon) uten å skrive kompleks kode.

For trening/finjustering er **llamafactory-cli train** kjerneunderkommandoen til LLaMA-Factory CLI. Den abstraherer finjusteringsarbeidsflyter (dataforbehandling, hyperparameterjustering, maskinvareoptimalisering) til én enkelt CLI-kommando, støtter flere finjusteringsparadigmer (LoRA/QLoRA/Full Fine-Tuning) og er optimalisert for GPU-er med begrenset ressurser (f.eks. QLoRA på 16 GB VRAM).

Du kan kjøre LLaMA-Factory-finjustering med følgende kommando, som er basert på den modifiserte konfigurasjonsfilen for Qwen3 LoRA-finjustering.
...
Etter å ha kjørt LLM-finjustering, lagres alle genererte utdata i "output_dir", inkludert modellsjekkpunktfiler, konfigurasjonsfiler og treningsmålinger.
...
**llamafactory-cli chat** er designet for interaktiv chat/inferens med LLM-er (både grunnmodeller og LoRA-finjusterte modeller). LLaMA-Factory tilbyr eksempelkonfigurasjon for å kjøre inferens av finjusterte modeller i [examples/inference](https://github.com/hiyouga/LlamaFactory/tree/main/examples/inference). Du kan også endre denne eksempelkonfigurasjonen for å endre innstillingene, for eksempel inferensbakenden.
...
`LLaMA-Factory` støtter også kodeløs finjustering av LLM-er via et nettgrensesnitt i nettleseren.
...
```
`LlamaFactory Web UI` tilbyr et strømlinjeformet grensesnitt for å administrere maskinlæringsarbeidsflyter, inkludert trening, evaluering, prediksjon, chat og eksport av modeller. Her er en kort introduksjon til hver fane:
...
* **Chat**: Når treningen er fullført, laster du inn modellen i Chat-fanen for å samhandle med den og se resultatene av arbeidet ditt. Denne funksjonen muliggjør sanntidskommunikasjon med den trente modellen.
...
For detaljert veiledning oppfordrer vi deg til å se den offisielle dokumentasjonen i [LlamaFactory GitHub-repositoriet](https://github.com/hiyouga/LlamaFactory#fine-tuning-with-llama-board-gui-powered-by-gradio) og [LlamaFactory ReadTheDocs](https://llamafactory.readthedocs.io/en/latest). I tillegg gir [Wiki LLaMA Board Web UI](https://deepwiki.com/xtong-zhang/Chain-of-Focus/3.2-llama-board-web-ui) verdifull innsikt i grensesnittet og dets funksjoner.

## Neste steg
- Prøv forskjellige modeller som `gpt-oss` og andre toppmoderne modeller.
- Eksperimenter med forskjellige bakender på den finjusterte modellen
