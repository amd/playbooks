...
<!-- @github-only -->
> [!IMPORTANT]
> Denne playbooken bruker spesielle tagger som GitHub ikke kan gjengi. Besøk [amd.com/playbooks](https://amd.com/playbooks) for å forhåndsvise dette innholdet på riktig måte.
<!-- @github-only:end -->

<!-- @device:stx,krk,rx7900xt,rx9070xt,r9700 -->
> [!NOTE]
> Denne playbooken krever minimum **32 GB** systemminne.
<!-- @device:end -->

## Oversikt

[Open WebUI](https://docs.openwebui.com) er et selvhostet, nettleserbasert grensesnitt som gir en kjent chatbot-opplevelse samtidig som det fungerer som et frontend for en eller flere AI-modellservere. I stedet for å være bundet til én leverandør kan Open WebUI koble til **enhver backend som eksponerer et OpenAI-kompatibelt API**, slik at du kan bytte modeller og funksjoner uten å bytte brukergrensesnitt.

I denne playbooken bruker vi [**Lemonade**](https://lemonade-server.ai) som backend fordi den eksponerer et **samlet OpenAI-kompatibelt endepunkt** som støtter flere modaliteter:
...
- Laste opp et bilde og stille spørsmål til en visjonsmodell om det
- Generere bilder fra tekstprompter med Stable Diffusion-modeller (SDXL-Turbo / SDXL)
...
Gjennom denne playbooken vil du jobbe med to separate tjenester:
...
Denne playbooken krever at Lemonade kjører som backend, og på Linux trengs en containermotor (Podman) for å kjøre Open WebUI. Sett opp dette før du installerer Open WebUI.
...
2. Bla gjennom de tilgjengelige modellene og last ned dem du vil bruke (f.eks. en LLM for chat, en visjonsmodell og/eller en Stable Diffusion-modell for bildegenerering).
3. Bekreft at API-et er tilgjengelig ved å besøke `http://localhost:13305/api/v1/models` i nettleseren din — du bør se modellene du har lastet ned, oppført der.
...
Dette henter Open WebUI-avbildet og skriver til persistent lagring.

Start Open WebUI ved å skrive `localhost:8080` i adresselinjen til nettleseren din.
...
<!-- @os:linux -->
> Containeren kjører i bakgrunnen. Fra mappen som inneholder `compose.yml`, kan du administrere den med `podman compose down` (stoppe) og `podman compose up -d` (starte). Kontoene og innstillingene dine lagres i `open_webui_data`-volumet.
<!-- @os:end -->
...
Nå som begge tjenestene kjører — Lemonade på `localhost:13305` og Open WebUI på `localhost:8080` — kobler vi dem sammen slik at Open WebUI kan bruke Lemonades modeller.
...
2. I Settings-panelet klikker du på **Admin Settings** nederst til venstre.

   <p align="center">
     <img src="assets/click_admin_settings.png" alt="Select Admin Settings" width="450"/>
   </p>

3. I sidefeltet for Admin Settings klikker du på **Connections** (eller navigerer direkte til `http://localhost:8080/admin/settings/connections`).
...
5. Sørg for at det under **"Manage OpenAI API Connections"** kun er `http://localhost:13305/api/v1` som er aktivert. Deaktiver eventuelle andre tilkoblinger (f.eks. standard OpenAI-tilkoblingen).
...
Nå er alt satt opp. La oss se på tre interessante ting du kan gjøre.
...
<!-- @os:windows -->
<!-- @device:halo,stx,krk -->
1. Klikk på nedtrekksmenyen øverst til venstre i grensesnittet. Dette viser Lemonade-modellene du har installert. Velg en for å fortsette. (eksempel: `Qwen3-4B-Hybrid`).

    <p align="center">
      <img src="assets/model_selection.png" alt="Model Selection" width="600"/>
    </p>

2. Skriv inn en melding til LLM-en og klikk på send (eller trykk Enter). LLM-en vil bruke noen sekunder på å lastes inn i minnet, og deretter vil du se svaret strømme inn.
...
<!-- @device:rx7900xt,rx9070xt,r9700 -->
1. Klikk på nedtrekksmenyen øverst til venstre i grensesnittet. Dette viser Lemonade-modellene du har installert. Velg en for å fortsette. (eksempel: `Qwen3.5-4B-GGUF`).
...
4. Åpne nå `Task Manager` på systemet ditt. Du vil se **høy GPU- eller NPU-bruk** avhengig av om modellen du valgte er **Hybrid** eller **NPU**. Ved hjelp av oppgavebehandlingen kan du bekrefte at du kjører modellen lokalt.
...
Dette viser at Open WebUI kan sende multimodale forespørsler (tekst + bilde) gjennom backend-en (Lemonade) til en Vision-modell.
...
4. Hvis du vil legge til flere parametere, kan du legge dem til i tekstfeltet som JSON. For eksempel: `{ "steps": 4, "cfg_scale": 1 }`. Se tilgjengelige parametere på [Image Generation (Stable Diffusion CPP)](https://lemonade-server.ai/models.html).
...
Dette viser at Open WebUI kan koordinere en "to-delt" arbeidsflyt:
  - LLM-en hjelper med å finjustere prompten
  - Bildet genereres via Lemonades Images-endepunkt ved bruk av Stable Diffusion
<!-- @os:end -->
...
1. I Lemonade GUI (`http://localhost:13305`), søk etter `SDXL-Turbo` (rask) eller `SDXL-Base-1.0` (høyere kvalitet) og last det ned.
2. Gå til **Admin Settings → Images** (http://localhost:8080/admin/settings/images)
3. Angi:
   - **Image Generation:** ON
   - **Image Generation Engine:** Default (OpenAI)
...
- Sjekk først Lemonade: åpne `http://localhost:13305/api/v1/models` i en nettleser og bekreft at modellene dine er oppført og nedlastet
...
- Start med `SDXL-Turbo` først (raskt, færre steg)
...
Du har nå en fungerende **«lokal AI-stack»**, ett enkelt grensesnitt som styrer flere modelltyper gjennom et standard-API.

Her er tre utvidelser som åpner for helt nye arbeidsflyter:

### 1. Tale-til-tekst med Whisper

Prøv å gjøre lyd om til tekst ved hjelp av en Whisper-modell, og mat den så inn i en LLM for oppsummering, oppgavepunkter eller omskriving. Dette er grunnlaget for møtenotater og talestyrte assistenter.

### 2. Python-koding inne i Open WebUI

Bruk Open WebUIs innebygde opplevelse for kjøring av kode til å kjøre Python-utdrag, inspisere utdata og iterere raskere—uten å forlate grensesnittet. [Referanse](https://lemonade-server.ai/docs/server/apps/open-webui/#python-coding)

### 3. HTML-rendering inne i Open WebUI

Vis HTML-utdata direkte i grensesnittet. Dette er overraskende kraftfullt for å bygge raske prototyper, formaterte rapporter og interaktive utdrag. [Referanse](https://lemonade-server.ai/docs/server/apps/open-webui/#html-rendering)
