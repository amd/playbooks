...
> [!NOTE]
> Denne veiledningen krever minimum **32 GB** systemminne.
<!-- @device:end -->

## Oversikt

Kodeagenter er kraftige verktøy som gir utviklere styrke gjennom samarbeid med AI-agenter drevet av store språkmodeller (LLM-er). De kan integreres i utviklingsmiljøet, som terminalen eller VS Code, slik at de sømløst kan integreres i en utviklers arbeidsflyt.
...
* Hvordan kjøre VS Code med kodeagenten Cline for å hjelpe med programvareutviklingsoppgaver.
...
Vi skal bruke LM Studio til å tjene LLM-en som driver kodeagenten.
...
Deretter må vi laste inn LLM-en på systemet. Vi kommer til å bruke modellen `Qwen3-Coder-30B-A3B` med en stor kontekstlengde. (Bruk fanen Model for å installere den hvis du ikke allerede har gjort det).
...
- Kontroller statusbryteren og sørg for at den er satt til `Running`.
...
- Et Cline-ikon skal vises til venstre. Klikk på det for å åpne Cline. Det vil dukke opp et vindu som spør `How will you use Cline?` Siden vi skal bruke en lokal LLM som kjører via LM Studio, velger du `Bring my own API Key` og trykker `Continue`. 
...
>**Tips**: Nyere modeller kan være tilgjengelige. Vurder å laste ned og bytte til Qwen3.6-modeller om ønskelig.
...
La oss bruke vår lokale agent til å lage et nettsted! Åpne VS Code i en mappe etter eget valg, der Cline vil opprette filene.
- For å gjøre dette går du til `File -> Open Folder` øverst til venstre i VS Code og velger en mappe som `Documents`.

![Tom mappe i VS Code](assets/open-cline-test.png)

Nå er vi klare til å be den lokale kodeagenten om noe. 
- Klikk på Cline-utvidelsen i venstre kolonne og skriv inn en ledetekst for å starte agenten. La oss for eksempel bruke følgende ledetekst:
```code
Create a website showcasing the ability to run local large-language models on an AMD device.
```

Agenten vil deretter begynne å opprette filer i henhold til ledeteksten. Som bruker kan du se koden genereres i VS Code, som vist nedenfor. Det kan hende du må klikke på `Save` hver gang Cline vil opprette en fil. 

![Cline kodegenerering](assets/cline-code-generation.png)

Etter at programvaren er generert, er agenten ferdig, og du kan kjøre applikasjonen. I dette tilfellet skrev agenten til tre filer: `index.html`, `script.js` og `styles.css`. Ved ganske enkelt å dobbeltklikke på HTML-filen kan vi laste inn og samhandle med det genererte nettstedet.
...
Etter at du har generert nettstedet, kan du fortsette å jobbe med Cline for å forbedre nettstedet. To mulige forbedringer er:
...
- **Retro arkadespill**: Prøv noen andre prompter. Det kan også være morsomt for agenten å lage retrostilte spill i Python ved hjelp av `PyGame`-pakken med følgende prompt:

```code
Create a simple pong game using the PyGame python package.
```

- **Dataanalyse**: Et område hvor kodeagenter er spesielt nyttige, er skripting og dataanalyse. Dette er en prompt for å vise frem den lokale modellens evne til å generere dataanalyseprogramvare for visualisering av aksjekurser:
...
Nedenfor er noen flere ressurser for å lære mer om kodeagenter, Cline og kjøring av arbeidsbelastninger på 

* Mer informasjon om AMD sitt partnerskap og integrasjon med LM Studio: https://www.amd.com/en/ecosystem/isv/consumer-partners/lm-studio.html
