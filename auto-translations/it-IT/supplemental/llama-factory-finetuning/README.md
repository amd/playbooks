...
Il fine-tuning efficiente è fondamentale per adattare i modelli linguistici di grandi dimensioni (LLM) a compiti specifici. LLaMA Factory è una piattaforma open-source e di facile utilizzo che semplifica l'addestramento e il fine-tuning di modelli linguistici di grandi dimensioni e di modelli multimodali. Consente agli utenti di personalizzare centinaia di modelli pre-addestrati localmente con una codifica minima.

Questo playbook ti insegna come effettuare il fine-tuning degli LLM utilizzando LLaMA Factory sul tuo hardware AMD locale.

<!-- @device:stx,krk -->
> **Nota:** Le tecniche di fine-tuning descritte in questo playbook richiedono almeno **32 GB di RAM di sistema**, con almeno **16 GB disponibili per la GPU** (i 16 GB fanno parte dei 32 GB totali, non si aggiungono ad essi).
<!-- @device:end -->
...
> - Non passa alla memoria GPU condivisa (RAM di sistema) quando la VRAM si esaurisce.
...
**Concedi al tuo utente l'accesso ai dispositivi GPU** (esegui il logout e nuovamente il login affinché questo abbia effetto):
...
Dopo aver installato con successo LLaMA Factory, procediamo con l'esecuzione del fine-tuning.
...
LLaMA Factory supporta dataset per il fine-tuning nei formati Alpaca e ShareGPT. Tutti i dataset disponibili sono stati definiti nel file [dataset_info.json](https://github.com/hiyouga/LlamaFactory/blob/main/data/dataset_info.json). Se stai utilizzando un dataset personalizzato, assicurati di aggiungere una descrizione del dataset in `dataset_info.json` e di specificare il nome del dataset prima dell'addestramento. Maggiori dettagli sono disponibili nella loro documentazione [qui](https://llamafactory.readthedocs.io/en/latest/getting_started/data_preparation.html).
...
LLaMA Factory supporta molteplici schemi di fine-tuning.
...
- `model_name_or_path` - Nome del modello su Hugging Face o percorso del file del modello locale.
...
- `lora_rank` - La dimensionalità della matrice a basso rango utilizzata in LoRA, valori tipici: 4, 6, 8, 16 (valori più piccoli = meno parametri = fine-tuning più veloce; valori più grandi = migliore adattamento al task ma maggiore utilizzo di risorse).
...
- `save_steps` - Intervallo di salvataggio dei checkpoint del modello.
...
- Impostare `dataloader_num_workers` a `0` su Windows per evitare errori `"Can't pickle local object<>"` causati dal caricamento dei dati multiprocesso.
...
Per l'addestramento/fine-tuning, **llamafactory-cli train** è il sottocomando principale della CLI di LLaMA Factory. Astrae i flussi di lavoro di fine-tuning (preelaborazione dei dati, ottimizzazione degli iperparametri, ottimizzazione hardware) in un unico comando CLI, supportando molteplici paradigmi di fine-tuning (LoRA/QLoRA/Full Fine-Tuning) ed è ottimizzato per GPU a basse risorse (ad esempio QLoRA su 16GB di VRAM).

È possibile eseguire il fine-tuning di LLaMA Factory utilizzando il seguente comando, basato sul file di configurazione modificato per il fine-tuning LoRA di Qwen3.
...
Dopo aver eseguito il fine-tuning dell'LLM, tutti gli output generati vengono memorizzati nella directory "output_dir", inclusi i file di checkpoint del modello, i file di configurazione e le metriche di addestramento.
...
```
Di seguito è mostrato un esempio di chat che utilizza il modello sottoposto a fine-tuning:
...
```
<!-- @test:end -->
## Utilizzare la GUI di LLaMA Factory
...
```
La `LlamaFactory Web UI` offre un'interfaccia semplificata per gestire i flussi di lavoro di machine learning, tra cui addestramento, valutazione, predizione, chat ed esportazione dei modelli. Ecco una breve introduzione a ciascuna scheda:

* **Train**: questa scheda consente di selezionare un modello e un dataset, configurare i parametri di addestramento e avviare il processo di addestramento. È essenziale comprendere i parametri obbligatori e opzionali per ottimizzare la configurazione dell'addestramento.
* **Evaluate & Predict**: dopo l'addestramento, è possibile valutare le prestazioni del modello ed effettuare predizioni utilizzando questa scheda. Fornisce informazioni sull'accuratezza e sull'efficacia del modello su nuovi dati.
...
* **Export**: questa scheda facilita l'esportazione dei modelli addestrati per la distribuzione o per un ulteriore utilizzo. Puoi salvare i tuoi modelli in vari formati adatti a diverse applicazioni.

Per indicazioni dettagliate, ti invitiamo a consultare la documentazione ufficiale sul [repository GitHub di LlamaFactory](https://github.com/hiyouga/LlamaFactory#fine-tuning-with-llama-board-gui-powered-by-gradio) e su [LlamaFactory ReadTheDocs](https://llamafactory.readthedocs.io/en/latest). Inoltre, il [Wiki LLaMA Board Web UI](https://deepwiki.com/xtong-zhang/Chain-of-Focus/3.2-llama-board-web-ui) fornisce informazioni preziose sull'interfaccia e sulle sue funzionalità.

## Prossimi passi
- Prova modelli diversi come `gpt-oss` e altri modelli all'avanguardia.
- Sperimenta con backend diversi sul modello sottoposto a fine-tuning
