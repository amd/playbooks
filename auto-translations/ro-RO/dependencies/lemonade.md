...
<!-- @os:windows -->
Descărcați cel mai recent program de instalare de pe [lemonade-server.ai](https://github.com/lemonade-sdk/lemonade/releases/latest/download/lemonade.msi) și rulați fișierul `.msi`.

După instalare:
- CLI-ul `lemonade` este adăugat automat în PATH-ul sistemului
- Serverul Lemonade este configurat să ruleze automat în fundal

De asemenea, îl puteți instala silențios din linia de comandă:
```cmd
...
Pentru alte distribuții sau pentru a instala din sursă, consultați [opțiunile complete de instalare](https://lemonade-server.ai/docs/guide/install/).
<!-- @os:end -->
...
Pentru referință rapidă, iată câteva comenzi CLI Lemonade uzuale:

| Comandă | Ce face |
| --- | --- |
| `lemonade --help` | Afișează toate comenzile și opțiunile disponibile. |
| `lemonade --version` | Afișează versiunea Lemonade instalată. |
| `lemonade status` | Confirmă dacă serverul Lemonade rulează și poate fi accesat. Adresa URL implicită de bază pentru API-ul compatibil OpenAI este `http://localhost:13305/api/v1`. |
...
| `lemonade config` | Afișează valorile configurației Lemonade curente. |
| `lemonade config set llamacpp.backend=rocm` | Setează backend-ul implicit llama.cpp la ROCm. |

Pentru cele mai recente opțiuni ale serverului Lemonade sau depanare, vă rugăm să consultați [documentația oficială Lemonade](https://lemonade-server.ai/docs/lemonade-cli/).