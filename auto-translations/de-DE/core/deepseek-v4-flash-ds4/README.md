...
[ds4 (Dwarf Star 4)](https://github.com/antirez/ds4) ist eine dedizierte Inferenz-Engine, die speziell für diese Modellarchitektur entwickelt wurde. Statt einer universellen Laufzeitumgebung zielt ds4 direkt auf die DeepSeek-V4-Familie ab, mit architekturspezifischen Kernel-Optimierungen für AMD ROCm™ Software. Es ist derzeit eine der leistungsstärksten Implementierungen von DeepSeek V4 Flash auf Strix Halo.
...
- Wie Sie den ds4-ROCm-Toolbox-Container erstellen
...
> - Der GPU-Shared-Memory-Pool **auf mindestens 110 GB eingestellt**: Führen Sie `amd-ttm --set 110` aus (siehe den obigen Schritt zur Speicherkonfiguration) und starten Sie neu. Niedrigere Werte führen beim Laden des Modells mit 126k Kontext zu Out-of-Memory-Fehlern. Wenn Ihr System über weniger verfügbaren Speicher verfügt, verringern Sie stattdessen den Wert **Context** im Server-Modus.
...
Der Autor von ds4 stellt mehrere quantisierte Versionen von DeepSeek V4 Flash im GGUF-Format bereit. Alle unten aufgeführten Modelle verwenden eine Importance-Matrix-Kalibrierung (imatrix), die eine höhere Präzision für die Teile des Modells beibehält, die für Coding- und Reasoning-Aufgaben am wichtigsten sind.
...
| [Hybrid Q2/Q4 imatrix](https://huggingface.co/antirez/deepseek-v4-gguf) | ~97 GB | Behält die Schichten 37–42 in Q4-Präzision für höhere Genauigkeit bei. Passt in 128 GB, lässt aber weniger Platz für Kontext |
...
[ds4-cockpit](https://github.com/kyuz0/strix-halo-ds4-toolbox) ist eine leichte Terminal-UI, die den Einstieg in ds4 auf Strix Halo erleichtert. Sie übernimmt das Erstellen von Toolbox-Containern, das Herunterladen von Modellgewichten und das Starten von Servern. Installieren Sie es mit `pipx`:
...
Wählen Sie im Tab **Interactive Toolboxes** die neueste verfügbare Toolbox (z. B. `ds4-rocm-7.2.4`) aus und klicken Sie auf **Create/Update**. Dadurch wird das Container-Image abgerufen und die Toolbox-Umgebung erstellt.

> **Tipp**: Die Toolbox-Version ändert sich im Laufe der Zeit, wenn neuere ROCm-Builds veröffentlicht werden. Wählen Sie die neueste in der Liste verfügbare Version.
...
Gehen Sie zum Tab **Model Manager**. Wählen Sie **IQ2_XXS imatrix (~80,8 GB)** aus dem Dropdown-Menü aus und klicken Sie auf **Download**. Die Modelldateien werden standardmäßig unter `~/ds4` gespeichert (Sie können den Speicherpfad ändern).
...
Gehen Sie zum Tab **Server Mode**. Wählen Sie das heruntergeladene Modell und die Toolbox aus und konfigurieren Sie dann die Kontextgröße (z. B. 126000), Host und Port (8000). Wenn alles bereit ist, klicken Sie auf **Start ds4-server**.

> **KV Disk Cache (optional).** Wenn Sie **KV Disk Cache** aktivieren, wird der KV-Cache auf die Festplatte ausgelagert (unter **Host Cache Dir**, standardmäßig `~/.cache/ds4-kv`), sodass wiederholte System-Prompts von der SSD wiederhergestellt werden, anstatt neu berechnet zu werden. Dies ist eine Leistungsoptimierung für Coding-Agent-Workflows mit langen, wiederholten Prompts und **nicht erforderlich**, um den Server auszuführen.

<p align="center">
  <img src="assets/ds4-cockpit-server-mode.png" alt="Configuring and starting the ds4 server" width="800"/>
</p>

Der Server startet und lauscht auf Port 8000, wobei ein OpenAI-kompatibler API-Endpunkt unter `http://localhost:8000/v1` bereitgestellt wird.
...
- **Multi-Node-Clustering**: Wenn Sie über zwei Halo-Geräte verfügen, unterstützt ds4 die Verteilung des Q4-Modells (~153 GB) auf beide Rechner mittels Pipeline-Parallelität. Setup-Anweisungen finden Sie in der [ds4-toolbox-Dokumentation](https://github.com/kyuz0/strix-halo-ds4-toolbox#distributed-inference-pipeline-parallelism).
- **Spekulative Dekodierung (MTP)**: Laden Sie die MTP-Gewichte (~3,6 GB) herunter und übergeben Sie `--mtp` an den Server, um die Generierungsgeschwindigkeit zu erhöhen.
