...
Ollama هي أداة خفيفة الوزن وشائعة لتشغيل نماذج اللغة الكبيرة محليًا. تتولى تنزيل النماذج وضغطها وتقديمها من خلال واجهة سطر أوامر بسيطة وتطبيق سطح مكتب، مما يتيح لك البدء بالمحادثة مع نموذج LLM في غضون دقائق.

يرشدك هذا الدليل خلال تثبيت Ollama، وتنزيل نموذج GPT-OSS 20B، وإجراء محادثة معه، سواء عبر الطرفية أو تطبيق سطح المكتب.

## ما ستتعلمه

- كيفية تثبيت Ollama وتشغيله على نظامك
- تنزيل نموذج GPT-OSS 20B وتشغيله محليًا
- الدردشة مع النماذج باستخدام واجهة سطر الأوامر
- الاستعلام عن النماذج برمجيًا عبر REST API
...
1. قم بتنزيل المثبّت من [ollama.com/download](https://ollama.com/download).
2. شغّل مثبّت `.exe` واتبع التعليمات.
...
يجب أن ترى رقم الإصدار المثبّت مطبوعًا في وحدة التحكم.
<!-- @os:end -->

<!-- @os:linux -->

شغّل سكريبت التثبيت الرسمي:
...
تدير Ollama النماذج عبر سجل مشابه لصور الحاويات. لتنزيل GPT-OSS 20B:

```bash
ollama pull gpt-oss:20b
```

يؤدي هذا إلى تنزيل أوزان النموذج على جهازك المحلي (حوالي 12 غيغابايت). يحدث التنزيل مرة واحدة فقط، وتُحمَّل عمليات التشغيل اللاحقة النموذجَ من القرص.

يمكنك التأكد من توفر النموذج بالأمر:

```bash
ollama list
```

يجب أن ترى `gpt-oss:20b` في الإخراج مع حجمه وتاريخ آخر تعديل.
...
تتبع أسماء نماذج Ollama الصيغة `name:tag`. تشير العلامة عادةً إلى عدد المعاملات أو نوع الضغط. بعض الأوامر المفيدة لإدارة النماذج:

| الأمر | الوصف |
|---------|-------------|
| `ollama list` | عرض جميع النماذج المنزَّلة |
| `ollama pull <model>` | تنزيل نموذج دون تشغيله |
| `ollama rm <model>` | حذف نموذج لتحرير مساحة القرص |
| `ollama show <model>` | عرض بيانات النموذج ومعاملاته |

## الدردشة من الطرفية

أطلق جلسة دردشة تفاعلية مباشرةً من سطر الأوامر:

```bash
ollama run gpt-oss:20b
```

تُحمِّل Ollama النموذج في الذاكرة وتنقلك إلى موجه الأوامر. جرّب أن تسأله شيئًا:

```
>>> What is the capital of France and why is it historically significant?
```

يبثّ النموذج استجابته رمزًا تلو الآخر مباشرةً في الطرفية. اكتب `/bye` أو اضغط `Ctrl+D` للخروج من الجلسة.

> **تلميح**: يستغرق التشغيل الأول بضع ثوانٍ لتحميل النموذج في الذاكرة. تستجيب المطالبات اللاحقة ضمن الجلسة ذاتها بشكل أسرع بكثير لأن النموذج يبقى محملًا.

<!-- @os:windows -->
## الدردشة من تطبيق سطح المكتب

يأتي Ollama أيضًا مع تطبيق سطح مكتب يوفر واجهة دردشة أنيقة للتفاعل مع نماذجك.
...
2. اختر **gpt-oss:20b** من القائمة المنسدلة للنموذج في الزاوية السفلية اليمنى من منطقة إدخال الدردشة.
...
يحتفظ تطبيق سطح المكتب بسجل محادثاتك في الشريط الجانبي، مما يسهّل العودة إلى المحادثات السابقة.
<!-- @os:end -->

## استخدام REST API

بعد التثبيت، يعمل Ollama كخدمة في الخلفية ويعرض REST API على `http://localhost:11434` يمكنك استخدامه لدمج النماذج في تطبيقاتك وسكريبتاتك الخاصة.
...
الاستجابة عبارة عن كائن JSON يحتوي على مخرجات النموذج في حقل `response`.


### مثال بلغة Python
الآن بعد أن أصبح بإمكاننا الوصول إلى Ollama API برمجيًا، لنستدعِه من Python.
...
<!-- @os:end -->
#### إنشاء ملف Python
في المجلد ذاته، استخدم VS Code أو محررًا آخر لإنشاء ملف .py وانسخ الكود التالي فيه. ثم شغّل الملف في بيئتك المُفعَّلة باستخدام `python your_file_name.py`
...
|----------|--------|---------|
| `/api/generate` | POST | توليد نص أحادي الدور |
| `/api/chat` | POST | محادثة متعددة الأدوار مع سجل الرسائل |
| `/api/tags` | GET | سرد النماذج المتاحة |
| `/api/show` | POST | عرض تفاصيل النموذج |
| `/api/pull` | POST | تنزيل نموذج من السجل |

للاطلاع على مرجع API الكامل، راجع [توثيق Ollama API](https://github.com/ollama/ollama/blob/main/docs/api.md).

## الخطوات التالية

- **جرّب نماذج مختلفة**: تصفّح [مكتبة نماذج Ollama](https://ollama.com/library) لاستكشاف مئات النماذج المتاحة، من مساعدي البرمجة الصغار إلى نماذج الاستدلال الكبيرة.
- **أنشئ نماذج مخصصة**: استخدم [Modelfile](https://github.com/ollama/ollama/blob/main/docs/modelfile.md) لضبط مطالبات النظام المخصصة ودرجة الحرارة وغيرها من المعاملات للحصول على تجربة مصمَّمة خصيصًا.
- **ابنِ باستخدام API**: استخدم مكتبات العميل [Python](https://github.com/ollama/ollama-python) أو [JavaScript](https://github.com/ollama/ollama-js) لدمج Ollama في تطبيقاتك.
- **اربط بواجهات أمامية**: اقرن Ollama بأدوات مثل [Open WebUI](https://github.com/open-webui/open-webui) للحصول على واجهة دردشة غنية بالميزات مع البحث والشخصيات ورفع المستندات.

لمزيد من المعلومات، راجع [توثيق Ollama](https://github.com/ollama/ollama/blob/main/README.md).