...
function Invoke-LmsLoadAttempt([int]$Attempt) {
  ...
  try {
    lms load ${lms_model} --context-length 32768 --gpu max --identifier "$ID" -y 1> $stdoutFile 2> $stderrFile
...
run_lms_load_attempt() {
  ...
  set +e
  LOAD_STDOUT="$(lms load ${lms_model} --context-length 32768 --gpu max --identifier "$ID" -y 2>"$stderr_file")"
  LOAD_EXIT=$?
  set -e
