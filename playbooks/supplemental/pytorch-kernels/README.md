...
**How it works:**
<!-- @test:id=test-vector-addition timeout=300 -->
```python
import torch
...
**Run:**
<!-- @test:id=run-vector-addition-add-one-kernel-py timeout=600 -->
```bash
...
<!-- @test:id=run-vector-addition-add-one-kernel-cu timeout=600 -->
```bash
python "assets/Vector Addition/setup.py" build_ext --inplace
```
<!-- @test:end -->
...
<!-- @test:id=verify-vector-addition-add-one-kernel-so timeout=400 hidden=True -->
import os
os.chdir("assets/Vector Addition")
import add_one_ext
import torch

x = torch.ones(10, device="cuda")
print(add_one_ext.add_one(x))
<!-- @test:end -->
...
**How it works:**
<!-- @test:id=test-matmul-kernel timeout=800 -->
```python
import torch
...
"""
# Inputs: A is M x N, B is N x P, C is M x P
M, N, P = 1024, 512, 768
...
**Run:**
<!-- @test:id=run-matmul-kernel-py timeout=800 -->
```bash
python "assets/Matrix Multiplication/matmul_kernel.py"
```
<!-- @test:end -->
...
<!-- @test:id=run-matmul-kernel-cu timeout=800 -->
```bash
python "assets/Matrix Multiplication/setup.py" build_ext --inplace
```
<!-- @test:end -->
...
**Step 3: Use from Python:**
<!-- @test:id=verify-matmul-kernel-so timeout=300 hidden=True -->
import os
os.chdir("assets/Matrix Multiplication")
import matmul_ext
import torch
